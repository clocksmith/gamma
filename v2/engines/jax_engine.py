...
class JaxEngine(LLMEngine):
    ...
    def _s(self, l: jnp.ndarray) -> jnp.ndarray: return jax.nn.softmax(l, axis=-1) # type: ignore
    def _temp(self, l: jnp.ndarray, t: float) -> jnp.ndarray: return l / jnp.maximum(jnp.array(t, dtype=l.dtype), 1e-6) if t > 0 else l # type: ignore
    def _k(self, l: jnp.ndarray, k_v: int) -> jnp.ndarray: # type: ignore
        v_s = l.shape[-1]; k = min(k_v, v_s)
        if k <= 0 or k >= v_s: return l
    ...
    def _p(self, l: jnp.ndarray, p_v: float) -> jnp.ndarray: # type: ignore
        if p_v <= 0.0 or p_v >= 1.0: return l
    ...
    def _get_top(self, l: jnp.ndarray, k_sh: int) -> Tuple[List[str], List[float], List[int]]: # type: ignore
        if l.size == 0 or jnp.all(jnp.isinf(l)): return ["<No Valid Tokens>"], [1.0], [-1] # type: ignore
        pr = self._s(l); v_s = pr.shape[-1]; eff_k = min(k_sh if k_sh > 0 else v_s, v_s)
        t_p, t_i = jax.lax.top_k(pr, k=eff_k) # type: ignore
        if t_p.ndim > 1: t_p = jnp.squeeze(t_p, axis=0); t_i = jnp.squeeze(t_i, axis=0) # type: ignore
        t_i_l = np.array(t_i).tolist()
        return ([self.get_token_text(idx) for idx in t_i_l], np.array(t_p).tolist(), t_i_l)
    ...
    def predict_next(self, input_ids: jnp.ndarray, attention_mask: Optional[jnp.ndarray], temperature: float, top_k: int, top_p: float, output_attentions: bool = False, output_hidden_states: bool = False) -> Dict[str, Any]: # type: ignore
        ...
        l_raw = outputs.logits[:, -1, :]; l_temp = self._temp(l_raw.copy(), temperature); l_k = self._k(l_temp.copy(), top_k); l_proc = self._p(l_k.copy(), top_p)
        p_proc = self._s(l_proc); next_id_j = jnp.argmax(p_proc, axis=-1); next_id = np.array(next_id_j).item() # type: ignore
        ...
        return {"next_token_id": next_id, "logits_raw": l_raw, "logits_processed": l_proc, "probabilities_raw": self._s(l_raw), "probabilities_temp": self._s(l_temp),
                "probabilities_top_k": self._s(l_k), "probabilities_processed": p_proc, "top_tokens_processed": top_txts, "top_probs_processed": top_p_vals,
    ...
    def get_probabilities_at_step(self, data: Any, step_name: str, k: int) -> Tuple[List[str], List[float], List[int]]:
        ...
        return self._get_top(probs_tensor, k_sh=k)
