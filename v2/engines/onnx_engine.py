...
class ONNXEngine(LLMEngine):
    ...
    def _s(self, l: np.ndarray) -> np.ndarray: e_x = np.exp(l - np.max(l, axis=-1, keepdims=True)); return e_x / np.sum(e_x, axis=-1, keepdims=True)
    ...
    def _k(self, l: np.ndarray, k_val: int) -> np.ndarray:
        ...
        if k_actual <= 0 or k_actual >= l.shape[-1]: return l
        filtered_l = np.copy(l); indices_to_remove = np.argpartition(l, -k_actual, axis=-1)[..., :-k_actual]
        np.put_along_axis(filtered_l, indices_to_remove, -np.inf, axis=-1); return filtered_l
    def _p(self, l: np.ndarray, p_val: float) -> np.ndarray:
        if p_val <= 0.0 or p_val >= 1.0: return l
        sorted_indices = np.argsort(l, axis=-1)[..., ::-1]; sorted_l = np.take_along_axis(l, sorted_indices, axis=-1)
        cumulative_probs = np.cumsum(self._s(sorted_l), axis=-1); indices_to_remove_sorted = cumulative_probs > p_val
        indices_to_remove_sorted[..., 1:] = indices_to_remove_sorted[..., :-1].copy(); indices_to_remove_sorted[..., 0] = False
        mask_original_order = np.zeros_like(l, dtype=bool); np.put_along_axis(mask_original_order, sorted_indices, indices_to_remove_sorted, axis=-1)
    ...
    def predict_next(self, input_ids: np.ndarray, attention_mask: Optional[np.ndarray], temperature: float, top_k: int, top_p: float, output_attentions: bool = False, output_hidden_states: bool = False) -> Dict[str, Any]:
        ...
        probs_proc = self._s(logits_proc); next_token_id_val = int(np.argmax(probs_proc, axis=-1).item())
        ...
        return {"next_token_id": next_token_id_val, "logits_raw": logits_raw, "logits_processed": logits_proc, "probabilities_raw": self._s(logits_raw),
                ...
                "top_tokens_processed": top_texts_list, "top_probs_processed": top_probs_list, "attention": attention_data, "hidden_states": None, "forward_time": time.time() - st}
