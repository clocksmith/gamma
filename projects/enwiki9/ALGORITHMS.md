...
The closest counted projection is now the repaired original-order
compact-`200` plus FX2-lite endpoint428 wrapper. Its guarded exact-`10M`
archive is `1,635,695` bytes versus the `1,637,513`-byte compact base, a gross
gain of `1,818` bytes or `181.8 B/1M`. With the deterministic `349,195`-byte
source package, the conservative projection is `109,557,404`, leaving
`57,404` bytes (`57.404 B/1M`) to the target. The archive missed its frozen
`1,635,120` ceiling by `575` bytes, so the unchanged blend is retired and no
`1G` gate is authorized. This is an exact guarded `10M` archive screen and a
counted linear projection, not a constructive full-corpus result.
...
| Best forecast | Repaired original-order compact-`200` plus FX2-lite endpoint428: conservative projected `109,557,404` | The exact guarded `10M` archive saves `1,818` bytes over its compact base. Linear extension with the counted `349,195`-byte source package leaves `57,404` bytes to target. This is forecast evidence, not a constructive `1G` bound. |
| Current blocker | No full-corpus constructive result exists. Endpoint428 is correct and decimal-memory-safe at its proved boundary, but its cumulative exact-`10M` gain is `575` archive bytes below the frozen counted screen. | The unchanged `31:9` blend is retired. A successor must add more than `57.404 B/1M` net after its source/code cost and confirm that gain on sealed target-substrate rows. |
| Next gate | A new retrieval representation or a reversible WikiIR compiler component on the exact endpoint428 trace | Do not tune the retired title, typed Skip-CTS, entity-trie, direct event-SRSTC, or blend policies. First require a new candidate universe whose matched-control oracle and causal exact replay clear `57.404 B/1M` plus compressed incremental source cost with stable blocks; only then integrate natively. |
