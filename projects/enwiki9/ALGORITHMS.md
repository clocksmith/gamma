...
| Endpoint428 runtime-qualification branch | BPTT loop-region consolidation improves a matched `250K` encode from `97.3465 s` to a `92.81485 s` median (`4.655%`). Output-region consolidation, one mature-context lookup, and extra-input fusion reach `90.5648 s` with exact stream identity. A fused three-gate dense traversal plus explicit output-weight update loop saves `9` bytes at `250K` and transfers strongly: the exact `10M` archive is `1,634,500` bytes, saving `1,195` bytes (`119.5 B/1M`) over endpoint428 and `674` over the prior best stream. Its deterministic LZMA source package is `280,147` bytes; two clean builds reproduce backend SHA-256 `d1066630...` and wrapper SHA-256 `37ee8cd7...`. | Exact decode and independent clean-build re-encode reproduce archive SHA-256 `93d7f5cb...`; the worst peak single RSS is `9,078,564 KiB`, below decimal `10GB`. After paying the `1,322`-byte package increase, the frozen calibration improves the forecast by `43,806` bytes to `109,408,345` (`10.9408345%`), margin `91,655`. Runtime remains unqualified, and overlapping WGSL host load excludes the `10M` elapsed measurements from controlled timing evidence. The instrumented candidate attributes `49.00%` of aggregate sampled CPU time to `gomp_barrier_wait_end`; active waiting, forced gate SIMD, and the tested transposed AVX2 layouts are negative. Receipt: `results/endpoint428_pair_layer0_runtime_successor_10m_v1/receipt.json`; preserved source delta: `patches/endpoint428_gate_dot_fuse_output_update_loop_v1.patch`. |
...
| Best forecast | Endpoint428 compact/FX2 pair plus the 26-endpoint layer-0 online residual mixer, fused recurrent gate dots, explicit output update, and counted LZMA source ZIP: `109,408,345` (`10.9408345%`) | Exact guarded `10M` encode, roundtrip, independent deterministic re-encode, clean-build identity, and package reconstruction pass. The `91,655`-byte margin is calibrated forecast evidence, not an official `1G` score. |
