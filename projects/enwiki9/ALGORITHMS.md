...
| Endpoint428 runtime-qualification branch | Preserve the exact probability stream while optimizing the native-fast-math, static-OpenMP source reconstruction. BPTT loop-region consolidation improves a matched `250K` encode from `97.3465 s` to a `92.81485 s` median (`4.655%`). Output-region consolidation, one mature-context lookup, and extra-input fusion produce the best exact-stream screen at `90.5648 s`; every archive and guard remains clean. Explicit four-core and eight-core placements take `98.5903 s` and `99.3529 s`, reproduce archive SHA-256 `73d215cc...`, peak at `8,988,784` and `8,988,620 KiB`, and remain below decimal `10GB`. | Thread-count tuning is terminal: neither published-reference core count improves on the unrestricted host. The exact `1M` encode still takes `319.3730 s`. Mixer last-context caching, all-gates single-team BPTT, asynchronous compact FXCM, fused Adam, and interpolated-logistic-table variants are retired because they regress runtime or alter the archive. The next qualifying mechanism must parallelize or replace dominant substrate model groups, not polish the residual mixer or retune OpenMP thread count. Quarantined runtime evidence is under `/home/x/enwiki9-nonproof/results/endpoint428_pair_layer0_native_fastmath_staticgomp_*`. |
| Prepared neutral-FXCM compact endpoint | The quarantined ablation preserves all `431` compact FXCM input slots as neutral values and continuously evolves the remaining compact contexts, PPMD, two-layer `200`-cell LSTM, mixers, and SSE, while excluding the unused FXCM object code and static state. Its deterministic two-entry source ZIP is `222,361` bytes and two clean builds reproduce the `1,297,496`-byte trace backend. | Source/package evidence only; no codelength or RSS result exists yet. Under the same original-order penalty its optimistic package floor is `1,226.871 B/1M`. Run one guarded opening trace only after the active full-endpoint `10M` gate releases the heavy lock. Receipt: `/home/x/enwiki9-nonproof/results/cmix21_lstm200_neutral_fxcm_source_package_v1/receipt.json`. |
| Shared-FXCM compact mixer hypothesis | The exact FX2 attribution trace and compact CMIX21 model expose the same ordered `431`-slot FXCM vector. Feed the already continuously evolved FX2 vector into a second compact-`200` mixer/LSTM/SSE trajectory instead of allocating a duplicate compound FXCM model. | This is dependency-closure evidence and a test plan, not a codelength result. It preserves recurrent continuity, targets the individual interactions lost by family averaging, and removes the largest duplicate state. After the frozen `10M` result, replay the existing exact `1M` FX2 attribution rows through this shared-input endpoint; use the neutral-FXCM endpoint as its matched ablation. Require exact stream identity, fixed-point replay, stable holdout gain above the counted floor, and guarded coexistence RSS before native integration. |
...
| Best forecast | Endpoint428 compact/FX2 pair plus the 26-endpoint layer-0 online residual mixer with the counted LZMA source ZIP: `109,452,151` (`10.9452151%`) | Exact guarded `10M` encode, roundtrip, and independent deterministic re-encode pass. The `47,849`-byte margin is calibrated forecast evidence, not an official `1G` score. |
...
| Next gate | Matched runtime/identity gate for a substrate-level successor to endpoint428 | Preserve the `278,825`-byte package and exact archive identity when possible. Require byte-identical `1K` output first, then matched guarded runtime evidence with enough measured headroom to justify a larger scope. Do not launch the exact full-`1G` gate until runtime qualifies. |
