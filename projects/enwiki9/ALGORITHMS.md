...
| Next gate | Observation-neutral matched compact-`200`/endpoint428 trace and causal blend-policy screen | Freeze fixed-weight, reset-aware, or regret-routed candidates using training/development rows only. Promote the smallest policy only if exact sealed replay clears `57.404 B/1M` plus payload with stable blocks; do not run another broad native gate or `1G` first. |
