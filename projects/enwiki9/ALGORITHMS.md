...
| Counted endpoint428 LZMA source package | A standard direct-entry LZMA-method ZIP containing `Makefile` and readable `source_bundle.sh` is `278,825` bytes. Two independent extractions and clean builds reproduce the exact wrapper-proven backend and program hashes. | The representation is `72,117` bytes smaller than the prior BZip2 ZIP without changing codec bytes. Combined with the exact `10M` archive and completed codec replay, it clears the calibrated target and authorizes the official full gate. Package receipt: `results/endpoint428_pair_layer0_online_lzma_package_v1/receipt.json`; replay receipt: `results/endpoint428_pair_layer0_online_lzma_replay_v1/receipt.json`. |
...
| Current blocker | No exact full-corpus constructive result exists. | The candidate and package are frozen and full-gate-authorized; the remaining blocker is the exact `1,000,000,000`-byte encode, decode, deterministic replay, and final package-plus-archive accounting. |
| Next gate | Guarded exact full-enwik9 proof for the frozen endpoint428 pair/layer-0 mixer and LZMA package | Preserve wrapper, package, corpus, and guard identities. Accept promotion only if exact roundtrip and deterministic replay pass and archive plus `278,825` package bytes total at most `109,500,000`. |
