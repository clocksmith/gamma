...
| WikiIR-MDL reversible compiler | `docs/wikiir_mdl.md`, `programs/wikiir_template_grammar_v1/`, `programs/wikiir_prior_page_delta_v1/`, `programs/wikiir_webgraph_v1/`, `programs/wikiir_title_vertex_v1/`, `programs/wikiir_title_vertex_twopass_v1/`, `tools/wikiir_prior_page_columnar_probe.py`, `tools/wikiir_title_vertex_tail_layout.py`, and `tools/wrt_sequence_memoizer_trace.py` | Highest representation novelty. Builds an exact typed Wiki/XML IR, then permits literal, parameterized grammar, prior-page delta, graph, columnar, Sequence-Memoizer, and Skip-CTS descriptions under complete MDL accounting. | Nine exact `1M` probes now settle the first shapes. The strongest title graph preserves literal titles and reconstructs forward links in two passes: `3,967` fewer raw bytes and `52` LZMA bytes saved before mode, but the guarded hybrid loses `171` (`174,134` versus `173,963`). At `10M` it removes `20,682` raw bytes yet LZMA loses `28`; unchanged preprocessing is retired. No WikiIR row changes the score. | Move title/link reuse into a direct WRT entity-continuation endpoint on the unchanged stream, then typed-field Skip-CTS. Require exact base identity, held-out gain, counted code/state, matched controls, and native replay before integration. |
