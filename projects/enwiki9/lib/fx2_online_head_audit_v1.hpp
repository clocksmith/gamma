...
class Audit {
 public:
  ...
  explicit Audit(Model& m):model(m){
    if(const char* p=std::getenv("GAMMA_FX2_HEAD_STATE")){state_file=std::fopen(p,"wbx");require(state_file);}
    if(const char* p=std::getenv("GAMMA_FX2_HEAD_BASE")){base_file=std::fopen(p,"wbx");require(base_file);}
  }
  ~Audit(){
  ...
  void after_observe(){if(model.observations%2048==0)write_state();}
