...
// Deliberately recomputes the 64-state forward graph; kernel measurements must
// precede any production implementation or corpus gate.
class OpenProfileFixture {
 public:
  ...
  Bytes serialize() const {
    Bytes out{'O','P','F','X',1}; put(out, model_tag, 4);
 ...
 private:
  ...
  void refresh_prediction() {
    ...
    const auto cache = forward(true);
    const auto start = prefix_.size() * 256;
    double sum = 0;
    for (unsigned b = 0; b != 256; ++b) {
      const auto p = cache.probabilities[start + b];
    ...
    unsigned assigned = 0;
    for (unsigned b = 0; b != 256; ++b) {
      const auto scaled = 65280.0 * cache.probabilities[start + b] / sum;
