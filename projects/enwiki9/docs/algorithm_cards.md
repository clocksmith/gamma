...
| Fine-valve `cmix21` PPMD ladder | Keep the strong cmix21 next-bit model, but shave memory in the least damaging place until larger gates fit the RSS guard. | Active proof lane; `ppmd21248k` is packaged after `ppmd21376k` failed unchanged `100M` RSS. |
...
| `cmix21_text_mmap_paq5_ppmd21376k_fxcmidx13div2_fxcmrcm20_ppmdguard2_rcm32_bufthirtysecond_minmaps_v1` | PPMD cap cut after `ppmd21504k` failed unchanged `100M` RSS. | Exact prefix replays passed; unchanged `100M` RSS guard exceeded by `116` KiB before scored archive. | This is now the upper memory bracket for the active cut. | Keep as bracket evidence; do not promote unchanged at `100M`. |
| `cmix21_text_mmap_paq5_ppmd21248k_fxcmidx13div2_fxcmrcm20_ppmdguard2_rcm32_bufthirtysecond_minmaps_v1` | PPMD cap cut after `ppmd21376k` failed unchanged `100M` RSS. | Exact `1M`: archive `174,531`, local score `738,805`, program `564,274`, roundtrip true, determinism true, max sampled single RSS `10,443,844` KiB. | Package is deterministic through `1M`; unchanged `10M` is the active proof gate. | Wait for the unchanged `10M` gate and promote only after exact gate receipts pass. |
