...
| Causal WRT reference-prefix CTS | Completed prior references populate normalized WRT-event continuation tables. The scorer activates only after a causally recognized escaped reference opening, filters candidate codes by already decoded current-event bits, and inserts the current reference only after its close. | Exact FX2 trace/store/archive identity passes on the event-dense `500K` selection window. The best variant loses `1` exact byte and has negative qbits on train, development, and holdout. Its positive-event oracle is just `0.072266` byte. | Retire this endpoint and routing over its event wins unchanged. The raw COPY/ADD oracle does not survive either layout compression or direct FX2 residual prediction. |
| Normalized WRT phrase copy | Long exact and digit/whitespace-normalized completed-event suffixes predict exact next WRT codes. A per-context online regret gate and frozen probability-bucket calibration abstain using only decoder-built state. | Unconditional `k12` loses `72` exact bytes. Context regret saves `4` exact bytes but only `8 B/1M`, with negative holdout qbits. Two frozen calibrators lose `2` and `3` exact bytes and regress on holdout. | Retire the measured endpoint and routers. The causal mechanism finds some complement, but not enough transferable net rate to pay the remaining debt or implementation bytes. |
