...
## 2026-08-09 - Source-built full-1G deterministic score aggregator frozen

Candidate `cmix_obias_source_full1g_determinism_accounting_qm0_v1` is a
dependency-gated receipt aggregator for the active A/B full-corpus source
roundtrips. It refuses to run before both terminal decisions and guards exist.
It requires both archives to reconstruct the exact billion-byte canonical
input, both payload sizes and hashes to match, both self-extracting archive
sizes and hashes to match, the frozen compressor and head identities to match,
temporary disk below `100,000,000,000` bytes, and both process trees below the
strict decimal `9,765,625 KiB` boundary.

The counted local score is frozen as exact source-built archive bytes plus the
`468,481`-byte compressor, `23,002`-byte head, and conservative `48`-byte
invocation. The receipt separately reports identity with the external archive,
distance to `105,000,000`, and margin below `109,685,196`.

A pass will establish deterministic local `C(enwik9)=A` and `D(A)=enwik9`
