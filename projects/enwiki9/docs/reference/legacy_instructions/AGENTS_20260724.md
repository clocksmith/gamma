...
- `results/run_ledger.jsonl` for the canonical append-only per-run registry
  (algorithm_id, scope, inferred purpose, timing, memory, determinism,
  and result path back-pointer);
...
For broad sweeps and re-runs, add explicit run context at driver invocation:

```bash
python3 projects/enwiki9/lib/driver.py <id> \
  --run-purpose baseline|smoke|verification|rebaseline \
  --run-scope-label 1m \
  --run-context gate_1m \
  --run-source queue|manual|script \
  --run-tag baseline \
