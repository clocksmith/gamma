...
| `nncp_delta_midas_named_midpoint_gradient_q3.py` | Launches two closed-teacher F encodes for the zero-credit direct-F32 named-gradient retry and compares its group decisions with the bound q2 control; it must run only as a revision-bound job under the process-tree memory and scratch guard. |
| `materialize_nncp_named_midpoint_gradient_q3.py` | Cannot launch a process; replaces only q2's BF16 product/reduction path with LibNC's direct F32 squared-sum reduction. |
