...
Candidate `fx2_kda_carry_opening250k_v1`, owner `root_kda_carry`, freezes one [native recurrent-memory comparison](fx2_kda_carry_20260913.md) on exposed raw [0,250000). Memory/state-transfer lenses3/4 select fixed1/16 carry at existing KDA article/piece resets; a value-column rotation disrupts semantic alignment, and K computes then discards carry. Reset matches training, so this is a new predictive hypothesis rather than a bug repair. The apparent double-update and already closed closing-name studies are not revived. Optimized/UBSan reset fixtures pass; a prior compiler temporary-file quota failure has no compression interpretation. Twelve codec phases require exact inverses/repeats, byte-identical P/K archives and authoritative KDA reset states, and retained released-parent archive/coder parity. Two same-path builds test binary repeatability; final packaging and full-corpus score remain unknown. The single held job awaits publication and separate-storage admission. Forecast109389323, target90000000, debt+19389323; no new measured gain or objective credit.
