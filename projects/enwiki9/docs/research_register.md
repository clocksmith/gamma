...
finds no transition evidence; [v2 passes eight synthetic tests](../operations/evidence/20260909_opcode_event_opportunity_decode250k_v2_unit.json), preserving the [v1 preflight failure](../operations/evidence/20260909_opcode_event_opportunity_preflight_v1.json). Owner `next_prediction_gate_review` freezes [decode-only observation](../operations/adaptive/experiments/opcode_event_opportunity_decode250k_q0_v2.json); job `20260909T140149Z_274c03e2c0` is held for publication and fresh CPU3 admission. Empty runtime files now authenticate exactly; no retained archive executes, predictor changes or score credit are claimed.
...
The [native fixture passes](../operations/provenance/fx2_weight_sign_magnitude_fixture50051_v1_terminal.json): three exact3223-byte archives, inverses/repeats and nine parent coder traces; guarded peak RAM5,531,414,528 bytes. Package delta-456 remains scoped. The [separately frozen transfer](../operations/adaptive/experiments/fx2_weight_sign_magnitude_transfer250k_q0_v1.json) binds opening/distant250KB,20 phases and18 retained trace comparisons; four runner tests and367-input preflight pass. Job20260909T135014Z_5f0d74a545 is held for publication and admission; [direct resource evidence review](../operations/evidence/20260909_smg_transfer_resource_basis.json) verifies the prior transfer job/guard without changing frozen science. No new corpus result.
...
rows per tensor. It keeps the range coder and all weight bits. This differs from
the rejected neighbor and width-carry policies. Independent source review agrees
on the coding law and flags transitive helper closure, supplied in a separate
prospective execution manifest without changing the frozen algorithm contract.
The [synthetic contract](../operations/adaptive/experiments/fx2_weight_sign_magnitude_v1.json)
requires P/K/parent identity, exact inverses, repeats, every pre-weight count
boundary, exact Q11 vectors, and malformed-stream rejection. One fixed policy,
...
The [published model audit](../operations/provenance/fx2_weight_neighbor_model_v1_plan.json)
passes three synthetic adapter tests, then all111 trained-model tensors after publication
and fresh CPU3 admission. The [terminal receipt](../operations/provenance/fx2_weight_neighbor_model_v1_terminal.json)
verifies1,000 subprocess phases and1,110 artifact references: all inverses/repeats and P/K agree.
Fully framed INT4 substreams total P/K2,782,839 versus D2,879,988 bytes: D loses97,149.
Conditioning saves2,751 range-stream bytes but adds99,900 table bytes. No tensors are excluded.
Reject this fixed table/context realization without another inference or packaging sweep.
All exact artifacts remain locally retained and publish in one verified ZIP to limit file clutter.
This is a substream comparison, not a complete model container or full-corpus score.
