...
Candidate `alias_residual_opportunity1m_q0_v1`, owner `root_alias_residual`, freezes the [natural abbreviation opportunity study](alias_residual_opportunity_20260913.md) using lenses2/9. It reuses the exact strongest-parent opening1M probability trace and its repeated archive certificate, preserving original initialization and dictionary. Explicit article-local definitions and fully decoded opposite-form8-byte donor windows test a new relation beyond the retired generic typed mixture. Five causality fixtures pass. The bounded repeated census reports available parent log loss and donor agreement; these are not saved bits, new archives or improved research-system evidence. The [research context](research_concepts_50.md) now distinguishes reproduction, counted descendant improvement and a separate matched-search RSI claim, with Reploid/Doppler roles and quine accounting kept inside existing lab authority. No model, adapter or competing framework is added.
