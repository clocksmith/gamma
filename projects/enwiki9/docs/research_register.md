...
|---|---|---|---|---|---|
| SRSTC / streaming self-referential retrieval | Causal k-nearest continuation memory over deterministic SimHash/minhash sketches, self-referential span tables, patch-copy priors, and fixed-point regret routing | `docs/streaming_retrieval_mixer.md`, `tools/streaming_retrieval_mixer_plan.py`, `tools/streaming_retrieval_shadow.py`, `tools/streaming_retrieval_raw_shadow.py`, `results/streaming_retrieval_shadow/` | Complete-block raw receipts are now positive through `8,192K`; the best receipt saves `112,212` held-out bytes and `99,924` net bytes after a `12,288` byte code estimate, with `0` block regressions. Conditional-copy attribution is now instrumented so copy availability, router-selected copy, typed retrieval, and byte-prior lift are measured separately. | Exact shadow coder shows positive held-out net bytes after counted code/table cost, adjacent-scope confirmation stays positive, conditional attribution proves the selected copy or typed-retrieval component beats its controls, then the smallest winning component survives prefix replay. | Requires shipping an embedding/index payload, duplicates existing match-model signal, exceeds memory cap, shows concentrated prefix-only gains, fails conditional attribution against typed/byte controls, or fails when integrated into the replayed compressor. |
