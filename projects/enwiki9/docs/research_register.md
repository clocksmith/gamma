...
| Active endpoint428 pair/layer-0 online residual mixer with score-qualified compact replacement | Preserve the frozen causal compact/FX2 pair frontier while replacing FX2 with the frozen compact layer-0 plus WRT phase residual when it improves package economics | `results/endpoint428_pair_layer0_runtime_successor_10m_v1/receipt.json`, `results/endpoint428_dual112_replacement_v1/decision.json`, `/home/x/enwiki9-nonproof/results/endpoint428_pair_layer0_gate_dot_fuse_output_update_loop_minified_lzma_source_package_v1/decision.json`, `patches/endpoint428_gate_dot_fuse_output_update_loop_v1.patch`, `docs/endpoint428_runtime_frontier.md`, `tools/minify_cpp_source.py`, `tools/wrt_phase_residual_native.{h,cpp}` | The frozen pair remains the best counted forecast at `109,389,323` (`10.9389323%`). The compact replacement confirms on the offset-`500M` population, then composes natively at `1K`, `250K`, and `1M`. Its pruned deterministic LZMA package is `235,420` bytes; two reconstructed builds are identical. The exact `174,099`-byte `1M` archive roundtrips and re-encodes byte-identically with peak tree RSS `7,578,364 KiB`. After the `25,705`-byte package saving, the measured projection is `109,499,618` (`10.9499618%`), margin `382`. Runtime remains unqualified. | Preserve the replacement as the score-qualified frontier and pursue a materially faster recurrent representation or model-level concurrency path before another larger gate. | Any score projection above target, nondeterminism, decimal-memory failure, or throughput movement too small to qualify under the official runtime rule terminates the implementation before `1G`. |
| Retired endpoint428 independent page sharding | Reset the dual-`112` half-horizon parent on decoder-reconstructed page ranges and run independent ranges concurrently, with reduced dense-state variants when necessary | `results/endpoint428_page_sharding_terminal_v1/decision.json`, `patches/endpoint428_page_shard_state_scaling_v1.patch`, `tools/enwiki9_page_shards.py`, `tools/enwiki9_shard_container.py`, `tests/test_enwiki9_page_shards.py`, `tests/test_enwiki9_shard_container.py` | Four exact opening-prefix shards roundtrip and pack into a `185,524`-byte container, `11,563` bytes above the `173,961`-byte unsharded archive. Unmodified four-way initialization breaches decimal `10GB` at `9,841,944 KiB` tree RSS. Quarter-state still breaches at `10,293,400 KiB`. One-sixteenth state misses the four-way per-shard ceiling even at `1K`; at `222,281` bytes it fits a three-way ceiling with `3,035,592 KiB` but loses `15` archive bytes (`67.482 B/1M`), above the parent’s `51.677 B/1M` score budget. | None unchanged. Preserve the reversible page container as infrastructure; pursue concurrency inside one shared model state or a genuinely low-state recurrent replacement. | Independent shards require either aggregate memory above the official ceiling or lossy state reduction that projects above target. Do not retry smaller dense tables or the same three/four-way process architecture. |
...
| Retired PAQ8 direct-index and SIMD lookup runtime proxies | Replace the bounded PAQ8 mixer context hash with direct indexing, then compare each ContextMap2 bucket's seven checksums with one SSE2 operation while preserving match and replacement semantics | `patches/cmix21_paq8_direct_context_v1.patch`, `patches/cmix21_paq8_simd_bucket_find_v1.patch`, `results/endpoint428_paq8_direct_context_runtime_proxy_v1/decision.json`, `results/endpoint428_paq8_simd_bucket_find_runtime_proxy_v1/decision.json` | On the closest ancestor, direct indexing preserves six balanced `10K` archives and improves median runtime `2.954%`. SIMD lookup preserves sixteen archives, roundtrips, and improves median runtime `1.136%` with six of eight pairs positive. Optimistically adding both to the exact-source composite raises leverage from `13.963%` to only `18.053%`, versus `83.093%` required. | None unchanged. Preserve the source techniques for a future faster architecture, but do not spend an exact-source or `250K` gate on this bundle. | The best-case runtime bound cannot change the official eligibility decision. |
| Retired PAQ8 ContextMap2 micro-variants | Hoist bit-position dispatch outside the context loop or fast-path the recorded second MRU checksum | `patches/cmix21_paq8_update_specialized_v1.patch`, `patches/cmix21_paq8_second_mru_v1.patch`, `results/endpoint428_paq8_update_specialized_runtime_proxy_v1/decision.json`, `results/endpoint428_paq8_second_mru_runtime_proxy_v1/decision.json` | Both variants preserve every tested archive. Dispatch hoisting moves the median only `0.038%`; the second-MRU shortcut regresses the balanced median `0.341%`. | None unchanged. | Non-material or negative balanced medians terminate both implementations. |
...
| Retired diagonal-reservoir FX2 replacement | Use `8` to `128` decoder-built independent leaky byte states and online fixed-point readouts directly over the same-execution FX2-lite428 endpoint | `tools/endpoint_diagonal_reservoir_screen.cpp`, `tests/test_endpoint_diagonal_reservoir_screen.py`, `results/fx2_diagonal_reservoir_replacement_1m_v1/decision.json` | Development-only selection chooses the zero-update control and saves `0 B/1M`. The best post-hoc full replay saves only `5 B/1M`, regresses `3` blocks, and loses development qbits, versus `681.114 B/1M` required before code. | None unchanged. A future fast recurrent replacement needs cross-state interactions, token/event inputs, or teacher-trained dynamics rather than a larger diagonal ladder. | Zero selected gain and less than `1%` of required post-hoc headroom terminate this representation before native integration. |
| Retired compact-teacher sparse GRU replacement | Distill the same-execution compact endpoint into a `16`-cell causal byte GRU over prior decoded bytes, causal current-byte prefixes, and the fast FX2 logit; select checkpoints on development only | `tools/endpoint_sparse_gru_distill_screen.py`, `tests/test_endpoint_sparse_gru_distill_screen.py`, `results/endpoint_sparse_gru_distill_1m_v1/decision.json` | The teacher exposes `2,830.637` bytes of full-scope headroom and `2,296.853 B/1M` on sealed holdout. The selected float student saves `23` exact bytes overall but loses `4.893` holdout bytes (`-24.466 B/1M`), retaining less than `1%` of teacher headroom against a payload-aware `688.818 B/1M` floor. | None unchanged. Attribute the compact complement to specific state transitions or event representations before training another replacement model. | Negative sealed-holdout transfer and less than `4%` of the required full-scope gain retire this architecture before quantization or native integration. |
| Retired residual-logit sparse GRU replacement | Keep the `16`-cell architecture, split, seed, and gate fixed while replacing teacher KL with direct robust regression on `logit(compact)-logit(FX2)` | `tools/endpoint_sparse_gru_distill_screen.py`, `results/endpoint_sparse_gru_residual_distill_1m_v2/decision.json` | Training loss decreases monotonically, but every development checkpoint regresses. Development selects epoch one; exact replay loses `84` bytes overall and `26.283` bytes on sealed holdout (`-131.417 B/1M`). | None unchanged. Require component/state attribution or a materially different event-level representation before another learned replacement. | The stronger residual objective makes transfer worse, so the v1 miss is representational rather than merely a weak-gradient artifact. |
| Retired dilated causal-byte FX2 residual model | Predict a correction over fast FX2 from exact prior WRT bytes at lags `1/2/4/8/16/32/64`, a learned causal-prefix head, and direct truth BCE; no recurrent bottleneck | `tools/endpoint_dilated_context_screen.py`, `tests/test_endpoint_dilated_context_screen.py`, `results/endpoint_dilated_context_1m_v1/decision.json` | Development selects epoch one. Train gains produce `83` exact full-scope bytes, but development is `-5.202 B/1M` and sealed holdout is `-3.901 B/1M`; every later checkpoint worsens development. Estimated int8 payload raises the floor to `693.186 B/1M`. | None unchanged. Stop generic byte-history replacements and obtain component-level compact-state attribution or build a WRT-event-native endpoint. | Negative development and holdout transfer proves the apparent full-scope gain is train-dominated and cannot justify quantization or native work. |
...
| Retired multi-endpoint Bayesian fixed-share stack | Maintain context-local causal posteriors over preserved P1 endpoints using only decoder-rebuilt online weights | `tools/endpoint_fixed_share_stack.cpp`, `tests/test_endpoint_fixed_share_stack.py`, `results/endpoint_fixed_share_stack_1m_v1/decision.json` | Development-only selection chooses the `99%` base prior and `10`-ppm share byte-regime model. Exact `1M` replay saves only `4` bytes overall and `1` on sealed holdout despite large noncausal span-oracle headroom. | None unchanged. Do not expand prior, share, or context ladders. | Retained gain cannot pay integration or code and does not improve the stronger causal composite. |
...
## 2026-07-26 OMEGA archive-search infrastructure

OMEGA is now integrated into `tools/enwiki9_lab.py` rather than implemented as a separate runner. New durable commands record and list machine-readable exclusions and rank mutation ancestors by descendant productivity. Proposals now carry a mechanism-change class, search bonus or penalty, exposed interfaces, retired neighborhoods, and optional parent proposal. Search ordering favors deleting work, changing the coded alphabet or update schedule, replacing representations, compiling state machines, adding state coordinates, and adding macro families; parameter tuning and mixture expansion are penalized. These priorities never alter exact score, package, runtime, memory, or promotion gates. Five terminal neighborhoods were recorded: XML21 runtime-only shortening, Wheeler truth-to-causal failure, surface symmetry orbits, paid parallel-MDL model description, and epoch-frozen byte contexts. PHASECODE, VULCAN, and EPSILON-COMPILE were registered as separate proposals.

Two workflow defects were repaired while executing the first OMEGA candidate. Adaptive development now atomically registers new candidates in `index.json`. The driver now passes the correct `run_purpose` keyword to `_infer_run_purpose`. Baseline-control failures in candidate triage now classify the tested candidate as `blocked_dependency` rather than retiring it.

## 2026-07-26 VULCAN V0 event-control result

`vulcan_event_control_v0` is a constructive integer arithmetic coder over exact WRT event byte groups. It uses an adaptive event-kind model and bounded categorical byte contexts with a literal path for every byte sequence. The exact raw 1K gate roundtripped deterministically, reduced arithmetic decisions by 75 percent, and produced 738 archive bytes versus 412 for LZMA; its 9,570-byte program made the counted result negative.
...
## 2026-08-02: Cross-device typed-event and JANUS trace synthesis

The synchronized clocksmith findings and the originating-host exact frontier
results agree on the important boundary. Completed trigger/suffix
continuations have large gains against a weak order-1 raw literal model:
19,763 bytes for C0 on opening 1M. They are not new information relative to a
mature compressor. The complete raw archive is 183,180 bytes worse than LZMA,
the explicit receipt-copy IR makes the same LZMA payload 260 to 304 bytes
larger, the older endpoint428 point-mass experts lose 400 to 861 bytes, and the
current recovered endpoint428 E3 realization also loses to its parent. The
Wiki-typed refinements trail their simpler suffix/trigger controls in both raw
and endpoint shadows.

Therefore no trigger, 32-byte suffix, Wiki-key, receipt-copy, point-mass,
global selector, or capacity/threshold rescue is eligible as the next lane.
Those results are merged as complementary causal and constructive evidence,
not as competing forecasts. The exact recovered JANUS-plus-quotient P1 now
authorizes attribution only to find an information source outside these
retired families. A successor may be materialized only after a frozen
...
## 2026-08-02: WIKISECTION exact-heading body-lexicon ceiling proposed

Proposal `wikisection_exact_heading_body_lexicon_qm1_v1` is a dormant
successor to the WIKIBACK decision and receives zero score credit. It uses an
exact completed section heading as a decoder-visible key into exact WRT token
lexicons staged from same-heading section bodies and published only after their
earlier pages close. This differs from the rejected ten-state heading
calibrator, which never indexed exact heading text or prior section bodies, and
from TESSERA's static corpus-wide types, WIKIBACK's page-title/backlink graph,
...
## 2026-08-02: stale SRSTC frontier activity retired

The canonical frontier still marked `srstc_blockposterior_raw_shadow` active
after its unchanged endpoint428 transfer and both frozen residual-program
successors had failed. That stale status is corrected to `retired_unchanged`.
The `65,536K` raw shadow remains historical positive information, but it earns
