...
Prospective distinct challenger: round each INT4 weight/2 ties-to-even, multiply by 2 and clamp to [-6,6]; C rotates each mapped row left one position. The fixed map changes 3,119,371 of 5,868,864 symbols; retained histogram arithmetic suggests 872,951.59 fewer ideal global-entropy bytes per model, without a measured packed size or archive penalty. The source draft preserves original container decoding while changing learned weights; fresh corpus inversion, repeats and causal controls are required. Compare package deltas against the smaller validated marginal model as well as public original to avoid recounting earlier savings.
