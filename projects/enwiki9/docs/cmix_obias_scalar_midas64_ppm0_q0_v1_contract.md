# CMIX-obias Scalar-MIDAS64 PPM0 q0 v1 Contract
...
- Bytes `0..31`: use the exact parent gate `g0 = 0.15f` and collect gradient
  and curvature after each byte truth is decoded.
...
For a completed first-half byte `t`, define `x_t[j]` as the exact FP32 value
