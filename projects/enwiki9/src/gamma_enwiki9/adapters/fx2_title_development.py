"""Matched CPU full-model retraining with causal title conditioning.
...
from .fx2_title_conditioning import conditioning, metadata_entries, validate_features
...
def train(*, upstream: Path, checkpoint: Path, parent_packed: Path,
          token_path: Path, prior_path: Path, feature_path: Path, mode: int, output: Path, plan: dict):
    import numpy as np
    import torch
    ...
    features = validate_features(feature_path, len(tokens))
    if mode not in (1, 2):
        raise ValueError("metadata training mode differs")
    ...
    child = conditioning(model, package)
    initial = {n: p.detach().clone() for n, p in child.named_parameters()}
    active_loss_rows = sum(int(np.count_nonzero(features[start + warmup:end, 0])) for start, end in windows)
    if active_loss_rows == 0:
        raise ValueError("frozen windows have no metadata opportunities")
    optimizer = torch.optim.AdamW(child.parameters(), lr=plan["learning_rate"],
    ...
    with (output / "metrics.jsonl").open("x") as metrics:
        for step in range(plan["steps"]):
            ...
            offset = 1 if mode == 1 else 206
            counts = torch.from_numpy(features[start:end, offset:offset + 205].astype(np.float32)).unsqueeze(0)
            logits = child.compute_logits(inputs, prior, bounds, counts)
            data_bits = torch.nn.functional.cross_entropy(
                logits[0, warmup:], targets[warmup:]) / math.log(2)
            rate = quantized_weight_rate_bits(model) + (192 * 4 + 4) * 8
            ...
            objective.backward()
            gradient_norm = torch.nn.utils.clip_grad_norm_(child.parameters(), 1., error_if_nonfinite=True)
            optimizer.step()
            if any(not torch.isfinite(p).all() for p in child.parameters()):
            ...
            history.append(row)
    changed = [n for n, p in child.named_parameters() if not torch.equal(p, initial[n])]
    if not any(n.startswith("base.blocks.") and n.endswith(".weight") for n in changed):
        raise ValueError("training did not change the hidden representation")
    if "gain" not in changed:
        raise ValueError("metadata gain did not train")
    torch.save({"base": model.state_dict(), "metadata_gain": child.gain.detach(), "metadata_mode": mode}, output / "checkpoint.tch")
    ...
    entries += metadata_entries(child.gain.detach().numpy(), mode)
    ...
    receipt = {"schema": "gamma.enwiki9.fx2-title-training.v1", "plan": plan,
               "runtime": {"python": __import__("sys").version, "torch": torch.__version__,
                           "device": "cpu", "profile": adaptation["profile"]},
               "native_capture": rows, "metadata_mode": mode, "active_unique_loss_rows": active_loss_rows,
               "gain_nonzero": int(torch.count_nonzero(child.gain)), "changed_parameters": changed,
