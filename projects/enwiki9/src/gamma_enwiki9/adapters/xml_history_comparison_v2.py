"""Maintained comparison; v1 remains frozen after its built-in-zlib reporting failure."""
import argparse
import json
import os
...
def native_module_identity(module):
    """Built-in modules are identified by their interpreter, not an invented file."""
    path = getattr(module, '__file__', None)
    if path is not None:
        return {'kind': 'extension', 'artifact': fingerprint(Path(path).resolve(), Path('/'))}
    if module.__spec__.origin != 'built-in':
        raise ValueError('native module has neither file nor built-in origin')
    return {'kind': 'built-in', 'module': module.__name__,
            'interpreter': fingerprint(Path(sys.executable).resolve(), Path('/'))}
...
def run(root, output, experiment_path, closure_path, cpu):
    ...
    entry = snapshot / 'tools/xml_history_gate_v2.py'
    commands, rows = [], {}
    ...
    result = {'schema': 'gamma.enwiki9.xml-history-diagnostic.v1',
        ...
        'runtime': {'python': sys.version, 'zlib': zlib.ZLIB_RUNTIME_VERSION,
                    ...
                    'zlib_extension': native_module_identity(zlib)},
