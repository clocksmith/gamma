...
class _OwnedLock:
    ...
    def acquire(cls, lease_path: Path) -> "_OwnedLock":
        ...
        try:
            # The directory inode is stable across lock-file replacement. Hold
            # admission through publication and cleanup; a contender never waits
            # on a full experiment and never unlinks a namespace it did not own.
            fcntl.flock(directory_descriptor, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BaseException:
            os.close(directory_descriptor)
            raise LeaseError("exclusive lease namespace is owned")
        ...
        payload = canonical_json(
            {
                "pid": os.getpid(),
                "proc_start_ticks": proc_identity(os.getpid())[1],
    ...
    def publish_lease(self, raw: bytes) -> None:
        ...
        self.lease_identity = _write_new_at(
            self.directory_descriptor,
...
class ManagedExclusiveLease:
    ...
    def acquire(
        ...
        *,
        ...
        try:
            ...
            # A failed write can leave a partial lease even before it returns.
            # Preserve both names for explicit recovery whenever publication began.
            if published or _present_at(owned.directory_descriptor, owned.lease_path.name):
    ...
    def _require_owner(self) -> None:
        ...
        observed = self.lease_path.lstat()
        if (observed.st_dev, observed.st_ino) != (self.owned_lock.lease_identity.st_dev, self.owned_lock.lease_identity.st_ino):
            raise LeaseError("managed lease file identity changed")
    ...
    def _commit(self, event: str | None = None) -> None:
        ...
        self.owned_lock.lease_identity = self.lease_path.lstat()
        self.owned_lock.assert_owned()
...
def file_sha256(path: Path) -> str:
    ...
    absolute = _absolute(path)
    return fingerprint(absolute, absolute.parent)["sha256"]


def recover_stale(lease_path: Path, *, expected_lock: bytes,
                  expected_lease: bytes | None, evidence_path: Path) -> None:
    """Explicit recovery, under the same directory lock as live ownership.

    Exact observed bytes are required. Malformed owners fail closed. A partial
    lease is recoverable only when its lock has a proven dead owner. No PID is
    signaled here, and no partial evidence is promoted to a terminal result.
    """
    from gamma_enwiki9.evidence.artifacts import publish_immutable_artifact

    lease, fd, _ = _open_parent(lease_path)
    lock_name = lease.name + ".lock"
    try:
        try:
            fcntl.flock(fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise LeaseError("cannot recover an owned lease namespace") from exc
        lock_identity = os.stat(lock_name, dir_fd=fd, follow_symlinks=False)
        if _read_named_owned(fd, lock_name, lock_identity) != expected_lock:
            raise LeaseError("stale lock changed since inspection")
        owner = json.loads(expected_lock)
        if owner.get("schema") != LOCK_SCHEMA or not all(
            type(owner.get(k)) is int and owner[k] > 0 for k in ("pid", "proc_start_ticks")
        ) or not isinstance(owner.get("boot_id"), str):
            raise LeaseError("stale lock owner is unverifiable")
        same_boot = owner["boot_id"] == Path("/proc/sys/kernel/random/boot_id").read_text().strip()
        if same_boot and identity_is_live(owner["pid"], owner["proc_start_ticks"]):
            raise LeaseError("cannot recover while runner identity is live")
        if expected_lease is None:
            if _present_at(fd, lease.name):
                raise LeaseError("lease appeared since inspection")
        else:
            identity = os.stat(lease.name, dir_fd=fd, follow_symlinks=False)
            if _read_named_owned(fd, lease.name, identity) != expected_lease:
                raise LeaseError("stale lease changed since inspection")
            try:
                record = json.loads(expected_lease)
            except (ValueError, UnicodeError):
                # No live codec can have been authorized before initial publication.
                # A damaged existing lease cannot establish that, so refuse recovery.
                raise LeaseError("partial lease needs independent process-closure evidence")
            pid, ticks = record.get("codec_pid"), record.get("codec_proc_start_ticks")
            if pid is not None or ticks is not None:
                if type(pid) is not int or type(ticks) is not int:
                    raise LeaseError("codec ownership is unverifiable")
                if same_boot and identity_is_live(pid, ticks):
                    raise LeaseError("cannot recover while codec identity is live")
        publish_immutable_artifact(evidence_path, canonical_json({
            "schema": "gamma.enwiki9.lease-recovery.v1", "lock_hex": expected_lock.hex(),
            "lease_hex": expected_lease.hex() if expected_lease is not None else None,
            "recovered_at": utc_now(), "scientific_authority": False,
        }))
        if expected_lease is not None:
            _unlink_exact_named(fd, lease.name, expected_lease, "stale lease")
        _unlink_exact_named(fd, lock_name, expected_lock, "stale lock")
    finally:
        os.close(fd)
