...
class IndependentLiteralFirstTests(unittest.TestCase):
    ...
    def test_complete_cost_selects_best_reversible_proposal(self):
        ...
        with patch.object(codec, "discover", side_effect=lambda _: (proposals, dict(spans=16, proposals=2))):
    ...
    def test_raw_D_repeats_discovery_but_decoder_does_not(self):
        raw = b"".join(b"<p><title>" + str(i).encode() + b"</title><text>" + str(i).encode() +
                       b" exact\xff</text></p>\n" for i in range(24))
        frame_size = 257
        frames = (len(raw) + frame_size - 1) // frame_size
        with patch.object(codec, "discover", wraps=codec.discover) as discovery:
            archive, report = codec.encode(raw, "D", frame_size)
            repeated, repeat_report = codec.encode(raw, "D", frame_size)
            self.assertEqual(discovery.call_count, 2 * frames)
            self.assertEqual((archive, report), (repeated, repeat_report))
            inverse, _ = codec.decode(archive)
            self.assertEqual(discovery.call_count, 2 * frames)
        self.assertEqual(inverse, raw)
        self.assertLessEqual(len(archive), len(published_parent.encode(raw, mode="plain", frame_size=frame_size)[0]))
