"""Predetermined synthetic gate success and falsification checks; no corpus."""
import contextlib
import copy
import hashlib
import io
...
from tools import causal_bucket_gate_v1 as gate
from tools import causal_bucket_v1 as codec
...
class GateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp = tempfile.TemporaryDirectory(prefix='causal_bucket_gate_')
        ...
        for _ in range(8192):
            ...
            walk.append(value)
        cls.raws = dict(favorable=bytes(walk), fallback=random.Random(491).randbytes(2048))
        cls.plans, cls.golds = {}, {}
        ...
        for name, raw in cls.raws.items():
            ...
            plan = dict(schema=gate.SCHEMA, candidate_id=name, stage='development', arms=gate.ARMS,
                ...
                transform_spec=codec.TRANSFORM_SPEC, kernel_basis='Predetermined synthetic fixtures only',
        ...
        write(cls.root/'recipe.json', dict(synthetic_only=True, corpus_bytes_opened=0,
            favorable='Random871;8192 steps; initial0 then add choice(-1,0,1) modulo256',
            fallback='Random491.randbytes(2048)',
            fixtures={n: dict(bytes=len(r), sha256=hashlib.sha256(r).hexdigest()) for n, r in cls.raws.items()},
            source_files=[cls.ref(cls.root/p) for p in sorted({gate.SELF, gate.SHARED_DRIVER, gate.TESTS, *gate.PACKAGE})]))

    @classmethod
    def tearDownClass(cls):
        target = os.environ.get('CAUSAL_GATE_EVIDENCE_ZIP')
        if target:
            with zipfile.ZipFile(target, 'x', compression=zipfile.ZIP_DEFLATED) as bundle:
                for p in sorted(cls.root.rglob('*')):
                    if p.is_file() and (p == cls.root/'recipe.json' or p.parts[-2] in ('gold_favorable', 'gold_fallback')
    ...
    def setUp(self):
        ...
        self.output.mkdir()
        self.plan, self.gold = self.plans['favorable'], self.golds['favorable']
    ...
    def run_mocked(self, mutation=None, final_error=None):
        ...
        return code, json.loads((self.output/'stage-decision.json').read_text())
    ...
    def test_eighteen_actual_phases_raw_repeats_and_accounting(self):
        for name, gold in self.golds.items():
            ...
            self.assertEqual(stage['costs']['strict_d_improvement'], name == 'favorable')
            ...
            self.assertEqual(indexed, {str(p.relative_to(self.root)) for p in gold.iterdir()
                                      if p.name not in ('artifacts.json', 'stage-decision.json')})

    def test_exact_fallback_identity(self):
        gold = self.golds['fallback']
        self.assertEqual((gold/'P.d2g').read_bytes(), (gold/'D.d2g').read_bytes())
        self.assertFalse(json.loads((gold/'costs-table.json').read_text())['confirmation_eligible'])

    def test_p_k_mismatch(self):
        def change(directory, phase, argv, record):
            if phase in ('K-encode', 'K-repeat'):
                with Path(argv[4]).open('ab') as stream:
                    stream.write(b'x')
        self.failed(change, 'P/K archive identity')

    def test_inverse_corruption(self):
        def change(directory, phase, argv, record):
            if phase == 'K-decode':
                Path(argv[4]).write_bytes(b'wrong')
        self.failed(change, 'independent inverse')
    ...
    def test_repeat_archive_corruption(self):
        def change(directory, phase, argv, record):
            if phase == 'D-repeat':
                with Path(argv[4]).open('ab') as stream:
                    stream.write(b'x')
        self.failed(change, 'raw encoder repeat')

    def test_repeat_transform_drift(self):
        self.failed(self.change_reports('D', lambda r: r['frames'][0]['comparison'].update(counts_sha256='0'*64), ('repeat',)),
                    'raw encoder repeat report')

    def test_decoder_payload_drift(self):
        self.failed(self.change_reports('D', lambda r: r['frames'][0].update(transmitted_payload_sha256='0'*64), ('decode',)),
                    'independent decoder projection')

    def test_joint_accounting_corruption(self):
        self.failed(self.change_reports('D', lambda r: r['costs'].update(framing=r['costs']['framing']+1)), 'complete archive accounting')

    def test_frame_accounting_corruption(self):
        self.failed(self.change_reports('D', lambda r: r['frames'][0]['costs'].update(framing=0)), 'frame identity or cost')

    def test_k_d_transform_disagreement(self):
        self.failed(self.change_reports('D', lambda r: r['frames'][0]['comparison'].update(counts_sha256='0'*64), ('encode', 'repeat')),
                    'K/D transformed candidate')

    def test_non_strict_selection(self):
        self.failed(self.change_reports('D', lambda r: r['frames'][0]['comparison'].update(delta=0), ('encode', 'repeat')),
                    'whole-frame comparison')

    def test_complete_package_overclaim(self):
        self.failed(self.change_reports('D', lambda r: r.update(complete_package_bytes=0)), 'identity or scope')

    def test_final_auth_failure_does_not_publish_costs(self):
    ...
    def test_projection_excludes_only_mode_and_comparison(self):
        ...
        enc['omitted_dependency'] = True
        self.assertNotEqual(gate.common_report(enc), dec)

    def test_plan_stage_population_source_and_bounds(self):
        plan = copy.deepcopy(self.plan)
        plan['candidate_id'] = 'shape'
        plan['population'].update(bytes=250000, sha256=gate.OPENING_SHA)
        plan['plain_archive'].update(bytes=89041, sha256=gate.PLAIN_SHA)
        gate.validate_plan(plan, 'shape')
        for mutate in (lambda p: p['transform_spec'].update(initial_predecessor=1),
                       lambda p: p['population'].update(sha256='1'*64), lambda p: p.update(phase_cpu_seconds=True),
                       lambda p: p['resources'].update(wall_seconds=True), lambda p: p['population'].update(path='../raw'),
                       lambda p: p.update(stage='confirmation')):
        ...
        del fresh['plain_archive']
        gate.validate_plan(fresh, 'shape')

    def test_auth_missing_source_and_retained_baseline(self):
        paths = {gate.SELF, gate.SHARED_DRIVER, gate.TESTS, *gate.PACKAGE, self.plan['plain_archive']['path']}
        inputs = [dict(path=p, sha256='sha256:'+self.ref(self.root/p)['sha256']) for p in paths]
        ...
        with patch.object(gate, 'ROOT', self.root), patch.object(gate.driver, 'read_json', return_value=contract), \
            ...
            auth.reset_mock()
            contract['inputs'] = [r for r in inputs if r['path'] != gate.PLAIN]
