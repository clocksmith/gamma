...
def binary(tmp_path_factory):
    ...
    subprocess.run(['g++','-std=c++17','-O1','-Wall','-Wextra','-Werror','-fsanitize=undefined','-fno-sanitize-recover=all','-I',str(ROOT/'lib'),str(ROOT/'tests/causal_relational_fixture_v2.cpp'),'-o',str(dest)],check=True,timeout=60)
    return dest
