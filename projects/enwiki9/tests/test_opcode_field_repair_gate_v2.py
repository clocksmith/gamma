...
from tools import opcode_field_repair_gate_v2 as gate
...
class GateTests(unittest.TestCase):
    ...
    def test_frozen_validation_plan_and_population_controls(self):
        cid='opcode_field_validation250k_q0_v1'
        plan=json.loads((PROJECT/'programs'/cid/'gate-plan.json').read_text())
        gate.validate_plan(plan,cid)
        for mutate in (lambda p:p['population'].update(sha256='0'*64),
                       lambda p:p['package_files'][0].update(sha256='0'*64),
                       lambda p:p['resources'].update(memory_bytes=2**40),
                       lambda p:p.update(stage='development')):
            bad=copy.deepcopy(plan);mutate(bad)
            with self.assertRaises(ValueError):gate.validate_plan(bad,cid)

    def test_positive_bound_selection_and_rejected_controls(self):
        cid='opcode_field_validation250k_q0_v1'
        plan=json.loads((PROJECT/'programs'/cid/'gate-plan.json').read_text())
        contract=json.loads((PROJECT/'operations/adaptive/experiments'/(cid+'.json')).read_text())
        gate.validate_selection(plan,contract)
        selected=json.loads((PROJECT/plan['selection_receipt']['path']).read_text())
        for mutate in (lambda r:r['measurements'].update(netBytesSaved=0),
                       lambda r:r['attribution'].update(controlsEquivalent=False),
                       lambda r:r['decision'].update(nextGateBytes=1000000),
                       lambda r:r.update(candidateId='unrelated')):
            bad=copy.deepcopy(selected);mutate(bad)
            with patch.object(gate.driver,'read_json',return_value=bad),self.assertRaises(ValueError):
                gate.validate_selection(plan,contract)
