...
#ifndef FX3_LSTM_LR_FINAL
#define FX3_LSTM_LR_FINAL 0.0f
#endif
#ifndef FX3_LSTM_LR_DECAY_START_BYTES
#define FX3_LSTM_LR_DECAY_START_BYTES 0
#endif
#ifndef FX3_LSTM_LR_DECAY_END_BYTES
#define FX3_LSTM_LR_DECAY_END_BYTES 0
#endif
#ifndef FX3_LSTM_SKIP_OUTPUT_COPY_ON_NO_TRAIN
#define FX3_LSTM_SKIP_OUTPUT_COPY_ON_NO_TRAIN 0
#endif
static_assert(FX3_LSTM_LR_FINAL >= 0.0f,
              "FX3_LSTM_LR_FINAL must be >= 0");
static_assert(FX3_LSTM_LR_DECAY_START_BYTES >= 0,
              "FX3_LSTM_LR_DECAY_START_BYTES must be >= 0");
static_assert(FX3_LSTM_LR_DECAY_END_BYTES >= 0,
              "FX3_LSTM_LR_DECAY_END_BYTES must be >= 0");
#if FX3_LSTM_SKIP_OUTPUT_COPY_ON_NO_TRAIN != 0 && \
    FX3_LSTM_SKIP_OUTPUT_COPY_ON_NO_TRAIN != 1
#error "FX3_LSTM_SKIP_OUTPUT_COPY_ON_NO_TRAIN must be 0 or 1"
#endif
...
inline void Lstm::SetInput(const std::valarray<float>& input) {
  FX3_PROFILE_SCOPE("lstm.set_input");
...
inline float Lstm::EffectiveLearningRate() const {
  const float final_lr = FX3_LSTM_LR_FINAL > 0.0f
      ? static_cast<float>(FX3_LSTM_LR_FINAL)
      : learning_rate_;
  if (FX3_LSTM_LR_DECAY_END_BYTES <= FX3_LSTM_LR_DECAY_START_BYTES) {
    return learning_rate_;
  }
  if (perceive_steps_ <= FX3_LSTM_LR_DECAY_START_BYTES) {
    return learning_rate_;
  }
  if (perceive_steps_ >= FX3_LSTM_LR_DECAY_END_BYTES) {
    return final_lr;
  }
  const float span = static_cast<float>(
      FX3_LSTM_LR_DECAY_END_BYTES - FX3_LSTM_LR_DECAY_START_BYTES);
  const float pos = static_cast<float>(
      perceive_steps_ - FX3_LSTM_LR_DECAY_START_BYTES);
  const float t = pos / span;
  return learning_rate_ + (final_lr - learning_rate_) * t;
}

inline std::valarray<float>& Lstm::Perceive(unsigned int input) {
  return Perceive(input, true);
}

inline std::valarray<float>& Lstm::Perceive(unsigned int input, bool train) {
  FX3_PROFILE_SCOPE("lstm.perceive.total");
  const float effective_learning_rate = EffectiveLearningRate();
  if (train) {
    for (auto& layer : layers_) {
      layer.SetLearningRate(effective_learning_rate);
    }
  }
  int last_epoch = epoch_ - 1;
  if (last_epoch == -1) last_epoch = horizon_ - 1;
...
//          else error = output_[epoch][i];
  ...
  if (train || !FX3_LSTM_SKIP_OUTPUT_COPY_ON_NO_TRAIN) {
    for (unsigned int i = 0; i < output_size_; ++i) {
//    float error = 0;
...
//    else error = output_[last_epoch][i];
      ...
      if (train) {
        ...
        output_layer_[epoch_][i] -= effective_learning_rate * error * hidden_;
      }
    }
  }
  if (train) {
...
inline std::valarray<float>& Lstm::Predict(unsigned int input) {
  FX3_PROFILE_SCOPE("lstm.predict.total");
  for (unsigned int i = 0; i < layers_.size(); ++i) {
