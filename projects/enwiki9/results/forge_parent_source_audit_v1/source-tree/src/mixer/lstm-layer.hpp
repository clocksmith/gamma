...
#ifndef FX3_LSTM_ADAM_CACHE_LIMIT_CONSTANTS
#define FX3_LSTM_ADAM_CACHE_LIMIT_CONSTANTS 0
#endif
#if FX3_LSTM_ADAM_CACHE_LIMIT_CONSTANTS != 0 && \
    FX3_LSTM_ADAM_CACHE_LIMIT_CONSTANTS != 1
#error "FX3_LSTM_ADAM_CACHE_LIMIT_CONSTANTS must be 0 or 1"
#endif
...
#if FX3_LSTM_ADAM_CACHE_LIMIT_CONSTANTS
    static const double limit_alpha_denominator =
        sqrt(5e-5f * UPDATE_LIMIT + 1.0f);
    alpha = learning_rate * 0.1f / limit_alpha_denominator;
#else
...
#if FX3_LSTM_ADAM_CACHE_LIMIT_CONSTANTS
    static const float limit_beta1_correction =
        static_cast<float>(1.0f - pow(beta1, UPDATE_LIMIT));
    static const float limit_beta2_correction =
        static_cast<float>(1.0f - pow(beta2, UPDATE_LIMIT));
    (*w) -= alpha * (((*m) / limit_beta1_correction) /
        (sqrt((*v) / limit_beta2_correction + eps)));
#else
...
inline void LstmLayer::ForwardPass(const std::valarray<float>& input, int input_symbol,
  ...
  FX3_PROFILE_SCOPE("lstm_layer.forward.total");
...
inline void LstmLayer::SetLearningRate(float learning_rate) {
...
inline void LstmLayer::BackwardPass(const std::valarray<float>&input, int epoch,
  ...
  FX3_PROFILE_SCOPE("lstm_layer.backward.total");
