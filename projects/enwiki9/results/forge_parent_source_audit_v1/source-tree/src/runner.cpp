...
#ifndef FX3_ENABLE_PAYLOAD_LEX
#define FX3_ENABLE_PAYLOAD_LEX 0
#endif
#if FX3_ENABLE_PAYLOAD_LEX != 0 && FX3_ENABLE_PAYLOAD_LEX != 1
#error "FX3_ENABLE_PAYLOAD_LEX must be 0 or 1"
#endif
#if FX3_ENABLE_PAYLOAD_LEX
...
int main(int argc, char** argv) {
  ...
  if (argc == 1) {
    ...
    selfextract_decomp();
#ifdef ARCHIVE9_DYNAMIC_SPLIT_HEADER
    HeaderInfo archive_header = {};
    read("test.dat", archive_header);
#endif
    ...
    const char* post_wrt_side_path = nullptr;
#if FX3_ENABLE_PAYLOAD_LEX
    post_wrt_side_path = ".r1_payload_lex_side_decomp";
#endif
    if (!RunDecompression(input_path, temp_path, output_path, dictionary,
        &input_bytes, &output_bytes, post_wrt_side_path)) {
      return Help();
    }
    std::cout << "Cmix decompression finished" << std::endl;

#ifdef ARCHIVE9_DYNAMIC_SPLIT_HEADER
    if (archive_header.main_prepr_size > 0 || archive_header.intro_size > 0 ||
        archive_header.coda_size > 0) {
      split4DecompDynamic(archive_header.main_prepr_size,
          archive_header.intro_size, archive_header.coda_size);
    } else {
      split4Decomp();
    }
#else
  ...
  if (argv[1][1] == 's') {
...
#ifdef ARCHIVE9_DYNAMIC_SPLIT_HEADER
    size_t main_prepr_size = getFileSize(".main_phda9prepr");
    size_t intro_size = getFileSize(".intro");
    size_t coda_size = getFileSize(".coda");
#endif
    ...
    const char* post_wrt_side_path = nullptr;
#if FX3_ENABLE_PAYLOAD_LEX
    post_wrt_side_path = ".r1_payload_lex_side";
#endif
    if (!RunCompression(enable_preprocess, input_path, temp_path, output_path,
        dictionary, &input_bytes, &output_bytes, post_wrt_side_path)) {
      return Help();
    }
...
#ifdef ARCHIVE9_DYNAMIC_SPLIT_HEADER
    header.main_prepr_size = static_cast<int>(main_prepr_size);
    header.intro_size = static_cast<int>(intro_size);
    header.coda_size = static_cast<int>(coda_size);
#endif
