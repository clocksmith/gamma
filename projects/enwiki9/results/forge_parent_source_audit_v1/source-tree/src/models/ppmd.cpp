...
//
// The RSS-only cmix-lex backport keeps the disk-backed PPM mapping stable and
// drops resident pages with madvise(MADV_DONTNEED). The separate default-off
// FX3_ENABLE_CMIX_LEX_PPMD_PREDICT path gates cmix-lex's byte-relevant binary
// tree prediction logic for isolated archive9 tests.
...
// Disk-backed PPM keeps the 14GB logical heap outside anonymous RAM, but
// resident file-backed pages still count in RSS. Drop residency periodically
// without tearing down the mapping; PPMD stores raw pointers into HeapStart.
...
void PPMD::ByteUpdate() {
  FX3_PROFILE_SCOPE("ppmd.byte_update");
  ++counter_;
