...
void reorder() {
    ...
    loadFile(".main");
    const int article_count = static_cast<int>(vec.size());
    std::vector<int> used(article_count, 0);
    ...
    while (getline(infile2, line)) {
      ...
      int res = it->second;  // Article numbers get remapped.
      if (res < 0 || res >= article_count) {
        continue;
      }
    ...
    if (positions.size() < static_cast<size_t>(article_count)) {
