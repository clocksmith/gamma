...
int main(int argc,char**){
 ...
 for(unsigned role=0;role<2;++role)for(const auto& rec:a.records(role)){
   std::string word(rec.raw.begin(),rec.raw.end());assert(word!="title"&&word!="text"&&word!="xml"&&word!="space"&&word!="preserve");
 }
