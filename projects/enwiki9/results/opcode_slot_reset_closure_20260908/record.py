"""Validate and normalize the closed slot-reset job before its reflection."""
import hashlib
import json
...
CID='opcode_slot_reset_v1'
JID='20260908T165054Z_e47978f5d4'
JOB='operations/adaptive/completed/909_'+JID+'.json'
OUT='operations/provenance/opcode_slot_reset_terminal_20260908'
...
def main():
    ...
    for arm in 'PKD':
        ...
        result=dict(schema='gamma.enwiki9.driver-result.v2',program_id=CID,program_name='Decoded slot-reset comparison',
            ...
            run_purpose='diagnostic',execution_mode='discovery',timing_authority='diagnostic',run_scope_label='opening250KB-slot-reset-'+arm,
            run_tags=['slot-reset','standalone','diagnostic',arm],
            run_context='One fixed development comparison resetting only the slot at a completed text-close opcode. P/K identity, independent inverses, raw repeats and complete state witnesses pass. Package economics are separate.',
    ...
    gain=stage['archive_saving_bytes'];cost_path='results/opcode_slot_reset_source_zip_20260908/attempt01/cost.json';cost=read(cost_path)
    terminal=dict(schema='gamma.enwiki9.opcode-slot-reset-terminal.v1',candidate_id=CID,job=ref(JOB),guard=ref(guard_path),
        ...
        post_update_slot_occupancy={a:{k:stage['arms'][a]['audit'][k] for k in ['slot_byte_counts','changed_slot_bytes','mode_slot_byte_counts','mode_changed_slot_bytes','complete_text_close_events','nonzero_slot_before_close_events']} for a in 'PKD'},
