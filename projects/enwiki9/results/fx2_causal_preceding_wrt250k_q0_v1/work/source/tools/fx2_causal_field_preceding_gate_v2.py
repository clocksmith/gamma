...
SELF = "tools/fx2_causal_field_preceding_gate_v2.py"
...
def read_metadata_ref(root, ref):
    require(ref["path"] not in {row[0] for row in POPULATION.values()},
            "population payload requested before admission")
    return read_ref(root, ref)


def validate_guard_command(guard):
...
def authenticate(root=ROOT, *, execution=False):
    ...
    plan = json.loads(read_metadata_ref(root, plans[0]))
    ...
    buffers = {name: read_metadata_ref(root, inputs[name]) for name in
    ...
    for path in sources:
        ...
        require(name in inputs, "missing frozen source: " + name)
        buffers[name] = read_metadata_ref(root, inputs[name])
    for ref in plan["local_package_files"]:
        require_ref(inputs, ref)
        read_metadata_ref(root, ref)
    ...
    adapter_tree = ast.parse(read_metadata_ref(root, inputs[adapter_name]), filename=adapter_name)
    ...
    for name, value in FIXED.items():
        ...
        require_ref(inputs, ref)
        buffers[name] = read_metadata_ref(root, ref)
    ...
    for ref in [audit["job"], audit["guard"], audit["decision"], *plan["antecedents"]]:
        require_ref(inputs, ref)
        buffers[ref["path"]] = read_metadata_ref(root, ref)
