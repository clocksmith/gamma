// Gamma exact sign/magnitude loader successor; retain upstream GPLv3 LICENSE.
// Gamma adaptive marginal successor. Preserve upstream GPLv3 LICENSE.
...
WeightsFile load_v2(Reader& r, const char* path, bool marginal = false, bool adaptive_counts = false, bool sign_magnitude = false) {
  ...
  const char selected = sign_magnitude ? 'S' : adaptive_counts ? 'A' : marginal ? (gamma.mode == 1 ? 'D' : 'G') : bookkeeping ? 'K' : 'P';
  if (std::fprintf(stderr, "\nGamma weight loader selected=%c tensors=%u histogram_tensors=%zu histogram_symbols=%llu side_information_bytes=%zu canonical=%d\n",
      ...
      marginal ? size_t(65) + gamma.records.size() * 64 : size_t(0), (marginal || adaptive_counts || sign_magnitude) ? 1 : 0) < 0 || std::fflush(stderr) != 0)
...
WeightsFile WeightsFile::load_compressed(const char* path) {
  ...
  if (static_cast<unsigned long>(size) > kGammaFileLimit) {
    ...
    if (std::memcmp(prefix, "GFX2MAR1", 8) == 0 || std::memcmp(prefix, "GFX2ADM1", 8) == 0 || std::memcmp(prefix, "GFX2SMG1", 8) == 0) die("%s: Gamma model file exceeds bound", path);
  }
