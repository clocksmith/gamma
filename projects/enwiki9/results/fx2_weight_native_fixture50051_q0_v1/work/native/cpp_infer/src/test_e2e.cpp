// test_e2e: stream all 1024 test articles, compute the loss two ways, compare
// with ref_loss.txt and with the reference probability file (SPEC section 6)
#include <cmath>
#include <cstdio>
#include <cstring>
#include <ctime>
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    else if (std::strcmp(argv[i], "--lowprob-skip") == 0) {
      lowprob_skip = true;
      if (i + 1 < argc && argv[i + 1][0] != '-')
        lowprob_threshold = static_cast<float>(std::atof(argv[++i]));
    } else {
      std::fprintf(
          stderr,
          "usage: %s [--data DIR] [--articles N] [--lowprob-skip [THR]]\n",
          argv[0]);
      return 2;
    }
  }
  ...
  model.set_attention_lowprob_skip(lowprob_skip, lowprob_threshold);
  if (lowprob_skip)
    std::printf(
        "NOTE: attention low-prob skip ENABLED, threshold %.3g "
        "(approximate mode)\n",
        lowprob_threshold);
  ...
  // probability comparison vs test_ref_probs.f16 (excluding article-final rows)
  ...
  for (int a = 0; a < n_articles; a++) {
    ...
    if (len < 2) continue;  // length-1 articles contribute nothing
    ...
    for (long t = 0; t < len - 1; t++) {
      ...
      // (a) from logits via log-sum-exp in double
      ...
      // (b) from the emitted fp32 probabilities
      loss_probs += -std::log(double(probs[target]));
      count++;

      // compare with the reference probability row
  ...
  if (full_run) {
    ...
    std::printf("C++ mean loss (from logits):     %.10f\n", mean_logits);
    ...
    std::printf("\n(subset run: %d articles -- no verdict; reference mean over "
                "ALL articles is %.10f)\n",
                n_articles, ref_mean);
    std::printf("probs vs test_ref_probs.f16: rows %ld, mean L1 %.3e, max L1 "
                "%.3e, max prob diff %.3e, rows L1>1e-2: %ld\n",
                rows_cmp, l1_sum / double(rows_cmp), l1_max, prob_abs_max,
                rows_l1_gt);
    return 0;
  }
}
