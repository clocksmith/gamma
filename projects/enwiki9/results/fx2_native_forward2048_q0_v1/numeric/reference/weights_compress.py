...
import numpy as np
from .export_weights import DTYPE_BF16_BITS, DTYPE_FLOAT32, DTYPE_INT8, DTYPE_INT32, _DTYPE_TO_NUMPY, read_tensor_file
MAGIC_V1 = b'FX2TFWC1'
_TOP = 1 << 24
_MASK32 = 4294967295
...
class RangeEncoder:
    ...
    def _shift_low(self) -> None:
        if self.low < 4278190080 or self.low > _MASK32:
            carry = self.low >> 32
            self.out.append(self.cache + carry & 255)
            for _ in range(self.cache_size - 1):
                self.out.append(255 + carry & 255)
            self.cache = self.low >> 24 & 255
            self.cache_size = 0
        self.cache_size += 1
...
def write_tensor_file_v1(filename: str, entries: list[tuple[str, int, np.ndarray]]) -> None:
...
ENC_ROPE_SIN = 4
ENC_ROPE_COS = 5
_F32_2_OVER_PI = np.uint32(1059256707).view(np.float32)
_F32_RED = [np.uint32(u).view(np.float32) for u in (3217625050, 3013747048, 2814522565)]
_F32_KCOS = [np.uint32(u).view(np.float32) for u in (936094720, 3132491757, 1026206395, 3204448255)]
_F32_KSIN = [np.uint32(u).view(np.float32) for u in (3108847955, 1007191524, 3190467240)]
_PH_SCALE = np.uint64(4321522680772963609).view(np.float64)
_I2OPI = (1011060801, 3680671129, 4113882560, 4230436817, 1313084713, 2734261102)
...
def _fmaf_vec(x: np.ndarray, y, z) -> np.ndarray:
    ...
    hazard = (np.abs(d - (f64 + below) * 0.5) <= ulp) | (np.abs(d - (f64 + above) * 0.5) <= ulp)
...
def _trig_slowpath(a: np.float32) -> tuple[np.float32, int]:
    ...
    sign = ia & 2147483648
    e = (ia >> 23 & 255) - 128
    ia = (ia << 8 | 2147483648) & 4294967295
    result = []
    hi = 0
    for k in range(6):
        p = _I2OPI[k] * ia + hi
        result.append(p & 4294967295)
        hi = p >> 32
    result.append(hi)
    idx = 4 - (e >> 5)
    sh = e & 31
    ...
    if sh:
        rhi = (result[idx + 2] << sh | result[idx + 1] >> 32 - sh) & 4294967295
        rlo = (result[idx + 1] << sh | result[idx] >> 32 - sh) & 4294967295
    q = rhi >> 30
    nhi = (rhi << 2 | rlo >> 30) & 4294967295
    nlo = rlo << 2 & 4294967295
    top = nhi >> 31
    q += top
    if sign:
        q = -q
...
def _pick_encoding(name: str, dtype_code: int, array: np.ndarray, seen: dict[str, np.ndarray]) -> int:
    if dtype_code == DTYPE_INT8:
    ...
    if dtype_code == DTYPE_FLOAT32:
        if name in ('rope.sin', 'rope.cos') and array.ndim == 2 and ('rope.inv_freq' in seen) and (seen['rope.inv_freq'].shape == (array.shape[1],)):
            table = cuda_rope_table(seen['rope.inv_freq'], array.shape[0], cos=name == 'rope.cos')
            if np.array_equal(table.view(np.uint32), np.ascontiguousarray(array).view(np.uint32)):
...
def write_tensor_file_v2(filename: str, entries: list[tuple[str, int, np.ndarray]]) -> None:
    ...
    for name, dtype_code, array in entries:
        ...
        if encoding == ENC_INT4:
            ...
            for v in data.reshape(-1).tolist():
                ...
                enc.encode_tree(models.bf16_lo_tree(hi), 8, v & 255)
...
def read_tensor_file_v2(filename: str) -> dict[str, tuple[int, np.ndarray]]:
    ...
    for _ in range(n_tensors):
        ...
        if encoding == ENC_INT4:
            ...
            probs = models.int4
            array = np.array([dec.decode_tree(probs, 4) - 7 for _ in range(count)], dtype=np.int8).reshape(shape)
        elif encoding == ENC_BF16:
            ...
            planes = models.plane
            raw = bytes((dec.decode_tree(planes[i & 3], 8) for i in range(count * 4)))
            ...
            array = cuda_rope_table(inv, shape[0], cos=encoding == ENC_ROPE_COS)
        else:
            assert encoding == ENC_RAW
            probs = models.raw
            raw = bytes((dec.decode_tree(probs, 8) for i in range(count * dtype.itemsize)))
            array = np.frombuffer(raw, dtype=dtype).reshape(shape)
        assert name not in tensors
        tensors[name] = (dtype_code, array)
    return tensors
...
def _entries_from_reference(filename: str) -> list[tuple[str, int, np.ndarray]]:
    ...
    return [(name, dtype_code, array) for name, (dtype_code, array) in read_tensor_file(filename).items()]

def _verify_equal(reference: dict[str, tuple[int, np.ndarray]], decoded: dict[str, tuple[int, np.ndarray]]) -> None:
...
def main() -> None:
    import os
    usage = 'usage: python -m pysrc.weights_compress {compress|compress1|decompress|verify} <input> <output|compressed>'
    if len(sys.argv) != 4:
        sys.exit(usage)
    ...
    for path in inputs:
        if not os.path.isfile(path):
            sys.exit(f'error: input file {path!r} does not exist' if not os.path.exists(path) else f'error: input file {path!r} is not a regular file')
    if command in ('compress', 'compress1'):
        ...
        print(f'{a} ({os.path.getsize(a)} bytes) -> {b} ({os.path.getsize(b)} bytes), round-trip verified bit-exact')
    elif command == 'decompress':
        from .export_weights import write_tensor_file
        tensors = _read_compressed(a)
        write_tensor_file(b, [(name, code, arr) for name, (code, arr) in tensors.items()])
