...
//  std::cout << "Decompressing the file with the new article order..." << std::endl;
  RunSubprocess("./cmix -d .new_article_order.comp .new_article_order --ppmd-only");

//  std::cout << "Decompressing dictionary..." << std::endl;
  RunSubprocess("./cmix -d .dict.comp .dict --ppmd-only");
...
int selfextract_decomp() {
  ...
  RunSubprocess("./archive9 -d .dict.comp_decomp .dict --ppmd-only");//_decomp
  VerifyDictionary(".dict");
