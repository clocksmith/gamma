#!/usr/bin/env python3
"""Bounded preceding-field replay using explicitly supplied parent Q16 values.
...
from causal_field_preceding_adapter250k_v1 import make_adapter, ORIGINAL_POLICY
...
def replay(operation, body, q16, prefix, words, arm, raw_bytes, raw_sha256, *, adapter_factory=make_adapter):
    ...
    adapter = adapter_factory(words, arm=arm, raw_limit=raw_bytes)
    ...
    for index, (parent_q,) in enumerate(struct.iter_unpack("<H", q16)):
        ...
        if adapter.activation_id != activation:
            mixture.reset(adapter.donor if arm in "TORS" else None)
            activation = adapter.activation_id
        q = mixture.predict(parent_q)
        changed += q != parent_q
        active_bits += bool(adapter.donor) and arm in "TORS"
    ...
    if operation == "decode":
        ...
        output = archive
    report = {"arm": arm, "adapter_policy_id": getattr(adapter, "policy_id", ORIGINAL_POLICY),
        "adapter_constructor_arm": adapter.arm, "decoder_arm_option_bytes": 1,
...
def main(argv=None):
    ...
    restrict(resource.RLIMIT_CPU, 60)
