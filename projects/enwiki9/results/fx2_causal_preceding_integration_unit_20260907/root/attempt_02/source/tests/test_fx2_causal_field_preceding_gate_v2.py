...
import fx2_causal_field_preceding_gate_v2 as gate_tool
import fx2_causal_field_replay_v1 as original_core
...
SOURCE_NAMES = [gate_tool.CORE, *gate_tool.DYNAMIC_SOURCES]
...
def report(arm="T"):
    ...
    value.update(arm=arm, raw_bytes=100, modeled_bytes=101, archive_bytes=100, payload_bytes=54,
                 ...
                 adapter_policy_id=("causal-field-original-first-wrt-v1" if arm in "PO"
                                    else "causal-field-immediately-preceding-wrt250k-v1"),
                 adapter_constructor_arm="T" if arm == "O" else arm, decoder_arm_option_bytes=1)
    return value


class GateTests(unittest.TestCase):
    def test_output_declarations_have_all_eighteen_independent_phases(self):
        paths = gate_tool.declared_outputs([{"path": name} for name in SOURCE_NAMES])
        self.assertEqual(len(paths), len(set(paths)))
        self.assertEqual(sum(path.endswith(".execution.json") for path in paths), 18)
        self.assertEqual(sum(path.endswith(".sync") for path in paths), 18)
        self.assertEqual(sum(path.endswith("/result.json") for path in paths), 6)
    ...
    def test_canonical_guard_metadata_uses_nul_separated_command_hash(self):
        # Operational metadata only: no corpus or probability payload is read.
        audit = json.loads((ROOT / gate_tool.AUDIT).read_text())
        guard = json.loads(gate_tool.read_ref(ROOT, audit["guard"]))
        gate_tool.validate_guard_command(guard)
        with self.assertRaises(ValueError):
            gate_tool.validate_guard_command({**guard, "command_sha256": "0" * 64})
        self.assertNotEqual(gate_tool.digest(b"\\0".join(map(os.fsencode, guard["command"]))),
                            guard["command_sha256"])

    def test_preflight_cannot_open_reserved_payload_through_antecedents(self):
        for name, size, value in gate_tool.POPULATION.values():
            with self.subTest(name=name), patch.object(gate_tool, "read_ref",
                    side_effect=AssertionError("payload reader reached")), self.assertRaisesRegex(
                        ValueError, "population payload requested"):
                gate_tool.read_metadata_ref(ROOT, {"path": name, "bytes": size, "sha256": value})
    ...
    def comparison_reports(self):
        ...
        rows["O"]["archive_bytes"] = rows["R"]["archive_bytes"] = rows["S"]["archive_bytes"] = 110
        return rows
    ...
    def test_inactive_original_control_does_not_reject_active_adjacent_controls(self):
        rows = self.comparison_reports()
        rows["O"]["changed_probability_bits"] = 0
        result = gate_tool.scientific_comparison(rows, 30, 1)
        self.assertEqual(result["failure_class"], "conditional_archive_gain")
        self.assertFalse(result["original_control_active"])
        self.assertFalse(result["original_control_required_active"])

    def test_policy_and_option_are_mandatory(self):
        for arm in gate_tool.ARMS:
            value = report(arm)
            gate_tool.checked_report(value, "encode", arm, {})
            for changes in ({"decoder_arm_option_bytes": 0}, {"decoder_arm_option_bytes": True},
                            {"adapter_policy_id": "unbound"}, {"adapter_constructor_arm": "?"}):
                with self.subTest(arm=arm, changes=changes), self.assertRaises(ValueError):
                    gate_tool.checked_report({**value, **changes}, "encode", arm, {})
    ...
    def test_failed_control_and_weak_compression_are_distinct(self):
        ...
        rows["O"]["archive_bytes"] = rows["R"]["archive_bytes"] = rows["S"]["archive_bytes"] = 140
    ...
    def test_metadata_preflight_never_opens_payloads(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            indexed = [{"path": name, "bytes": size, "sha256": value}
                       for name, size, value in gate_tool.POPULATION.values()]
            decision_ref = add("results/synthetic-decision.json",
                               {"candidateId": gate_tool.ANCESTOR, "objective": objective, "artifacts": indexed})
            audit = {"candidate_id": gate_tool.ANCESTOR, "job": job_ref,
                     "guard": guard_ref, "decision": decision_ref,
                     "measurements": {key: True for key in (
                         "correctness_pass", "frozen_inputs_reverified", "artifact_closure_pass",
                         "continuous_guard_pass", "accounting_pass", "state_agreement_pass",
                         "parent_bookkeeping_identity_pass")},
                     "resource": {"cleanup_complete": True, "guards": {}}}
            audit_ref = add(gate_tool.AUDIT, audit)
            reflection_ref = add(gate_tool.REFLECTION, {"candidateId": gate_tool.ANCESTOR,
                                 "validity": {"valid": True}, "decision": {"verdict": "hold"},
                                 "objective": objective, "job": job_ref, "evidence": [audit_ref]})
            for name in (gate_tool.SELF, gate_tool.CORE, "lib/fx2_native_gate_v1.py",
                         "lib/artifacts.py", "lib/driver.py", "tools/research_contracts.py",
                         "tools/enwiki9_lab.py", *gate_tool.DYNAMIC_SOURCES):
            ...
            plan = {"candidate_id": gate_tool.ID, "resources": gate_tool.CAPS, "population": population,
                    "antecedents": [job_ref], "local_package_files": [inputs[name] for name in SOURCE_NAMES]}
    ...
    def test_all_eighteen_real_cli_processes_and_unmodified_driver_rows(self):
        ...
        with temporary as tmp:
            ...
            raw = (b"{{t|id=A|kind=x|v=mmmmmmmm}}"
                   b"{{t|id=B|kind=y|v=nnnnnnnn}}"
                   b"{{t|id=C|kind=x|v=mmmmmmmm}}")
            stored, trace, archive = fixture(raw)
            modeled = stored[10:]
            q16, projection = core.project_trace(trace, modeled, archive, len(raw))
            original_archive, original_sync, original_report = original_core.replay(
                "encode", modeled, q16, archive[:46], [], "T", len(raw), core.sha(raw))
            original_report.update(source_sha256=gate_tool.file_sha(ROOT / "tools/fx2_causal_field_replay_v1.py"),
                                   dictionary_sha256=core.sha(b""),
                                   synchronization_file_sha256=core.sha(original_sync))
            population = {"raw_bytes": len(raw), "modeled_bytes": len(modeled)}
            for field, name, payload in (
                    ("raw_path", "raw", raw), ("modeled_path", "modeled", modeled),
                    ("q16_path", "q16", q16), ("parent_archive_path", "archive", archive),
                    ("dictionary_path", "dictionary", b""),
                    ("original_archive_path", "original.archive", original_archive),
                    ("original_sync_path", "original.sync", original_sync),
                    ("original_result_path", "original.result.json",
                     json.dumps({"program_stats": {"phase": original_report}}).encode()),
                    ("projection_path", "projection.json", json.dumps(projection).encode())):
            ...
            self.assertEqual(len(gate.commands), 18)
            ...
            destination = os.environ.get("GAMMA_PRECEDING_GATE_SYNTHETIC_RETAIN")
            if destination:
