"""Reuse the fixed source-ZIP diagnostic on the compact parent and reset-only slot."""
import hashlib
...
module.SOURCES = {
    ...
    'treatment': ('opcode_slot_reset_v1', {
        'p': '7d331ba82c635af8afe6517bf5f4471b41dcf7854c6ec01cb5564078d9bb56a8',
        'program.py': 'a64ad5077250c1f5a10a3a26f94280c7c3c79f84bdd00ebab5d6308b66f5564e'})}
module.main()
out = Path(sys.argv[1]).resolve()
receipt = json.loads((out/'receipt.json').read_text())
counts = {r['name']: r['source_zip']['bytes'] for r in receipt['rows']}
wrapper = dict(schema='gamma.enwiki9.opcode-slot-reset-source-cost.v1',
    ...
    note='One fixed source-ZIP policy; six synthetic relocated codec phases. Runtime, options, licensing and accepted package form remain unresolved. Compare against the separate terminal corpus archive difference only after that job closes.')
with (out/'cost.json').open('x') as stream:
    json.dump(wrapper,stream,sort_keys=True,indent=2);stream.write('\n')
