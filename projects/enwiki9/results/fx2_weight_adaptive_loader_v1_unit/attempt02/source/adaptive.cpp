...
WeightsFile load_v2(Reader& r, const char* path, bool marginal = false, bool adaptive_counts = false) {
  ...
  const char selected = adaptive_counts ? 'A' : marginal ? (gamma.mode == 1 ? 'D' : 'G') : bookkeeping ? 'K' : 'P';
  if (std::fprintf(stderr, "\nGamma weight loader selected=%c tensors=%u histogram_tensors=%zu histogram_symbols=%llu side_information_bytes=%zu canonical=%d\n",
      ...
      marginal ? size_t(65) + gamma.records.size() * 64 : size_t(0), (marginal || adaptive_counts) ? 1 : 0) < 0 || std::fflush(stderr) != 0)
