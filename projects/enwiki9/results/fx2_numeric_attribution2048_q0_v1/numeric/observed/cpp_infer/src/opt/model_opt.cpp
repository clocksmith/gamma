...
gamma_numeric::dense(gamma_numeric::block("attention.query_projection"),L.qp.m,xn,L.qp.s_act,q5+0*D);
gamma_numeric::dense(gamma_numeric::block("attention.key_projection"),L.kp.m,xn,L.kp.s_act,q5+1*D);
gamma_numeric::dense(gamma_numeric::block("attention.value_projection"),L.vp.m,xn,L.vp.s_act,q5+2*D);
    }
    {
      PSCOPE(PS_MM_KQKV);
...
if(gamma_numeric::active()) {
          gamma_numeric::dense(gamma_numeric::block("attention.forget_gate_projection.up"),L.fg_up.m,xn,L.fg_up.s_act,q5+3*D);
          alignas(64) float temp[64]; qgemv_f32(L.fg_up.m,q5+3*D,temp);
          gamma_numeric::dense(gamma_numeric::block("attention.forget_gate_projection.down"),L.fg_down.m,temp,L.fg_down.s_act,q64b);
        }
    }
    {
      PSCOPE(PS_MM_GDN);
...
if(gamma_numeric::active()) {
          gamma_numeric::dense(gamma_numeric::block("attention.output_gate_projection.up"),L.og_up.m,xn,L.og_up.s_act,q5+4*D);
          alignas(64) float temp[64]; qgemv_f32(L.og_up.m,q5+4*D,temp);
          gamma_numeric::dense(gamma_numeric::block("attention.output_gate_projection.down"),L.og_down.m,temp,L.og_down.s_act,q64b);
        }
    }
    {
      PSCOPE(PS_MM_GDN);
...
gamma_numeric::dense(gamma_numeric::block("attention.output_projection"),L.op.m,gn,L.op.s_act,q5);
    }
    {
      PSCOPE(PS_MM_KOP);
...
gamma_numeric::dense(gamma_numeric::block("attention.output_projection"),L.op.m,pre,L.op.s_act,q5);
    }
    {
      PSCOPE(PS_MM_VOP);
      qgemv_add(L.op.m, q5, x);
...
gamma_numeric::dense(gamma_numeric::block("mlp.up"),mm.up.m,xn,mm.up.s_act,q5);
    }
    int nnz;
    {
      PSCOPE(PS_MM_MLP_UP);
...
gamma_numeric::emit("embedding.normalized",tok,D);
    int nnzp;
    {
      PSCOPE(PS_EL_PRIORQ);
...
gamma_numeric::layer=l; gamma_numeric::emit(gamma_numeric::block("input"),x,D);
      ...
      mlp_block(l);
gamma_numeric::emit(gamma_numeric::block("output"),x,D);
      if (l < 6) {
        PSCOPE(PS_EL_RESID);
...
gamma_numeric::dense("unembedding",M.unembed.m,xn,M.unembed.s_act,q5);
    }
    {
      PSCOPE(PS_MM_UNEMB);
