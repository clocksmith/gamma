...
def kda_gate_reference(g_raw: torch.Tensor, A_log: torch.Tensor, dt_bias: torch.Tensor) -> torch.Tensor:
...
def kda_recurrent_reference(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, g_raw: torch.Tensor, beta: torch.Tensor, A_log: torch.Tensor, dt_bias: torch.Tensor, cu_seqlens: torch.Tensor | None, dtype: torch.dtype=torch.float64, scale: float | None=None) -> torch.Tensor:
    ...
    qn = l2norm_reference(qf) * scale
    kn = l2norm_reference(kf)
    ...
    for a in range(len(bounds) - 1):
        ...
        for t in range(bounds[a], bounds[a + 1]):
            S = S * decay[t].unsqueeze(-1)
            r = (kn[t].unsqueeze(-1) * S).sum(-2)
            u = bf[t].unsqueeze(-1) * (vf[t] - r)
            S = S + kn[t].unsqueeze(-1) * u.unsqueeze(-2)
            o[t] = (qn[t].unsqueeze(-1) * S).sum(-2)
    return o.unsqueeze(0)

def fused_rmsnorm_gated_reference(x: torch.Tensor, gate: torch.Tensor, weight: torch.Tensor, eps: float=NORM_GATE_EPS, dtype: torch.dtype=torch.float32) -> torch.Tensor:
...
def causal_conv1d_silu_reference(x: torch.Tensor, weight: torch.Tensor, cu_seqlens: torch.Tensor | None, dtype: torch.dtype=torch.float32) -> torch.Tensor:
...
def _err(name: str, out: torch.Tensor, ref: torch.Tensor, floor: float=0.001):
    ...
    print(f'  {name:44s} max|d|={maxabs:9.3e}  max rel(floor {floor:g})={rel:9.3e}  max rel(|ref|>1e-2)={relbig:9.3e}  rms(ref)={rms:8.3e}')
    return (maxabs, rel)

def _load_ckpt_gate_params(device):
    path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'models', '6m-q4-fp32.tch')
    try:
        ...
        print(f'loaded A_log/dt_bias from checkpoint: exp(A_log)={A_log.exp().tolist()}, dt_bias in [{dt_bias.min().item():.4f}, {dt_bias.max().item():.4f}]')
...
def _validate(device: torch.device):
    ...
    from .reference_kernels import fused_recurrent_kda
    ...
    o_chunk, _ = chunk_kda(q=q, k=k, v=v, g=g_raw, beta=beta, A_log=A_log, dt_bias=dt_bias, use_qk_l2norm_in_kernel=True, use_gate_in_kernel=True, cu_seqlens=cu)
    o_rec, _ = fused_recurrent_kda(q=q, k=k, v=v, g=g_raw, beta=beta, A_log=A_log, dt_bias=dt_bias, use_qk_l2norm_in_kernel=True, use_gate_in_kernel=True, cu_seqlens=cu)
    ref64 = kda_recurrent_reference(q, k, v, g_raw, beta, A_log, dt_bias, cu, dtype=torch.float64)
    ref32 = kda_recurrent_reference(q, k, v, g_raw, beta, A_log, dt_bias, cu, dtype=torch.float32)
