#!/usr/bin/env python3
"""Owned synthetic-only retention wrapper; never opens a corpus or lifecycle row."""
import hashlib
import json
import os
...
PLAN = "operations/provenance/causal_field_preceding_synthetic_q0_v1_plan.json"
SOURCES = ["tools/causal_field_preceding_selector_v1.py", "tests/test_causal_field_preceding_selector_v1.py",
           "tools/causal_field_wrt_adapter_v1.py", "tools/causal_field_dependency_v1.py",
           "tools/wrt_exact.py", "tools/causal_field_parent_coder_v1.py"]
...
def main():
    ...
    command = [sys.executable, str(ROOT / SOURCES[1])]
    environment = {**os.environ, "TMPDIR": str(scratch), "TMP": str(scratch), "TEMP": str(scratch),
                   "GAMMA_PRECEDING_RETAIN": str(attempt), "PYTHONDONTWRITEBYTECODE": "1",
                   "OMP_NUM_THREADS": "1", "OPENBLAS_NUM_THREADS": "1", "MKL_NUM_THREADS": "1"}
    write(attempt / "command.json", {"command": command, "cwd": str(ROOT), "caps": CAPS,
        "publication": "3847955f074ee1b7d2739f0477786cdc65a892ab", "sources_before": before,
        "environment_overrides": {key: environment[key] for key in
            ("TMPDIR", "TMP", "TEMP", "GAMMA_PRECEDING_RETAIN", "PYTHONDONTWRITEBYTECODE",
    ...
    receipt = {"schema": "gamma.enwiki9.preceding-field-synthetic-execution.v1", "command": command,
        "returncode": returncode, "error": error, "wall_seconds": time.monotonic() - started,
    ...
    write(attempt / "inventory.json", {"files": rows, "indexed_bytes": total,
