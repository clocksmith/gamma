...
#ifndef GAMMA_FX2_HEAD_TRANSPORT_V2_HPP
#define GAMMA_FX2_HEAD_TRANSPORT_V2_HPP
#include <array>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
struct Model {
  ...
  uint32_t predict(uint32_t p){
    ...
    // Original byte_mixer_output is a forced zero sentinel before its first Perceive.
    const bool startup = position==0 && !active && native==1 && p==1;
    if (!(b==native || startup)) {
      std::fprintf(stderr,"native bit mismatch position=%llu reconstructed=%u captured=%u parent=%u active=%u\n",
                   (unsigned long long)position,b,native,p,unsigned(active));
      std::abort();
    }
