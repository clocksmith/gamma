...
void Predictor::AddMixers() {
  ...
  if (!transformer_) Fail("this build requires the native transformer profile");
...
void Predictor::Perceive(int bit) {
  ...
  if (byte_update) {
    ...
    if (byte_model_) {
      ...
      Fail("LSTM fallback is absent from this fixed transformer profile");
      }
    } else {
