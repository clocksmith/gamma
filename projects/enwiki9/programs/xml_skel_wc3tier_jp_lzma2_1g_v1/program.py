"""xml_skel_wc3tier_jp_lzma2_1g_v1 — wc3tier with shared-vocab joint-prose co-wordcoding (comment, title, username)."""
...
def compress(data: bytes) -> bytes:
    ...
    if rt_scaffold != scaffold:
        return _build_archive(MODE_LITERAL, _literal_bodies(data),
                              total_size, total_hash)
    # Co-wordcode the prose-class atom channels (comment, title, username)
    # with the shared scaffold vocab. id/timestamp/ip stay raw — non-prose.
    JOINT_CHANNELS = ("comment", "title", "username")
    packed_jp: dict[str, bytes] = {}
    for n in JOINT_CHANNELS:
        pkb = _pack_comment_with_vocab(channels[n], top1, top2, top3)
        if _unpack_comment_with_vocab(pkb, top1, top2, top3) != channels[n]:
            return _build_archive(MODE_LITERAL, _literal_bodies(data),
                                  total_size, total_hash)
        packed_jp[n] = pkb
    bodies = {"scaffold": packed_scaffold}
    for n in CHANNEL_NAMES:
        if n in JOINT_CHANNELS:
            bodies[n] = packed_jp[n]
        else:
