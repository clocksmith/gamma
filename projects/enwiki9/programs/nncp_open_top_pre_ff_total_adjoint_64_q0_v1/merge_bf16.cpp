...
std::vector<std::uint16_t> read_words(const fs::path & path) {
    ...
    if (!input) throw std::runtime_error("truncated pre-FF residual");
    return words;
}
...
void write_words(const fs::path & path,
    ...
    if (!output) throw std::runtime_error("cannot write pre-FF residual");
}
    ...
    try {
        if (argc != 5) {
            throw std::runtime_error(
                "usage: merge_bf16 BRANCH DIRECT TOTAL NEGATED_CONTROL");
        }
        ...
        std::fprintf(stderr, "pre-FF residual merge failed: %s\n", error.what());
        return 1;
    }
}
