...
std::vector<float> affine(const std::vector<float> &normalized,
                          const std::vector<float> &bias) {
    if (normalized.size() != kSamples * kWidth || bias.size() != kWidth)
        throw std::runtime_error("final RMSNorm affine geometry differs");
    std::vector<float> result(normalized.size());
    for (std::size_t sample = 0; sample < kSamples; ++sample) {
        for (std::size_t feature = 0; feature < kWidth; ++feature) {
            result[sample * kWidth + feature] = round_bf16(
                normalized[sample * kWidth + feature] + bias[feature]);
        }
    }
    return result;
}
    ...
    try {
        if (argc != 5)
            throw std::runtime_error(
                "usage: final_rmsnorm_order INPUT_BF16 INCOMING_BF16 "
                "BIAS_BF16 OUTPUT_DIRECTORY");
        ...
        const std::vector<float> bias = read_bf16(argv[3], kWidth);
        const std::vector<float> normalized = forward(source);
        const fs::path output = argv[4];
        if (!fs::is_directory(output))
            throw std::runtime_error("output directory is missing");
        write_bf16(output / "forward.bf16", normalized);
        write_bf16(output / "affine.bf16", affine(normalized, bias));
