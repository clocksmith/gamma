...
X=["xz","-q","-c","-T1","--check=crc32","--lzma2=preset=9e,dict=1024MiB,lc=4,lp=0,pb=0"]
U=["xz","-q","-d","-c","--memlimit-decompress=0"]
O=re.compile(rb"<title>|<id>|<timestamp>|<username>|<ip>|<comment>|<text xml:space=\"preserve\">")
F={b"<title>":(1,b"</title>"),b"<id>":(2,b"</id>"),b"<timestamp>":(3,b"</timestamp>"),b"<username>":(4,b"</username>"),b"<ip>":(5,b"</ip>"),b"<comment>":(6,b"</comment>"),b'<text xml:space="preserve">':(7,b"</text>")}
O6=re.compile(rb"<title>|<id>|<timestamp>|<username>|<comment>|<text xml:space=\"preserve\">")
F6={b"<title>":(1,b"</title>"),b"<id>":(2,b"</id>"),b"<timestamp>":(3,b"</timestamp>"),b"<username>":(4,b"</username>"),b"<comment>":(5,b"</comment>"),b'<text xml:space="preserve">':(6,b"</text>")}
...
def pg(d):
 s=bytearray();c=[[] for _ in range(8)];p=0
 ...
 for k in (7,1,2,3,4,5,6):
  pv(o,len(c[k]))
  for v in c[k]:pv(o,len(v));o.extend(v)
 return bytes(o)
def pg6(d):
 s=bytearray();c=[[] for _ in range(7)];p=0
 for m in O6.finditer(d):
  if m.start()<p:continue
  k,z=F6[m.group(0)];e=d.find(z,m.end())
  if e<0:continue
  em(s,d[p:m.end()]);s.extend((0,k));c[k].append(d[m.end():e]);p=e
 em(s,d[p:]);o=bytearray(b"GF1");pv(o,len(s));o.extend(s)
 for k in range(1,7):
...
def ug(d):
 ...
 p=3;n,p=gv(d,p);s=d[p:p+n];p+=n;c=[[] for _ in range(8)]
 for k in (7,1,2,3,4,5,6):
  q,p=gv(d,p)
  for _ in range(q):n,p=gv(d,p);c[k].append(d[p:p+n]);p+=n
 i=[0]*8;o=bytearray();p=0
 while p<len(s):
  b=s[p]
...
def uw(d):
 ...
 return bytes(o)
def compress(d):
 q=pw(pt(pg(d)));q6=pw(pt(pg6(d)));r=b"R"+x(d);g=b"O"+x(q);b=b"B"+bz2.compress(q,9);p=b"P"+x(q6);c=b"C"+bz2.compress(q6,9)
 return min((r,g,b,p,c),key=len)
def decompress(d):
 m=d[:1]
 if m==b"R":return ux(d[1:])
 if m in (b"O",b"B"):return ug(ut(uw(bz2.decompress(d[1:]) if m==b"B" else ux(d[1:]))))
 if m in (b"P",b"C"):return ug6(ut(uw(bz2.decompress(d[1:]) if m==b"C" else ux(d[1:]))))
