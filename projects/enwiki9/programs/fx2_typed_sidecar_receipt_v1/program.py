"""fx2_typed_sidecar_receipt_v1.

Score-honest wrapper around an fx2-cmix fork that keeps fx2's byte stream
unchanged and adds decoder-built typed receipt state for templates,
categories, refs, params, page sections, and page shape contexts.
"""
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(_BIN_GZ, "fx2-typed-receipt-cmix-", True)
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(_DICT_GZ, "fx2-typed-receipt-dict-", False)
