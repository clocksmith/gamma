...
ID='fx2_residual_features250k_v1'
PLAN='operations/provenance/'+ID+'_plan.json'
ADAPTER='operations/provenance/fx2_residual_features_adapter_v1.json'
...
def execute(g):
    ...
    for key in ('model','dictionary'):
        row=package[key];g.copy(row['path'],native/row['path'].split('/work/native/')[1])
