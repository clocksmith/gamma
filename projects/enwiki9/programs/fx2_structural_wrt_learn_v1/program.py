"""fx2_structural_wrt_learn_v1.

Score-honest wrapper around an fx2-cmix fork with structural sidecar models
and WRT-learned sidecar word expansion. The byte stream is unchanged; the
binary and dictionary are gzip-compressed beside this file and counted by the
harness.
"""
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(_BIN_GZ, "fx2-struct-wrt-cmix-", True)
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(_DICT_GZ, "fx2-struct-wrt-dict-", False)
