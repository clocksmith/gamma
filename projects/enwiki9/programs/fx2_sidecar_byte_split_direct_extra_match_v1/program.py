"""fx2_sidecar_byte_split_direct_extra_match_v1.
...
inputs, and extra sidecar match contexts. The binary and dictionary are stored
gzip-compressed beside this file and counted by the harness.
"""
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(
            _BIN_GZ, "fx2-sidebyte-split-direct-extra-match-cmix-", True
        )
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(
            _DICT_GZ, "fx2-sidebyte-split-direct-extra-match-dict-", False
        )
