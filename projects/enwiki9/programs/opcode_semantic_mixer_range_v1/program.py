...
TS = sorted(enumerate(TT, 1), key=lambda x: len(x[1]), reverse=True)
TD = dict(enumerate(TT, 1))
FULL = 1 << 32
HALF = 1 << 31
QTR = 1 << 30
TOT = 4096
...
def od(d: bytes) -> bytes:
    ...
    while i < n:
        ...
        if b:
            ...
            continue
        c = d[i + 1]
        o.extend(b"\0" if c == L0 else TD[c])
        i += 2
    return bytes(o)
...
class ST:
    ...
    def apply_op(self, c: int) -> None:
        if c == 1:
            ...
            self.w = 1
            self.ld = min(7, self.ld + 1)
            self.lh = hx(0, 67 if c == 22 else 73 if c == 23 else 0)
        elif c == 25:
            self.remember(self.lh)
            self.ld = max(0, self.ld - 1)
            self.w = 1 if self.ld else 0
...
def _compress_stream(d: bytes) -> bytes:
    ae = AE()
    c = COD()
...
def compress(d: bytes) -> bytes:
    x = oe(d)
    y = _compress_stream(x)
    LAST_STATS["opcoded_size"] = len(x)
    return y


def decompress(a: bytes) -> bytes:
    return od(_decompress_stream(a))
