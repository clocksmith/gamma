...
public:
    ...
    std::vector<float> multiply(const std::vector<float> & matrix,
        ...
        if (matrix.size() != static_cast<size_t>(k * outputs * batches) ||
            ...
            throw std::runtime_error("LibNC BF8 matrix geometry mismatch");
        if (k <= 0)
            throw std::runtime_error("LibNC matrix inner dimension must be positive");
        if ((k & 255) != 0)
            return multiply_direct_bf16(matrix, input, k, outputs, columns, batches);
        const int64_t blocks = k / 256;
        std::vector<float> output(outputs * columns * batches, 0.0f);
        std::vector<int8_t> matrix_q(matrix.size());
        std::vector<int8_t> input_q(input.size());
        std::vector<float> matrix_scale(batches * outputs * blocks);
        std::vector<float> input_scale(batches * columns * blocks);
        for (int64_t batch = 0; batch < batches; batch++) {
            for (int64_t row = 0; row < outputs; row++) {
                const size_t base = (batch * outputs + row) * k;
                quantize_blocks(matrix.data() + base, matrix_q.data() + base,
                                matrix_scale.data() + (batch * outputs + row) * blocks,
                                blocks);
            }
            for (int64_t column = 0; column < columns; column++) {
                const size_t base = (batch * columns + column) * k;
                quantize_blocks(input.data() + base, input_q.data() + base,
                                input_scale.data() + (batch * columns + column) * blocks,
                                blocks);
            }
            for (int64_t column = 0; column < columns; column++) {
                for (int64_t row = 0; row < outputs; row++) {
                    float accumulator = 0.0f;
                    const size_t matrix_base = (batch * outputs + row) * k;
                    const size_t input_base = (batch * columns + column) * k;
                    for (int64_t block = 0; block < blocks; block++) {
                        const int32_t dot = dot_i8_256(
                            matrix_q.data() + matrix_base + block * 256,
                            input_q.data() + input_base + block * 256);
                        const float scale =
                            matrix_scale[(batch * outputs + row) * blocks + block] *
                            input_scale[(batch * columns + column) * blocks + block];
                        accumulator = std::fma(static_cast<float>(dot), scale, accumulator);
                    }
                    output[(batch * columns + column) * outputs + row] = accumulator;
                }
            }
        }
        return output;
...
private:
    static std::vector<float> multiply_direct_bf16(
        const std::vector<float> & matrix, const std::vector<float> & input,
        int64_t k, int64_t outputs, int64_t columns, int64_t batches) {
        std::vector<float> result(outputs * columns * batches, 0.0f);
        for (int64_t batch = 0; batch < batches; batch++)
            for (int64_t column = 0; column < columns; column++)
                for (int64_t row = 0; row < outputs; row++) {
                    float accumulator = 0.0f;
                    ...
                    for (int64_t inner = 0; inner < k; inner++)
                        accumulator = std::fma(
                            matrix[matrix_base + inner], input[input_base + inner],
                            accumulator);
                    result[(batch * columns + column) * outputs + row] = accumulator;
                }
        return result;
    }

    static void quantize_blocks(const float * source, int8_t * destination,
                                float * scales, int64_t blocks) {
        constexpr float minimum_scale = 1.0e-35f;
        for (int64_t block = 0; block < blocks; block++) {
            const float * input = source + block * 256;
            int8_t * output = destination + block * 256;
            float maximum = 0.0f;
            for (int index = 0; index < 256; index++)
                maximum = std::max(maximum, std::fabs(input[index]));
            const float scale = bf16(std::max(maximum / 127.0f, minimum_scale));
            scales[block] = scale;
            const float inverse = 1.0f / scale;
            for (int index = 0; index < 256; index++) {
                int value = static_cast<int>(std::nearbyint(input[index] * inverse));
    ...
    static int32_t dot_i8_256(const int8_t * left, const int8_t * right) {
        ...
        for (int index = 0; index < 256; index += 32) {
            ...
            const __m256i absolute_a = _mm256_sign_epi8(a, a);
            const __m256i signed_b = _mm256_sign_epi8(b, a);
            const __m256i pairs = _mm256_maddubs_epi16(absolute_a, signed_b);
...
int main(int argc, char ** argv) {
    try {
        ...
        for (int layer = 0; layer < profile::layers; layer++) {
            ...
            std::vector<float> raw_relative = engine.multiply(
