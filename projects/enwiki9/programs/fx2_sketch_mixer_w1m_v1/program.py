"""fx2_sketch_mixer_w1m_v1.

Score-honest wrapper around an fx2-cmix fork that keeps the byte stream
unchanged and adds a fixed-seed Count-Min prefix sketch as a narrow mixer
context family. The binary and dictionary are gzip-compressed beside this file
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(_BIN_GZ, "fx2-sketch-cmix-", True)
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(_DICT_GZ, "fx2-sketch-dict-", False)
