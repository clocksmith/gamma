...
int main() {
    ...
    std::vector<float> final_grad(initial_weights.size());
    ggml_backend_tensor_get(weights, final_weights.data(), 0, ggml_nbytes(weights));
    ggml_backend_tensor_get(
        ggml_opt_grad_acc(opt, weights), final_grad.data(), 0,
        ggml_nbytes(ggml_opt_grad_acc(opt, weights)));
    for (float value : final_weights) {
        if (!std::isfinite(value)) {
            return 12;
        }
    }
    for (float value : final_grad) {
        if (!std::isfinite(value)) {
            return 13;
        }
    }
