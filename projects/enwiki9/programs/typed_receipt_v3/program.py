...
class TOK:
 ...
 def _vint_dec(s,a,d,kk):
  ...
  return v-1
 def ercptc(s,st,k,L):return s._vint_bits(s.__dict__.setdefault('ek',{}),st.c,k-1)+s._vint_bits(s.__dict__.setdefault('el',{}),st.c,L-4)
 def ercpte(s,a,st,k,L):
  s._vint_enc(a,s.__dict__.setdefault('ek',{}),st.c,k-1)
  s._vint_enc(a,s.__dict__.setdefault('el',{}),st.c,L-4)
 def ercptd(s,a,st):
  k=s._vint_dec(a,s.__dict__.setdefault('ek',{}),st.c)+1
  L=s._vint_dec(a,s.__dict__.setdefault('el',{}),st.c)+4
  return k,L
 ...
 while i<n:
  ...
  if st.ents:
   ...
   for k in range(1,ne+1):
    ...
    el=len(e)
    if el<4:continue
    # longest common prefix length, bounded by min(el, m)
    mm=min(el,m)
    if d[i]!=e[0]:continue
    L=1
    while L<mm and d[i+L]==e[L]:L+=1
    if L>=4 and L>best_L:best_L=L;best_k=k
   if best_L>=4:
    L=best_L;k=best_k
    lit=(pre[i+L]-pre[i])+L*tok.evc(st,0);c=tok.evc(st,3)+tok.ercptc(st,k,L)
    g=lit-c
  ...
  if best[1]==0:
   ...
   _,_,L,k,_,_=best;cnt[3]=cnt[3]+1 if len(cnt)>3 else 1;mb[3]=mb[3]+L if len(mb)>3 else L;tok.eve(a,st,3);tok.ercpte(a,st,k,L)
   for p in range(i,i+L):addh(ht,d,p);upd_byte(ppm,st,ch,d,p,d[p])
   i+=L
...
def decompress(x):
 ...
 while len(o)<n:
  ...
  if m==0:
   ...
   e=st.ents[-k]
   if L>len(e):raise ValueError("ercpt_L")
   for kk in range(L):
