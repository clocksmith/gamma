...
D=os.path.dirname(__file__);P=tempfile.gettempdir()+"/fx2ct"+str(os.getpid());B=P+"b";E=P+"d";O=b"  <page>\n";C=b"  </page>\n"
def b():
...
def k(p):
 return re.sub(rb"[^a-z0-9]+",b" ",(f(p,rb"\[\[Category:([^\]\|\n]{1,96})")or f(p,rb"\{\{([^\|\}\n]{1,96})")or f(p,rb"<title>(.*?)</title>")).lower()).strip()[:120]
def o(z):
 x=sp(z)
 if not x:return
...
def compress(z):
 y=o(z);return b"T"+r("-c",y)if y else b"R"+r("-c",z)
def decompress(z):
 y=r("-d",z[1:]);return e(y)if z[:1]==b"T"else y
