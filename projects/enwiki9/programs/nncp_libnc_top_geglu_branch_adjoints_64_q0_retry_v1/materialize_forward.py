#!/usr/bin/env python3
"""Derive a minimal exact forward exposing the final RMSNorm input."""
...
OLD_WRITER = """static void write_f32(const fs::path & path, const std::vector<float> & values) {
...
"""
NEW_WRITER = """static void write_f32(const fs::path & path, const std::vector<float> & values) {
    ensure_finite(values, path.filename().string());
    const std::string name = path.filename().string();
    if (name != \"output.f32\" && name != \"final_hidden.f32\" &&
        name != \"final_norm_input.f32\" &&
        name != \"layer_19_ff1_output.f32\" &&
...
"""
OLD_FINAL_NORM = """        const std::vector<float> final_hidden = rms_norm(
            hidden,
            fixture.parameter(\"ln_g_40\", {profile::width}),
            fixture.parameter(\"ln_b_40\", {profile::width}));
"""
NEW_FINAL_NORM = """        write_f32(output_dir / \"final_norm_input.f32\", hidden);
...
def main() -> int:
    ...
    if source.count(OLD_WRITER) != 1 or source.count(OLD_FINAL_NORM) != 1:
        raise ValueError("exact-forward patch boundary is not unique")
    source = source.replace(OLD_WRITER, NEW_WRITER)
    source = source.replace(OLD_FINAL_NORM, NEW_FINAL_NORM)
