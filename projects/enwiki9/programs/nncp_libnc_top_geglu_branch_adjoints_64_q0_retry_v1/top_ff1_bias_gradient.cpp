...
float streaming_dot(const float * left, const float * right) {
        ...
        workers.emplace_back([&, worker] {
            ...
            for (std::size_t sample = worker; sample < kSamples;
                ...
                for (std::size_t inner = 0; inner < kInner; ++inner) {
                    const float upstream = round_bf16(streaming_dot(
                        ff2.data() + inner * kWidth, current.data()));
...
int main(int argc, char ** argv) {
    try {
        ...
        const std::vector<float> ff2 = read_ff2(argv[1]);
