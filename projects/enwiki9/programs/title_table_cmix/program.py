"""title_table_cmix — title_table_v1's online back-reference preprocessor on
top of cmix as the back-end (instead of lzma).

This is the architecture test the analysis prescribed: title_table_v1 lost
under lzma (+0.28% / +596,688 bytes on full enwik9) because lzma's match
finder already extracts most of the title redundancy. cmix's match model is
weaker than its mixer, so the same preprocessor has a different chance of
helping on top of cmix. If it loses here too, retire the architecture for
this corpus; if it wins, the title-rolling-hash idea has measured backing
for cmix_sidecar_v1.

Score-honesty caveat: this wrapper subprocesses to a system-installed `cmix`
on PATH. For a contest-honest measurement, the cmix binary's bytes must be
counted in program_size — either inlined here as in cmix_wrapped/program.py,
or shipped as a sibling cmix.bin.gz. We intentionally do NOT inline here on
the first pass because the architecture test only cares about archive-size
delta vs. cmix_wrapped on the same input. Both compete with the same
unaccounted cmix binary, so the comparison is fair as a cmix-vs-cmix
oracle. Inline before reporting any leaderboard row.
"""
...
import re
import lzma  # imported for compatibility; not used as back-end here
