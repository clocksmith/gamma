...
class GST:
 def __init__(s):s.f=0;s.w=0;s.p=0;s.q=0;s.r=0;s.c=0;s.pg=0;s.col=0;s.tail=bytearray();s.word=bytearray();s.slot=0;s.ents=[];s.lb=bytearray();s.in_link=0;s.tb=bytearray();s.in_title=0;s.rb=bytearray();s.in_ref=0;s.mb=bytearray();s.in_macro=0;s.ub=bytearray();s.in_user=0
 def _ep(s,v):
  if 2<=len(v)<=128:
 ...
 def up(s,b):
  ...
  # template-name receipts: between {{ (opcode 20) and | / }} (opcode 21)
  if s.in_macro:
   if opc==21 or (b==124 and not opc):
    v=bytes(s.mb);s.mb=bytearray();s.in_macro=0;s._ep(v)
   else:
    if b!=0 and not (s.p==0 and b!=255):s.mb.append(b)
    if len(s.mb)>128:s.in_macro=0;s.mb=bytearray()
  if opc==20 and not s.in_macro:s.in_macro=1;s.mb=bytearray()
  # username receipts: between <username> (opcode 11) and </username> (opcode 12)
  if s.in_user:
   if opc==12:
    v=bytes(s.ub);s.ub=bytearray();s.in_user=0;s._ep(v)
   else:
    if b!=0 and not (s.p==0 and b!=255):s.ub.append(b)
  if opc==11:s.in_user=1;s.ub=bytearray()
  # ref-name receipts: detect "name=\"" in tail while in_ref state untouched (use t inspection)
  if s.in_ref:
   if b==34 or b==39:  # closing quote
    v=bytes(s.rb);s.rb=bytearray();s.in_ref=0;s._ep(v)
   else:
    if b!=0 and not (s.p==0 and b!=255):s.rb.append(b)
    if len(s.rb)>96:s.in_ref=0;s.rb=bytearray()
  if not s.in_ref and (tl.endswith(b'name="') or tl.endswith(b"name='")):s.in_ref=1;s.rb=bytearray()
...
def decompress(x):
 ...
 while len(o)<n:
  ...
  if m==0:
   ...
   e=st.ents[-k];L=len(e)
   for kk in range(L):
    b=e[kk];o.append(b);addc(ch,st,len(o)-1);ppm.up(b);st.up(b)
 return bytes(o[:n])
def stats():return _S
