...
int main(int argc, char ** argv) {
    try {
        ...
        for (int position = 0; position < profile::segment; position++) {
            input_symbols[position] = static_cast<int>(input_values[position]);
        ...
        for (int layer = 0; layer < profile::layers; layer++) {
            ...
            std::vector<float> query = engine.multiply(
            ...
            const std::vector<float> wkv = fixture.parameter(
                layer_name("w_kv", layer), {profile::width, 2 * profile::width});
            std::vector<float> key_value = engine.multiply(
                wkv, all_hidden, profile::width, 2 * profile::width, profile::position);
            ...
            for (int position = 0; position < profile::segment; position++)
                for (int head = 0; head < profile::heads; head++)
                    for (int dimension = 0; dimension < profile::key; dimension++) {
                        ...
                        attention_residual[destination] = bf16(hidden[destination] + attended[source]);
                    }
            ...
            const std::vector<float> ff1 = fixture.parameter(
                layer_name("ff1", layer), {profile::width, 2 * profile::inner});
            std::vector<float> ff1_output = engine.multiply(
                ff1, feedforward_input, profile::width, 2 * profile::inner,
                profile::segment);
            add_bias_round(
                ff1_output,
            ...
            const std::vector<float> ff2 = fixture.parameter(
                layer_name("ff2", layer), {profile::inner, profile::width});
            std::vector<float> feedforward = engine.multiply(
                ff2, geglu, profile::inner, profile::width, profile::segment);
            add_bias_round(
                feedforward,
        ...
        std::vector<float> logits = engine.multiply(
            fixture.parameter("embed_out", {profile::width, profile::vocabulary}),
            final_hidden, profile::width, profile::vocabulary, profile::segment);
        ...
        write_tree_path(output_dir / "tree_path.u32le", probabilities,
                        read_targets(fixture.root() / "target_symbols.u16le"));
