"""fx2_article_geometry_sort_v1.

Structural fx2-cmix with a reversible article-geometry page ordering candidate.
The order key is the v6 geometry key: redirects by target, category pages by
category set plus title prefix, then infobox/template/title fallback.
"""
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(_BIN_GZ, "fx2-article-geom-cmix-", True)
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(_DICT_GZ, "fx2-article-geom-dict-", False)
...
def _split_pages(data: bytes) -> tuple[bytes, list[bytes], bytes] | None:
    ...
    while True:
        ...
        pos = end
    return data[:first], pages, data[pos:]
...
def _geometry_key(page: bytes) -> bytes:
    redirect = _field(page, rb"#REDIRECT\s*\[\[([^\]\|\n]{1,140})")
    if redirect:
        return b"Z " + redirect
    title = _field(page, rb"<title>(.*?)</title>")
    cats = re.findall(rb"\[\[Category:([^\]\|\n]{1,100})", page, re.I)
    if cats:
        return b"C " + b" ".join(sorted(cats)) + b" T " + title[:40]
    infobox = _field(page, rb"\{\{\s*(infobox[^\|\}\n]{0,80})")
    if infobox:
        return b"I " + infobox
    template = _field(page, rb"\{\{([^\|\}\n]{1,80})")
    return b"X " + (template or title)


def _geometry_order(data: bytes) -> bytes | None:
    parts = _split_pages(data)
    if parts is None:
        return None
    head, pages, tail = parts
    ids = [_page_id(page, i) for i, page in enumerate(pages)]
    if len(pages) < 2 or ids != sorted(ids) or len(ids) != len(set(ids)):
        return None
    order = sorted(range(len(pages)), key=lambda i: (_norm(_geometry_key(pages[i])), ids[i]))
...
def _restore_id_order(data: bytes) -> bytes:
    ...
    if parts is None:
        return data
    head, pages, tail = parts
    pages.sort(key=lambda page: _page_id(page, 0))
    return head + b"".join(pages) + tail


def compress(data: bytes) -> bytes:
    raw = _run("-c", data)
    ordered = _geometry_order(data)
    if ordered is None:
        return _RAW + raw
    geom = _run("-c", ordered)
    if len(geom) < len(raw):
