"""fx2_sidecar_byte_split_direct_v1.

Score-honest wrapper around an fx2-cmix fork that keeps fx2's byte stream
unchanged and adds split semantic byte models plus direct sidecar probability
inputs. The binary and dictionary are stored gzip-compressed beside this file
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(_BIN_GZ, "fx2-sidebyte-split-direct-cmix-", True)
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(_DICT_GZ, "fx2-sidebyte-split-direct-dict-", False)
