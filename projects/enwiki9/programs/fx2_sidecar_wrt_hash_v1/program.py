"""fx2_sidecar_wrt_hash_v1.

Score-honest wrapper around an fx2-cmix fork that keeps fx2's byte stream
unchanged and adds split semantic byte models, direct sidecar probability
inputs, extra sidecar match contexts, page/shape sidecar match contexts, and
WRT-learned dictionary word expansion that also feeds decoded characters into
sidecar semantic hashes. The binary and dictionary are stored gzip-compressed
beside this file and counted by the harness.
"""
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(
            _BIN_GZ, "fx2-sidecar-wrt-hash-cmix-", True
        )
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(
            _DICT_GZ, "fx2-sidecar-wrt-hash-dict-", False
        )
