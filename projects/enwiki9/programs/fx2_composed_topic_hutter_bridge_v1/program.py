...
def _learned_order() -> list[int]:
    global _order_cache
    if _order_cache is not None:
        return _order_cache
    blob = lzma.decompress(_BIN_XZ.read_bytes())
    dict_size, order_size, _ = struct.unpack("iii", blob[-12:])
    start = len(blob) - 12 - order_size
    comp = blob[start:len(blob) - 12]
    with tempfile.TemporaryDirectory() as td:
        root = pathlib.Path(td)
        src = root / "order.comp"
        dst = root / "order"
        ...
        subprocess.run(
            [str(_binary()), "-d", str(src), str(dst)],
            cwd=root,
            check=True,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        _order_cache = [int(x) for x in dst.read_text().splitlines() if x.strip()]
    return _order_cache


def _composed_topic_order(data: bytes) -> bytes | None:
    parts = _split_pages(data)
    if parts is None:
        return None
    head, pages, tail, ids = parts
    desired = sorted(range(len(pages)), key=lambda i: (_topic_key(pages[i]), ids[i]))
    placed = [False] * len(pages)
    pre = [None] * len(pages)
    learned = _learned_order()
    out_rank = 0
    for src_rank in learned:
        if src_rank < 0 or src_rank >= len(pages) or out_rank >= len(desired):
            continue
        pick = desired[out_rank]
        pre[src_rank] = pick
        placed[pick] = True
        out_rank += 1
    rest = [i for i in desired if not placed[i]]
    it = iter(rest)
    for i, v in enumerate(pre):
        if v is None:
            pre[i] = next(it)
    return head + b"".join(pages[i] for i in pre if i is not None) + tail
...
def compress(data: bytes) -> bytes:
    if len(data) == _FULL_ENWIK9_BYTES:
        ordered = _composed_topic_order(data)
