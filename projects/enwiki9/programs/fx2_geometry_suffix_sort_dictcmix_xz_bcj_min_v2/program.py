...
def k(p):
 ...
 if c:return n(b"c "+b" ".join(S(c))+b" s "+u)
 i=f(p,rb"\{\{\s*(infobox[^|}\n]{0,80})")
 if i:return n(b"i "+i+b" s "+u)
 return n(b"x "+(f(p,rb"\{\{([^|}\n]{1,80})")or t)+b" s "+u)
def o(z):
 x=sp(z)
 if not x:return
