...
class COD:
 def __init__(s):s.m={};s.st=ST();s.evm={};s.lm={};s.dm={};s.last=(0,0);s.reps=[1,2,4,8]
 ...
 def lit_cost(s,b,pos):
  pfx=1;c=0.0
  for bp in range(8):
   bit=(b>>(7-bp))&1;ks=s.st.keys(bp,pfx,pos)
   ps=[s.bitp(k) for k in ks];p=sum(ps)//len(ps)
   c+=lg(p) if bit else lg(TOT-p);pfx=(pfx<<1)|bit
  return c
 def lit_enc(s,ae,b,pos):
  pfx=1
  for bp in range(8):
   bit=(b>>(7-bp))&1;ks=s.st.keys(bp,pfx,pos);p=sum(s.bitp(k) for k in ks)//len(ks);ae.bit(p,bit)
   for k in ks:s.model(k).u(bit)
   pfx=(pfx<<1)|bit
  s.st.up(b)
 def lit_dec(s,ad,pos):
  pfx=1;b=0
  for bp in range(8):
   ks=s.st.keys(bp,pfx,pos);p=sum(s.bitp(k) for k in ks)//len(ks);bit=ad.bit(p)
   for k in ks:s.model(k).u(bit)
   b=(b<<1)|bit;pfx=(pfx<<1)|bit
  s.st.up(b);return b
...
def compress(d):
 ...
 while i<n:
  best=None;lit1=c.evcost(0)+c.lit_cost(d[i],i)
  if i+4<=n:
   for L,D in matches(d,i,tab):
    lc=0.0;lim=min(L,32)
    ss=ST();ss.__dict__=c.st.__dict__.copy() if hasattr(c.st,"__dict__") else None
    for q in range(lim):lc+=c.lit_cost(d[i+q],i+q)
    mc=c.mcost(L,D)
    if mc+0.5<lc and (best is None or mc-lc<best[0]):best=(mc-lc,L,D)
  if best:
