"""xml_skel_wc3tier_lzma2_1g_v1 — Phase 1_WC3 + LZMA2 with 1 GB dict.

Typed MediaWiki XML field extraction + 3-tier wordcode pre-pass on the
prose scaffold + single monolithic LZMA2 (1 GiB dict_size) over the
concatenated typed buffer.

3-tier wordcode codespace:
  Tier 1: 128 single-byte codes 0x80-0xFF (most-frequent tokens)
  Tier 2: 256 two-byte codes 0x03 XX (next tier by save value)
  Tier 3: up to 4096 three-byte codes 0x04 XX YY (long-tail vocabulary)
  Total: up to 4480 vocabulary slots.

Picker assigns each token to the lowest tier where save = f*(L-c)-(1+L)
is positive (c is code width); each tier gets the next-best tokens not
already assigned to a higher tier. Token regex covers alphabetic words
[A-Za-z]{3,32} and wiki-markup tokens ([[, ]], {{, }}, ==+, '''+, '').

Escape byte 0x02 prefixes any literal 0x02, 0x03, 0x04, or >=0x80.
"""
...
def _wc_unpack(packed: bytes) -> bytes:
    ...
    return _wc_expand(body, top1, top2, top3)
