"""fx2_structural_sidecar_v2.

Score-honest wrapper around the current fx2-cmix structural sidecar build.
The byte stream is unchanged; structural sidecar contexts are consumed by the
backend predictor and mixer. The binary and dictionary are counted by the
harness.
"""
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(_BIN_GZ, "fx2-struct-v2-cmix-", True)
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(_DICT_GZ, "fx2-struct-v2-dict-", False)
