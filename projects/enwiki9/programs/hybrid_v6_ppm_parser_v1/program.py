 ...
 while i<n:
  ...
  if i+4<=n:
   for L,D in mat(d,i,tab):
    lc=0.0;lim=min(L,32)
    ss=ST()
    ss.f=st.f;ss.w=st.w;ss.p=st.p;ss.q=st.q;ss.c=st.c;ss.col=st.col
    ss.hist=list(st.hist);ss.tail=bytearray(st.tail)
    for q in range(lim):
     lc+=ppm.cost(ss,d[i+q])
     # We approximate that a literal costs the payload + the literal flag cost
     # Adding the literal flag cost makes the comparison fair vs match flag
     # tok.lc() returns the cost of literal flag (0) right now
     lc += tok.lc()
     ss.up(d[i+q])
    mc=tok.mc(L,D)
    if mc+0.5<lc and (best is None or mc-lc<best[0]):best=(mc-lc,L,D)
  if best:
   _,L,D=best;tok.me(a,L,D)
   for q in range(i,i+L):st.up(d[q]);add(tab,d,q)
   i+=L
  else:
