    ...
    try {
        ...
        for (std::size_t state = 0; state < kStates; ++state) {
            for (std::size_t stream = 0; stream < kStreams; ++stream) {
                ...
                const std::uint32_t shifted_target =
                    targets[stream + kStreams * ((state + 1) % kStates)];
                ...
                for (std::size_t symbol = 0; symbol < kVocabulary; ++symbol) {
                    const float scaled = row[symbol] * kLossScale;
                    gradient[symbol] += scaled;
                    shifted[symbol] += scaled;
                }
                gradient[target] -= kLossScale;
                shifted[shifted_target] -= kLossScale;
            }
        }
