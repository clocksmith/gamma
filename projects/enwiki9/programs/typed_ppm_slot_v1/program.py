...
class PPM:
 ...
 def keys(s,pref=b""):
  ...
  for n in range(s.o,-1,-1):
   if n<=len(s.t):r.append(pref+(bytes(s.t[-n:]) if n else b""))
  return r
 ...
 def up(s,b,pref=b""):
  for n in range(s.o+1):
   if n<=len(s.t):
    k=pref+(bytes(s.t[-n:]) if n else b"");m=s.mod(k);m.c[b]=m.c.get(b,0)+1;m.t+=1
  s.t.append(b)
...
def lit_prefix(d):
 p=PPM();st=GST();pre=[0.0]
 for b in d:
  pre.append(pre[-1]+p.cost(b,_pp(st)));p.up(b,_pp(st));st.up(b)
 return pre
