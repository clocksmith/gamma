...
def x(d):return lzma.compress(d,preset=P,check=0)
def ux(d):return lzma.decompress(d)
def pv(o,n):
...
def compress(d):
 q=pw(pt(pg(d)))
 return min((b"r"+bz2.compress(d,9),b"O"+x(q),b"B"+bz2.compress(q,9)),key=len)
def decompress(d):
 m=d[:1];p=d[1:]
 if m==b"r":return bz2.decompress(p)
 return ug(ut(uw(bz2.decompress(p) if m==b"B" else ux(p))))
