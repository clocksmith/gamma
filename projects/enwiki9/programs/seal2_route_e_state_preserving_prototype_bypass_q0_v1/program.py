"""State-preserving exact-copy bypass primitives for the Route E Q0 gate.

The score-bearing native hook is intentionally not claimed here. The gate
imports these deterministic framing, command, and range-coder primitives to
construct and decode an exact trace-level witness. ``compress`` and
``decompress`` provide a literal-only fallback for the repository program
contract and receive no score credit.
"""
...
MASK32 = (1 << 32) - 1
BYPASS_MAGIC = b"RTEBYPS1"
LITERAL_MAGIC = b"RTELIT1\0"
