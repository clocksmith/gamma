#!/usr/bin/env python3
"""Apply sealed Delta-MIDAS instrumentation, then realize corrected FTRL v5."""
...
#if KH_BITHEAD_FTRL_ARM == 1 || KH_BITHEAD_FTRL_ARM == 3 || KH_BITHEAD_FTRL_ARM == 4 || KH_BITHEAD_FTRL_ARM == 5
  for (int c0 = 0; c0 < kDmGateCoordinates; ++c0) {
    int64_t sum = 0;
    for (int j = 0; j < kDmRank; ++j) {
#if KH_BITHEAD_FTRL_ARM == 3
...
#endif
#if KH_BITHEAD_FTRL_ARM == 1 || KH_BITHEAD_FTRL_ARM == 2 || KH_BITHEAD_FTRL_ARM == 3 || KH_BITHEAD_FTRL_ARM == 4 || KH_BITHEAD_FTRL_ARM == 5
