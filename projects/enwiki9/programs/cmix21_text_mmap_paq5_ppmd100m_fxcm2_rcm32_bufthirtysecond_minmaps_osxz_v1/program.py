"""Score-honest xz package wrapper around disk-backed cmix version 21 text mode.
...
decompression uses the normal dictionary-aware decode path. This variant changes
only package coding from gzip to xz.
"""
...
def _extract(src: pathlib.Path, prefix: str, executable: bool) -> pathlib.Path:
    ...
    out.write_bytes(lzma.decompress(src.read_bytes()))
    if executable:
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(_BIN_XZ, "cmix21-mmap-bin-", True)
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(_DICT_XZ, "cmix21-mmap-dict-", False)
