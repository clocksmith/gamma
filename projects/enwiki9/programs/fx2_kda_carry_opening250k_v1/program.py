#!/usr/bin/env python3
"""One bounded native comparison of fixed causal KDA memory carryover."""
import hashlib
import json
...
ID='fx2_kda_carry_opening250k_v1'
...
ADAPTER='operations/provenance/fx2_kda_carry_adapter_v1.json'
...
N=151210
STATE_RECORD=20+9*(3*4096*4+3*4*192*4+4)
...
def states(path,zero=False):
    raw=path.read_bytes()
    require(len(raw)>STATE_RECORD and len(raw)%STATE_RECORD==0,'reset opportunity or framing absent')
    for at in range(0,len(raw),STATE_RECORD):
        row=raw[at:at+STATE_RECORD]
        require(row[:4]==b'KDC1' and row[4:20]==bytes(16),'clock boundary differs')
        for layer in range(9):
            start=20+layer*(STATE_RECORD-20)//9
            matrix=row[start:start+49152]
            require(all(abs(v)<float('inf') for v, in struct.iter_unpack('<f',matrix)), 'nonfinite state')
            require(row[start+49152:start+58372]==bytes(9220),'convolution state or position not reset')
        if zero:require(row[20:]==bytes(STATE_RECORD-20),'P/K carry state differs from reset')
    require(raw[:STATE_RECORD][20:]==bytes(STATE_RECORD-20),'cold initial memory differs')
...
def execute(g):
    ...
    for arm in 'PKDS':
        for phase in ('encode','decode','repeat'):
            ...
            try:
                g.run(name,[str(native/'cmix'),*args,'--transformer','models/6m-q4-fp32.tfwc2'],180,
                      env=dict(GAMMA_FX2_KDA_ARM=arm,GAMMA_FX2_KDA_TRACE=str(state),GAMMA_FX2_CODER_TRACE=str(trace)),work=native)
            finally:cleanup(g,name)
            require(trace.stat().st_size==N*8*28,'coder coordinate differs')
            count=states(state,arm in 'PK')
            if phase!='encode':
        ...
        arms[arm]=dict(archive=g.artifact(archive),archive_bytes=archive.stat().st_size,
            ...
            reset_boundaries=count,exact_inverse=True,archive_repeat_byte_equal=True,
            boundary_state_byte_equal=True,coder_trace_byte_equal=True)
        g.write('completed-arms.json',dict(arms=arms,complete=False))
    for suffix in ('arc','coder','state'):equal(native/('P-encode.'+suffix),native/('K-encode.'+suffix))
    ...
    verdict='Archive regression.' if gp<0 else 'No archive improvement.' if gp==0 else 'Parent improvement observed, but advantage over the rotated control is unconfirmed.' if gs<=0 else 'Positive native development comparison; confirmation and final packaging unmeasured.'
    return dict(arms=arms,g_P=gp,g_S=gs,n_1=None,n_2=None,
        ...
        PK_archive_byte_identity=True,PK_boundary_state_byte_identity=True,released_parent_archive_and_coder_parity=True,
        changed_state_audit='All nine KDA matrices, convolution rings and clocks at every reset; KV invalidated at t=0. Other native state follows unchanged source and the identical observed bits/probabilities.',
...
def main():
    ...
    result=dict(schema='gamma.enwiki9.kda-carry-native-terminal.v1',candidate_id=ID,experiment=g.reference,
