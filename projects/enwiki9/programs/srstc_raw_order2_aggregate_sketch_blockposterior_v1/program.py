...
ALPHA2 = 1
BLOCK_BYTES = 16_384
POSTERIOR_WEIGHT_BITS = 24
POSTERIOR_SRSTC_PRIOR_PPM = 500_000
...
class BlockPosteriorMixer:
    """Fixed-point posterior over base and SRSTC, reset per public block."""

    def __init__(self) -> None:
        self.scale = 1 << POSTERIOR_WEIGHT_BITS
        initial = (
            self.scale * POSTERIOR_SRSTC_PRIOR_PPM + 500_000
        ) // 1_000_000
        ...
        self.reset_count = 0
        self.rows = 0
        self.base_dominant_rows = 0
        self.srstc_dominant_rows = 0
        self.tied_rows = 0
        self.floor_clamp_count = 0
        self.ceiling_clamp_count = 0
...
class Predictor:
    def __init__(self) -> None:
        ...
        self.posterior = BlockPosteriorMixer()
        self.rows = 0
    ...
    def p1(
        self, bit_pos: int
    ) -> tuple[int, int, int, bytes, int, int, list[tuple[Any, ...]]]:
        ...
        srstc_p1 = blend_probability(base_p1, prior_p1)
        block = self.rows // (BLOCK_BYTES * 8)
        corrected = self.posterior.mix(block, base_p1, srstc_p1)
        if prior_p1 is not None:
            self.retrieval_rows += 1
            self.retrieval_hits += hits
        return corrected, base_p1, srstc_p1, history, partial_len, partial_prefix, keys
    ...
    def update(
        ...
        block = self.rows // (BLOCK_BYTES * 8)
        self.posterior.update(block, bit, base_p1, srstc_p1)
...
def compress(data: bytes) -> bytes:
    ...
    for byte in data:
        for bit_pos in range(8):
            ...
            p1, base_p1, srstc_p1, history, partial_len, partial_prefix, keys = pred.p1(
                bit_pos
            )
            enc.bit(bit, p1)
            pred.update(
    ...
    LAST_STATS.update(
        {
            ...
            "posterior_block_resets": pred.posterior.reset_count,
            "posterior_base_dominant_rows": pred.posterior.base_dominant_rows,
            "posterior_srstc_dominant_rows": pred.posterior.srstc_dominant_rows,
            "posterior_tied_rows": pred.posterior.tied_rows,
            "posterior_floor_clamp_count": pred.posterior.floor_clamp_count,
            "posterior_ceiling_clamp_count": pred.posterior.ceiling_clamp_count,
        }
    )
