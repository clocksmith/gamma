"""fx2_residual_feedback_v1.

Score-honest wrapper around an fx2-cmix fork with deterministic residual
feedback mixer contexts. The byte stream is unchanged; the mixer only sees
recent quantized prediction miss state derived from already-decoded bits.
"""
...
def _binary() -> pathlib.Path:
    ...
    if _extracted_bin is None or not _extracted_bin.exists():
        _extracted_bin = _extract(_BIN_GZ, "fx2-residual-cmix-", True)
...
def _dictionary() -> pathlib.Path:
    ...
    if _extracted_dict is None or not _extracted_dict.exists():
        _extracted_dict = _extract(_DICT_GZ, "fx2-residual-dict-", False)
