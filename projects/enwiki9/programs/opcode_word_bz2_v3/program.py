...
KS = (0, 4, 8, 12, 16, 20, 24, 32, 40, 48, 64, 96, 128)
...
def word_pack(data: bytes, words: list[bytes], whole: bool = False) -> bytes:
    ...
    if whole:
        pat = re.compile(rb"(?<![A-Za-z])(" + b"|".join(re.escape(w) for w in words) + rb")(?![A-Za-z])")
    else:
        pat = re.compile(b"|".join(re.escape(w) for w in words))
    pos = 0
    for m in pat.finditer(data):
        start = m.start(1) if whole else m.start()
        end = m.end(1) if whole else m.end()
        key = m.group(1) if whole else m.group(0)
        out.extend(esc(data[pos:start]))
        out.extend(mp[key])
        pos = end
...
def compress(data: bytes) -> bytes:
    ...
    for k in KS[1:]:
        words = pick_words(t, k)
        for whole in (False, True):
            body = bz2.compress(word_pack(t, words, whole), 9)
            if len(body) < len(best_body):
                best_body = body
                best_k = k
                best_mode = "whole" if whole else "substring"
    LAST_STATS.clear()
    LAST_STATS.update({
        "best_k": best_k,
        "token_size": len(t),
        "word_body_size": len(best_body),
        "mode": "raw" if len(raw) <= len(best_body) else best_mode,
    })
    if len(raw) <= len(best_body):
        return b"OWB3r" + raw
    tag = b"h" if best_mode == "whole" else b"s"
    return b"OWB3" + tag + best_body


def decompress(data: bytes) -> bytes:
    if data[:4] != b"OWB3":
        raise ValueError("bad opcode word bz2 v3 archive")
    mode = data[4:5]
    ...
    raise ValueError("bad opcode word bz2 v3 mode")
