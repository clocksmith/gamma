"""yellow_tucan_structural_range_v4.

Custom no-cmix/no-lzma arithmetic coder with MediaWiki parser-state contexts.
Raw bytes are unchanged; parser state selects probability tables. V4 keeps
the V2 selection rule but raises the structural context budget so later
article regions are not forced back to lower-order models after the table
fills.
"""
...
class Predictor:
    ...
    def model(self, create: bool) -> Model:
        k = self.st.key()
        m = self.ctx.get(k)
        if m is not None and m.t >= MIN_TRAINED:
            return m
        if m is None and create and len(self.ctx) < MAX_CTX:
            m = Model()
            self.ctx[k] = m
        m1 = self.o1.get(self.st.prev)
    ...
    def upd(self, s: int, m: Model) -> None:
        ...
        if m is not self.o0:
            self.o0.upd(s)
        m1 = self.o1.setdefault(self.st.prev, Model())
        if m1 is not m:
            m1.upd(s)
        ms = self.ctx.get(self.st.key())
...
def compress(data: bytes) -> bytes:
    ...
    body = e.finish()
    LAST_STATS.clear()
    LAST_STATS.update({"ctx1": len(p.o1), "structural_contexts": len(p.ctx)})
