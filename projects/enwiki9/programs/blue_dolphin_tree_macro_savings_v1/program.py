"""blue_dolphin_tree_macro_savings_v1 — savings-gated tree macros.
...
Admission rule:
    admit iff count >= MIN_FREQ and estimated raw stream saving is positive.

The estimate is intentionally conservative: it charges each macro definition,
each reference, each argument length, and a small safety margin for literal-run
fragmentation before allowing the LZMA backend to see the transformed stream.
...
def _arg_cost(args: list[bytes]) -> int:
    return len(_varint(len(args))) + sum(len(_varint(len(arg))) + len(arg) for arg in args)


def _rule_cost(rid: int, name: bytes, args: list[bytes]) -> int:
    keys = []
    for arg in args:
        eq = arg.find(b'=')
    ...
    return (
        1
        + len(_varint(rid))
        + len(_varint(len(name)))
        + len(name)
        + len(_varint(len(keys_sorted)))
        + sum(len(_varint(len(k))) + len(k) for k in keys_sorted)
    )


def _ref_cost(rid: int, args: list[bytes]) -> int:
    return 1 + len(_varint(rid)) + _arg_cost(args)


def _scan_shapes(data: bytes) -> dict[bytes, dict]:
        ...
        if i + 1 < n and data[i] == 0x7B and data[i + 1] == 0x7B:
            ...
            if length > 0:
                ...
                if name and 0 < len(args) <= 32 and len(name) < 200:
                    ...
                    row["count"] += 1
                    row["raw_bytes"] += length
                    row["arg_costs"].append(_arg_cost(args))
                i += length
                continue
        i += 1
    return stats
...
def _eligible_shapes(shape_stats: dict[bytes, dict]) -> dict[bytes, int]:
    eligible: dict[bytes, int] = {}
    next_id = 0
    for sh, row in shape_stats.items():
        if row["count"] < MIN_FREQ:
            continue
        # Estimate the actual stream format cost: first occurrence is a rule
        # definition plus its arguments, later occurrences are references plus
        # their arguments.
        rid = next_id
        encoded = _rule_cost(rid, row["name"], row["key_args"]) + sum(
            1 + len(_varint(rid)) + cost for cost in row["arg_costs"]
        )
        saving = row["raw_bytes"] - encoded
        if saving >= MIN_RAW_SAVING:
            eligible[sh] = saving
            next_id += 1
            if next_id >= MAX_RULES:
                break
    return eligible


def encode(data: bytes) -> tuple[bytes, dict]:
    shape_stats = _scan_shapes(data)
    eligible = _eligible_shapes(shape_stats)
