...
constexpr std::size_t kFullOutputs = 6144;
...
std::vector<std::uint16_t> project(
    ...
    if (inputs.size() != kSamples * kInputs ||
        ...
        throw std::runtime_error("FF1 weight-grid operand geometry differs");
    }
        ...
        for (std::size_t input = 0; input < kInputs; ++input) {
            for (std::size_t output = 0; output < kSliceOutputs;
                ...
                for (std::size_t stream = 0; stream < kStreams; ++stream) {
                    ...
                    std::uint16_t input_word =
                        inputs[sample * kInputs + input];
                    const float input_value = bf16_to_float(input_word);
...
void write_words(const fs::path & path,
    ...
    output.write(reinterpret_cast<const char *>(words.data()),
                 static_cast<std::streamsize>(words.size() * sizeof(words[0])));
    if (!output) throw std::runtime_error("cannot write FF1 weight-grid output");
}
    ...
    try {
        if (argc != 8) {
            throw std::runtime_error(
                "usage: ff1_weight_slice_kernel_grid INPUT RESIDUAL "
                "PRIOR_FMA POST_ADD NONFUSED REVERSE NEGATED");
        }
        ...
        std::fprintf(stderr, "open FF1 weight-grid failed: %s\n", error.what());
        return 1;
    }
}
