#!/usr/bin/env python3
"""Compare auxiliary packages after the shared ambient-input correction.
...
ZIPS = {'P': 'results/fx2_explicit_dictionary_preflight_v1/P-source.zip',
        'D': 'results/fx2_explicit_dictionary_preflight_v1/D-source.zip'}
...
def execute(g):
    ...
    for arm in 'PD':
        ...
        binary = native / 'cmix'
        # Both reference and treatment receive the same explicit-input repair.
        # Old main archives, clean builds and isolated inverses remain required.
...
def main():
    ...
    if validate:
        ...
        return 0
    result = dict(schema='gamma.enwiki9.trim-auxiliary-native.v2', candidate_id=ID,
