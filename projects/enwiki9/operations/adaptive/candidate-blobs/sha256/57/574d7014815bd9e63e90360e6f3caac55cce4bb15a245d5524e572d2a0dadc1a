#ifndef GAMMA_CAUSAL_RELATIONAL_V2_HPP
#define GAMMA_CAUSAL_RELATIONAL_V2_HPP
...
class Model {
 public:
  ...
  bool observe(unsigned truth,Bytes& raw){
    ...
    if(!raw.empty()){
      ...
      if(letters&&!in_tag_&&(fields_.field()==1||fields_.field()==6)){
  ...
  const std::vector<Record>& records(unsigned role)const { require(role<2);return records_[role]; }
  ...
  Bytes state()const{
    Bytes out{'R','G','0','2'};blob(out,inverse_.state());blob(out,fields_.state());
    for(uint64_t v:{modeled_,uint64_t(bit_),uint64_t(byte_),uint64_t(role_),uint64_t(in_tag_),uint64_t(link_),uint64_t(previous_raw_),uint64_t(word_role_),uint64_t(word_overflow_),uint64_t(pending_overflow_),uint64_t(pending_),uint64_t(parent_),uint64_t(relational_),uint64_t(mixed_),active_bits,mentions,pairs})put(out,v);
 ...
 private:
  ...
  void start_mention(bool opportunity=true){
    ...
    role_=!in_tag_&&fields_.field()==6?(link_?1:0):2;
  ...
  Record word_;Bytes pending_tokens_;bool word_overflow_=false,pending_overflow_=false,pending_=false,link_=false,in_tag_=false;
