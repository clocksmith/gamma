...
ID='fx2_explicit_lexical250k_v2'
...
def execute(g):
    ...
    return dict(arms=arms,lexical_comparison=g.artifact(g.result/'lexical-comparison.json'),
        ...
        state_boundary='P/K archive identity; all neural outputs match P across arms and encode/repeat. Decoder neural trace is unsupported and absent; independent raw inverse remains mandatory. Dictionary activation and lookup counters match within arm. Full FXCM model-state serialization is absent; D/S deliberately change lexical model and downstream mixer state.',
...
def main():
    ...
    result=dict(schema='gamma.enwiki9.explicit-lexical-terminal.v2',candidate_id=ID,experiment=g.reference,
