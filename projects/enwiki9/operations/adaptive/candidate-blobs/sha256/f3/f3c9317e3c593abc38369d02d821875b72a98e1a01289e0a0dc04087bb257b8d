...
constexpr std::size_t kOutputs = 1024;
...
void store_bf16_8(std::uint16_t * destination, __m256 values) {
    ...
    for (std::size_t lane = 0; lane < kLanes; ++lane) {
        if (!std::isfinite(lanes[lane])) {
            throw std::runtime_error("non-finite full w_o gradient lane");
        }
...
std::vector<std::uint16_t> project(
    ...
    if (inputs.size() != kSamples * kInputs ||
        ...
        throw std::runtime_error("full w_o gradient operand geometry differs");
    }
...
void write_words(const fs::path & path,
    ...
    output.write(reinterpret_cast<const char *>(words.data()),
                 static_cast<std::streamsize>(words.size() * sizeof(words[0])));
    if (!output) throw std::runtime_error("cannot write full w_o gradient");
}
    ...
    try {
        if (argc != 5) {
            throw std::runtime_error(
                "usage: w_o_gradient_full_post_add INPUT RESIDUAL "
                "TREATMENT NEGATED");
        }
        ...
        write_words(argv[3], project(inputs, residuals, false));
        write_words(argv[4], project(inputs, residuals, true));
        return 0;
    } catch (const std::exception & error) {
        std::fprintf(stderr, "open full w_o gradient failed: %s\n", error.what());
        return 1;
    }
}
