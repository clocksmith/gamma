#!/usr/bin/env python3
"""Bound local causal residuals, then conditionally replay a frozen scalar grid."""
import hashlib
import json
...
def main():
    ...
    if not report['arms'][0]['finite_replay_permitted']:
        report.update(verdict='Aligned positive-amplitude family cannot pay its two-byte header in bounded ideal likelihood.',
                      finite_replay=False)
    else:
        rows = []
