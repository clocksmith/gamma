#!/usr/bin/env python3
"""Transfer the unchanged released expert mixture, retaining its unpaid costs."""
import json
...
RAW = FIXTURE + 'work/native/distant.raw'
STORED = FIXTURE + 'work/native/distant.stored'
REFERENCE = FIXTURE + 'distant/P/archive.bin'
...
def execute(g):
    ...
    require(len(g.buffers[RAW]) == 250000, 'transfer population length differs')
    ...
    return dict(status='passed', arms=arms, g_P=gain,
        ...
        predictive_transfer_pass=gain > 0, package_hold_preserved=True,
        ...
        verdict=('The unchanged released mixture improves this disjoint cold population; retain fixed component costs before considering a separately priced 1MB design.' if gain > 0 else
                 'The unchanged released mixture fails predictive transfer on this disjoint cold population; no scale or parameter rescue is authorized.'))
...
def main():
    ...
    if validate:
        ...
        return 0
    result = dict(schema='gamma.enwiki9.expert-transfer-native.v1', candidate_id=ID,
        experiment=g.reference, raw_bytes=250000, raw_population='[500000000,500250000)',
        initialization='cold', prior_exposure='Previously tested by native weight-representation gates; not used to fit or select this expert mixture. Not globally untouched.',
