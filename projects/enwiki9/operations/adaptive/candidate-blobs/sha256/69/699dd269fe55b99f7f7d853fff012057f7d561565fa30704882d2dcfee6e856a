# Bit-Head FTRL512 PPM0 v4 Contract

## Claim boundary

`cmix_obias_bithead_ftrl512_ppm0_q0_v4` is a prepared, uncompiled,
unexecuted Gamma-authored successor to the sealed Delta-MIDAS v3 candidate.
...
## Motivation and falsifiable difference from v3

v3 estimates one rank-8 local gradient from coded bits 0..255 and applies one
fixed correction during bits 256..511. v4 instead performs strictly causal
follow-the-regularized-leader updates: the adapter used for bit `t` depends only
on coder probabilities, head states, and truths from earlier bits in the same
512-coded-bit segment. A diagonal local Gauss-Newton accumulator determines the
step magnitude. There is no tuned learning rate, PRNG, Gaussian generation, or
coefficient blob.

This is a stronger candidate, not a projected result. It can still fail because
the local tangent omits future recurrent effects and because local statistics
may not transfer even to the next coded bit.
...
K  complete D bookkeeping, zero injection
