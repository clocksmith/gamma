"""V2: the frozen P/E recipe with distinct guarded resident/address budgets.

V1 remains frozen after the sparse mapping admission failure.
"""
...
def run(root, output, experiment_path, closure_path, plan_path):
    ...
    with CgroupMemoryGuard.current(caps["memory_bytes"]) as resident_guard:
        ...
        entry = snapshot / "tools/fx2_entropy_gate_v2.py"
