                                        ...
                                        int negate)
{
    ...
    tensor = nc_new_tensor_2d(device, NC_TYPE_BF16, 1, GAMMA_ELEMENTS);
    if (tensor == NULL || nc_tensor_get_data(&data, tensor) == NULL ||
        data.item_type != NC_TYPE_BF16 || data.item_size != 2 ||
        data.n_dims != 2 || data.dims[0] != 1 ||
        data.dims[1] != GAMMA_ELEMENTS) {
        if (tensor != NULL)
    ...
    for (index = 0; index < GAMMA_ELEMENTS; index++) {
        ...
        uint8_t *destination =
            (uint8_t *)data.data + index * data.strides[1];
        if (negate)
                          ...
                          NCTensor *column_index)
{
    ...
    if (opaque != (void *)(uintptr_t)1 || column_index != NULL ||
        ...
        fprintf(stderr, "unexpected matmul LibNC gradient-merge tensor\n");
        abort();
    }
...
{
    NCTensor *column = nc_reshape_2d(
        nc_dup_tensor(parameter), GAMMA_ELEMENTS, 1);
    return nc_matmul(coefficient, column);
}
...
int main(int argc, char **argv)
{
    ...
    if (argc != 5) {
        fprintf(stderr,
                "usage: gradient_merge_matmul BRANCH DIRECT OUTPUT MODE\n");
        return 1;
    }
    mode = argv[4];
    ...
    if (parameter == NULL || branch == NULL || direct == NULL) {
        fprintf(stderr, "cannot allocate matmul gradient-merge tensors\n");
        return 1;
    }
    ...
    if (loss == NULL) {
        fprintf(stderr, "cannot construct matmul gradient-merge graph\n");
        return 1;
    }
