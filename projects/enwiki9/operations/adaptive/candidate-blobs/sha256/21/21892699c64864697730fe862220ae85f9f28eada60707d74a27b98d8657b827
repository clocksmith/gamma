#!/usr/bin/env python3
"""Native projected output-head learning on frozen transformer features."""
import hashlib
import json
...
ID='fx2_online_head_opening250k_v1'
...
ADAPTER='operations/provenance/fx2_online_head_adapter_v1.json'
...
def execute(g):
    ...
    for arm in 'PKDS':
        for phase in ('encode','decode','repeat'):
            name=arm+'-'+phase
            trace=native/(name+'.coder');state=native/(name+'.state');base=native/(name+'.base')
            ...
            try:
                g.run(name,[str(native/'cmix'),*args,'--transformer','models/6m-q4-fp32.tfwc2'],180,
                      env=dict(GAMMA_FX2_HEAD_ARM=arm,GAMMA_FX2_HEAD_STATE=str(state),GAMMA_FX2_CODER_TRACE=str(trace),GAMMA_FX2_HEAD_BASE=str(base)),work=native)
            finally:cleanup(g,name)
            require(trace.stat().st_size==N*8*28,'coder coordinate differs')
            count=states(state);predictions=base_state(base,state)
            if phase!='encode':
        ...
        arms[arm]=dict(archive=g.artifact(archive),archive_bytes=archive.stat().st_size,
            ...
            state_boundaries=count,transformer_predictions=predictions,exact_inverse=True,archive_repeat_byte_equal=True,
    ...
    for arm in 'KDS':equal(native/'P-encode.base',native/(arm+'-encode.base'))
    ...
    return dict(arms=arms,g_P=gp,g_S=gs,n_1=None,n_2=None,
        ...
        PK_archive_byte_identity=True,KD_adapter_state_byte_identity=True,all_frozen_transformer_feature_and_logit_digests_identical=True,released_parent_archive_and_coder_parity=True,trace_on_off_archive_identity=True,
        changed_state_audit='All39360 adapter weights,192 saved features,205 pending probabilities,five clocks and pending flag at2048 decoded bytes and termination. SHA256 over every frozen hidden feature and post-softcap logit in1024prediction blocks. Native final coder trace complete; downstream parent mixtures may adapt differently and are not required to match across D/S. Full deep/KV state is not serialized.',
...
def main():
    ...
    result=dict(schema='gamma.enwiki9.online-head-native-terminal.v1',candidate_id=ID,experiment=g.reference,
