...
static float gamma_bf16_to_f32(uint16_t word)
{
...
static NCTensor *gamma_read_coefficient(NCDevice *device, const char *path,
                                        int negate)
{
    FILE *input;
    uint16_t *words;
    ...
    tensor = nc_new_tensor_2d(
        device, NC_TYPE_F32, GAMMA_FEATURES, GAMMA_COLUMNS);
    if (tensor == NULL || nc_tensor_get_data(&data, tensor) == NULL ||
        data.item_type != NC_TYPE_F32 || data.item_size != sizeof(float) ||
                          ...
                          NCTensor *column_index)
{
    ...
    if (opaque != (void *)(uintptr_t)1 || column_index != NULL ||
        ...
        data.n_dims != 2 || data.dims[0] != GAMMA_FEATURES ||
        data.dims[1] != GAMMA_COLUMNS) {
        fprintf(stderr, "unexpected converted LibNC gradient-merge tensor\n");
        abort();
    }
...
{
    NCTensor *value = nc_convert(nc_dup_tensor(parameter), NC_TYPE_F32);

    return nc_sum(nc_mul(value, coefficient));
}
...
int main(int argc, char **argv)
{
    ...
    if (argc != 5) {
        fprintf(stderr,
                "usage: gradient_merge_f32_injection BRANCH DIRECT OUTPUT MODE\n");
        return 1;
    }
    mode = argv[4];
    ...
    branch = gamma_read_coefficient(device, argv[1], negate_branch);
    direct = gamma_read_coefficient(device, argv[2], 0);
    if (parameter == NULL || branch == NULL || direct == NULL) {
        fprintf(stderr, "cannot allocate converted gradient-merge tensors\n");
        return 1;
    }
    ...
    if (loss == NULL) {
        fprintf(stderr, "cannot construct converted gradient-merge graph\n");
        return 1;
    }
