#!/usr/bin/env python3
"""Run the generated HARM HORIZON-A field-entry observer fixture."""
...
def run(project_root: Path, output_dir: Path) -> dict[str, object]:
    ...
    if any(output_dir.iterdir()):
        raise FileExistsError("field-entry observer result boundary is not empty")
    ...
    measurements: dict[str, object] = {
        ...
        "fieldEntrySelectionPass": observed["field_entry_selection_pass"],
        "pretruthTransitionPass": observed["pretruth_transition_pass"],
        "hgsContainerBridgePass": observed["hgs_container_bridge"]["pass"],
        ...
        "emittedSeedCount": observed["emitted_seed_count"],
    ...
    source_closure = bool(
        all(
            ...
            for key in (
                ...
                "repeatIdentityPass",
                "fieldEntrySelectionPass",
                "pretruthTransitionPass",
                "hgsContainerBridgePass",
                "productionConfigClosurePass",
            )
        )
        and measurements["negativeControlRejectCount"] == 8
        and measurements["emittedSeedCount"] == 2
