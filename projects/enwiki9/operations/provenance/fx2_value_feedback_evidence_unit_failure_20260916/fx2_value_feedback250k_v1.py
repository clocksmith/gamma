...
from lib.fx2_value_feedback_evidence_v1 import observe_sources, residual_stream
...
def execute(g):
    ...
    for arm in 'PKDS':
        ...
        arms[arm] = dict(archive=g.artifact(native / (arm + '-clean-encode.arc')),
            ...
            residual=g.artifact(native / (arm + '-encode.residual')) if arm != 'P' else None,
    ...
    return dict(arms=arms, g_P=gp, g_S=gs, source_zip_increment_bytes=dz,
        ...
        confirmation_authorized=gp > 0 and gs > 0,
        ...
        scientific_verdict='Positive native parent/control separation authorizes unchanged reserved confirmation with package costs retained.' if gp > 0 and gs > 0 else 'This fixed feedback realization does not pass the native parent/control archive comparison.')
