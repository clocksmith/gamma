...
namespace {
  ...
  bool PretrainPredictor(Predictor* predictor, FILE* dictionary) {
    ...
    if (path != NULL && path[0] != '\0') {
      ...
      if (source == NULL) {
        fprintf(stderr, "\nfailed to open CMIX_PRETRAIN_FILE: %s\n", path);
        return false;
      }
      close_source = true;
    }
    preprocessor::Pretrain(predictor, source);
    if (close_source) {
      fclose(source);
    }
    return true;
  ...
  bool CopyStoredStream(FILE* input, FILE* output,
      unsigned long long* input_bytes, bool dictionary_used) {
    if (input == NULL || output == NULL || input_bytes == NULL ||
        *input_bytes < 5) {
      return false;
    }
    const int expected_header[5] = {
        dictionary_used ? 0x80 : 0, 0, 0, 0, 0};
    for (int i = 0; i < 5; ++i) {
      if (getc(input) != expected_header[i]) return false;
    }
    *input_bytes -= 5;
    for (unsigned long long i = 0; i < *input_bytes; ++i) {
      const int c = getc(input);
      if (c == EOF || putc(c, output) == EOF) return false;
    }
    return true;
  }
}
...
int Help() {
  ...
  printf("    replay stored WRT:  cmix -r [dictionary] [store] [output]\n");
...
bool RunCompression(bool enable_preprocess, bool text_mode,
  ...
  *input_bytes = ftell(data_in);
  ...
  if (replay_stored_stream) {
    if (dictionary == NULL ||
        !CopyStoredStream(data_in, temp_out, input_bytes, true)) {
      fclose(data_in);
      fclose(temp_out);
      remove(temp_path.c_str());
      return false;
    }
  } else if (enable_preprocess) {
    fprintf(stderr, "\rpreprocessing...");
    fflush(stderr);
  ...
  Predictor p(vocab);
  if ((enable_preprocess || replay_stored_stream) &&
      !PretrainPredictor(&p, dictionary)) return false;
...
bool RunDecompression(const std::string& input_path,
  ...
  *input_bytes = data_in.tellg();
  ...
  Predictor p(vocab);
  if ((dictionary_used || HasPretrainFile()) &&
      !PretrainPredictor(&p, dictionary)) return false;
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 4 || argc > 5 || strlen(argv[1]) != 2 || argv[1][0] != '-' ||
      ...
      argv[1][1] != 'n' && argv[1][1] != 't' && argv[1][1] != 'r')) {
    return Help();
  }
  ...
  const bool replay_stored_stream = argv[1][1] == 'r';
  if (argv[1][1] == 'n' || replay_stored_stream) enable_preprocess = false;
  ...
  if (argc == 5) {
    ...
    if (argv[1][1] == 't' || replay_stored_stream) return Help();
  }
  ...
  if (argv[1][1] == 's') {
    if (!Store(input_path, temp_path, output_path, dictionary, &input_bytes,
      ...
      argv[1][1] == 't' || replay_stored_stream) {
    if (!RunCompression(enable_preprocess, text_mode, replay_stored_stream,
        input_path, temp_path, output_path, dictionary, &input_bytes,
        &output_bytes)) {
      return Help();
    }
  } else {
