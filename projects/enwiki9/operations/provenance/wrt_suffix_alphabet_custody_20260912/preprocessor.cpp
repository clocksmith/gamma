...
static constexpr Filetype WRT_TEXT = static_cast<Filetype>(7);
      ...
      if (IsAscii(c)) {
        ...
        if (ascii_run > 500) {
          if (space_count < 5) {
            ...
            return fseek(in, start + ascii_start, SEEK_SET), WRT_TEXT;
          }
        }
      } else {
        ascii_start = -1;
      }
    } else if (type == WRT_TEXT) {
      if (IsAscii(c)) {
        ascii_run -= 2;
...
void EncodeSegment(FILE* in, FILE* out, int n, const std::string& temp_path,
  ...
  while (remainder > 0) {
    ...
    switch (type) {
      case WRT_TEXT: (*block_stats)[0] += len; break;
      case DEFAULT:
  ...
  if ((*block_stats)[0] / n > 0.95) {
    ...
    fprintf(out, "%c%c%c%c%c", WRT_TEXT, n>>24, n>>16, n>>8, n);
...
int DecodeByte(FILE* in, FILE* dictionary) {
  ...
  while (len==0) {
    ...
    len|=getc(in);
    if (len<0) len=1;
    if (type == WRT_TEXT) reset_text_decoder(in, dictionary);
  }
  --len;
  switch (type) {
    case WRT_TEXT: return decode_text(in);
    default: {
