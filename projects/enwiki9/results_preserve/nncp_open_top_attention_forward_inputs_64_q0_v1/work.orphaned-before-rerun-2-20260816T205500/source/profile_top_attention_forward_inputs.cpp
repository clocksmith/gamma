...
public:
    ...
    std::vector<float> multiply_libnc(const std::vector<float> & matrix,
        ...
        if (matrix.size() != static_cast<size_t>(outputs * k) ||
            input.size() != static_cast<size_t>(k * columns) || outputs % 8)
            throw std::runtime_error("LibNC matrix geometry mismatch");
        std::vector<float> result(outputs * columns);
        constexpr int64_t reduction_chunk = 128;
        for (int64_t column = 0; column < columns; column++) {
            const float * source = input.data() + column * k;
            for (int64_t output = 0; output < outputs; output += 8) {
                __m256 accumulated = _mm256_setzero_ps();
                for (int64_t chunk_start = 0; chunk_start < k;
                     chunk_start += reduction_chunk) {
                    const int64_t chunk_end = std::min(
                        chunk_start + reduction_chunk, k);
                    __m256 partial = _mm256_setzero_ps();
                    for (int64_t input_index = chunk_start;
                         input_index < chunk_end; input_index++) {
                        const __m256 weight = _mm256_loadu_ps(
                            matrix.data() + input_index * outputs + output);
                        partial = _mm256_fmadd_ps(
                            _mm256_set1_ps(source[input_index]), weight, partial);
                    }
                    accumulated = _mm256_add_ps(accumulated, partial);
                }
                _mm256_storeu_ps(result.data() + column * outputs + output,
                                 accumulated);
            }
        }
        return result;
    ...
    std::vector<float> multiply_transposed_libnc(
        const std::vector<float> & matrix,
        const std::vector<float> & input,
        int64_t batches, int64_t outputs, int64_t k, int64_t columns) {
        if (matrix.size() != static_cast<size_t>(batches * outputs * k) ||
            input.size() != static_cast<size_t>(batches * columns * k))
            throw std::runtime_error("LibNC transposed matrix geometry mismatch");
        std::vector<float> result(batches * columns * outputs);
        std::vector<float> reordered(outputs * k);
        std::vector<float> source(columns * k);
        for (int64_t batch = 0; batch < batches; batch++) {
            for (int64_t input_index = 0; input_index < k; input_index++)
                for (int64_t output = 0; output < outputs; output++)
                    reordered[input_index * outputs + output] =
                        matrix[(batch * outputs + output) * k + input_index];
            std::copy_n(input.data() + batch * columns * k,
                        columns * k, source.data());
            const std::vector<float> current = multiply_libnc(
                reordered, source, outputs, k, columns);
            std::copy(current.begin(), current.end(),
                      result.begin() + batch * columns * outputs);
        }
        return result;
    }
...
static float libnc_sum_squares64(const float * source) {
    std::array<__m256, 8> x;
    ...
    __m256 high_g = _mm256_permute2f128_ps(g, h, 0x31);
...
static float libnc_sum_squares(const float * source, int width) {
    if (width <= 0 || width % 64)
        throw std::runtime_error("unsupported LibNC squared-sum width");
    std::array<float, 16> partials{};
    int blocks = 0;
    for (int index = 0; index < width; index += 64, blocks++) {
        float block_sum = libnc_sum_squares64(source + index);
        int slot = 0;
        while (blocks & (1 << slot)) {
...
static std::vector<float> rms_norm(const std::vector<float> & input,
    ...
    for (size_t row = 0; row < rows; row++) {
        ...
        const float sum = libnc_sum_squares(source, profile::width);
        ...
        for (int feature = 0; feature < profile::width; feature++)
            output[row * profile::width + feature] =
                bf16(bf16(source[feature] * inverse) * gain[feature] + bias[feature]);
    }
    return output;
}
...
static __m256 libnc_exp8(__m256 value, __m256 maximum) {
    ...
    const __m256i exponent = _mm256_cvtps_epi32(_mm256_mul_ps(log2e, reduced));
    const __m256 exponent_float = _mm256_cvtepi32_ps(exponent);
    reduced = _mm256_fmadd_ps(
        minus_ln2_high, exponent_float, reduced);
    reduced = _mm256_fmadd_ps(ln2_low, exponent_float, reduced);
    __m256 polynomial = _mm256_set1_ps(0.00019841270113829523f);
    polynomial = _mm256_fmadd_ps(polynomial, reduced,
                                 _mm256_set1_ps(0.0013888889225199819f));
    polynomial = _mm256_fmadd_ps(polynomial, reduced,
                                 _mm256_set1_ps(0.008333333767950535f));
    polynomial = _mm256_fmadd_ps(polynomial, reduced,
                                 _mm256_set1_ps(0.0416666679084301f));
    polynomial = _mm256_fmadd_ps(polynomial, reduced,
                                 _mm256_set1_ps(0.1666666716337204f));
    polynomial = _mm256_fmadd_ps(polynomial, reduced,
                                 _mm256_set1_ps(0.5f));
...
static std::vector<float> softmax_rows(std::vector<float> values, int row_width) {
    ...
    for (size_t base = 0; base < values.size(); base += row_width) {
        ...
        for (; index + 64 <= row_width; index += 64, blocks++) {
            std::array<__m256, 8> exponentials;
            for (int lane = 0; lane < 8; lane++) {
                exponentials[lane] = libnc_exp8(
                    _mm256_loadu_ps(values.data() + base + index + lane * 8),
                    maximum8);
                _mm256_storeu_ps(values.data() + base + index + lane * 8,
                                 exponentials[lane]);
            }
            float block_sum = libnc_sum64(exponentials);
            int slot = 0;
            while (blocks & (1 << slot)) {
                block_sum += partials[slot];
                partials[slot++] = 0.0f;
        ...
        if (index < row_width) {
            if (row_width - index != 8)
                throw std::runtime_error("unsupported LibNC softmax tail");
            ...
            float block_sum = libnc_sum8(exponential);
            int slot = 0;
            while (blocks & (1 << slot)) {
                block_sum += partials[slot];
                partials[slot++] = 0.0f;
        ...
        for (int column = 0; column < row_width; column++)
            values[base + column] *= inverse_total;
    }
    return values;
}
...
static void write_tree_path(const fs::path & path,
    ...
    for (int position = 0; position < profile::segment; position++) {
        ...
        while (range > 1) {
            ...
            const float total0 = libnc_sum_values(table + start, range0);
...
int main(int argc, char ** argv) {
    try {
        ...
        for (int layer = 0; layer < profile::layers; layer++) {
            ...
            const std::vector<float> content = engine.multiply_transposed_libnc(
                keys, queries, profile::heads, profile::position,
                profile::key, profile::segment);
            std::vector<float> rounded_content = content;
            round_bf16(rounded_content);
            std::vector<float> raw_relative = engine.multiply_transposed_libnc(
                relative_weight, queries, profile::heads, profile::position,
                profile::key, profile::segment);
            round_bf16(raw_relative);
            ...
            for (int head = 0; head < profile::heads; head++) {
                ...
                for (int q = 0; q < profile::segment; q++) {
                    for (int key = 0; key < profile::position; key++) {
                        ...
                        float score = bf16(
                            bf16(rounded_content[destination] + shifted) * inverse_key_scale);
            ...
            std::vector<float> attended = engine.multiply_transposed_libnc(
                value_matrix, attention, profile::heads, profile::key,
                profile::position, profile::segment);
            round_bf16(attended);
            ...
            for (int position = 0; position < profile::segment; position++)
                for (int index = 0; index < profile::inner; index++)
                    geglu[position * profile::inner + index] = bf16(
                        bf16(libnc_gelu(
                            ff1_output[position * 2 * profile::inner + index])) *
