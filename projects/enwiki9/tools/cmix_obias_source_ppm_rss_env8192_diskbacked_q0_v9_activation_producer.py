...
P=Path(__file__).resolve().parents[1]; C="cmix_obias_source_ppm_rss_env8192_diskbacked_q0_v9"
A=P/f"operations/evidence/{C}-runtime-authority"; RECEIPT=A/"producer-receipt.json"; ACT=P/f"operations/adaptive/activations/{C}.json"; R=P/f"results/{C}"; S=P/f"scratch/{C}"
PARENT=Path("/sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/app.slice"); FUTURE=PARENT/"gamma-cmix-obias-env8192-opening1m-q0-v9"
...
PRODUCER_JOB_ID=C+"-runtime-authority"
...
def ints(p):
 ...
 return out
def producer_job_is_only_running():
 rows=[]
 for p in sorted(RUNNING.glob('*.json')):
  try: rows.append(json.loads(p.read_text()))
  except Exception: return False
 return len(rows)==1 and rows[0].get('job_id')==PRODUCER_JOB_ID and rows[0].get('candidate_id')==C
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--validation-only',action='store_true'); ap.add_argument('--cpu',type=int,required=True); ap.add_argument('--job-id',default=PRODUCER_JOB_ID,choices=[PRODUCER_JOB_ID]); a=ap.parse_args(); allowed=os.sched_getaffinity(0)
 ...
 if blockers or not producer_job_is_only_running() or LEASE.exists() or LOCK.exists(): raise RuntimeError('exclusive admission failed')
 ...
 try:
  ...
  if not producer_job_is_only_running() or R.exists() or S.exists() or FUTURE.exists(): raise RuntimeError('admission changed after lease acquisition')
  child=PARENT/f"gamma-q0-v9-gb-{secrets.token_hex(16)}"; child.mkdir(mode=0o700); inode=child.stat().st_ino; (child/'memory.max').write_text('10000000000'); (child/'memory.swap.max').write_text('0'); memory_max=int((child/'memory.max').read_text()); swap_max=int((child/'memory.swap.max').read_text()); empty=(child/'cgroup.procs').read_text().split()==[]; events_before=ints(child/'memory.events')
  ...
  if child is not None and child.exists():
   if (child/'cgroup.procs').read_text().split() and (child/'cgroup.kill').exists(): (child/'cgroup.kill').write_text('1')
   for _ in range(100):
    if not (child/'cgroup.procs').read_text().split(): break
    time.sleep(.01)
   events_after=ints(child/'memory.events'); final_peak=int((child/'memory.peak').read_text()); empty_cleanup=(child/'cgroup.procs').read_text().split()==[]; same=child.name.startswith('gamma-q0-v9-gb-') and child.stat().st_ino==inode
