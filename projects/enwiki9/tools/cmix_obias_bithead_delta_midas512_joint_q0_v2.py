#!/usr/bin/env python3
"""Read-only joint decision for the frozen C/P/K/O/R/D/S 250KB screen."""
...
def candidate_id(arm: str, ppm_always_purge: bool) -> str:
    if ppm_always_purge:
        return f"cmix_obias_bithead_delta_midas512_ppm0_{arm.lower()}_q0_v3"
...
def main() -> int:
    ...
    controls_live = bool(
        receipts["O"]["encode1"]["max_logit_q20"] > 0
        and receipts["R"]["encode1"]["max_gate_q20"] > 0
        and receipts["D"]["encode1"]["max_gate_q20"] > 0
        and receipts["S"]["encode1"]["max_gate_q20"] > 0
    )
    ...
    result = {
        "schema": (
            "gamma.enwiki9.cmix-obias-bithead-delta-midas512-ppm0-joint-q0-v3"
            if args.ppm_always_purge
            else "gamma.enwiki9.cmix-obias-bithead-delta-midas512-joint-q0-v2"
        ),
        "candidate_id": (
            "cmix_obias_bithead_delta_midas512_ppm0_joint_q0_v3"
            if args.ppm_always_purge
