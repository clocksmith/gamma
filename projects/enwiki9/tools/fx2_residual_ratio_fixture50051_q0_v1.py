...
ID='fx2_residual_ratio_fixture50051_q0_v1'
...
class Codec:
    ...
    def invoke(self,phase,args,traces=True):
        ...
        lines=(g.result/(name+'.stderr')).read_text().splitlines()
        require([x for x in lines if x.startswith('Gamma ratio selected=')]==['Gamma ratio selected='+self.arm],
...
def execute(g):
    ...
    for arm in ARMS:
        result=driver.run(ID,g.work/'prof_input/input',50051,True,run_purpose='diagnostic',
            ...
            run_source='canonical-tool',arm=arm,module=Codec(g,arm),artifact_dir=g.result/arm,package_inventory=(counted,local_package))
