#!/usr/bin/env python3
"""Confirm unchanged native trimming at the ten-million-byte scale."""
import io
import json
...
RAW = 'data/enwik9_10000000.bin'
CAPS = dict(cpus=[2], memory_bytes=9999998976, swap_bytes=0,
            scratch_bytes=16000000000, wall_seconds=24000)
...
def execute(g):
    ...
    require(len(g.buffers[RAW]) == 10000000, 'confirmation length differs')
    ...
    for arm in 'PD':
        ...
        if arm == 'D': equal(native / 'encode.arc', g.work / 'P/encode.arc')
        ...
        arms[arm] = dict(archive=g.artifact(arc), archive_bytes=arc.stat().st_size,
                         ...
                         fresh_parent_archive_byte_equal=True if arm == 'D' else None)
    ...
    return dict(arms=arms, archive_gain_bytes=gain, source_component_gain_bytes=source_gain,
                ...
                verdict='Frozen trimming transfers to this cold opening 10MB with exact native archives and unchanged component savings; full package and full corpus remain unproved.')
...
def main():
    ...
    result = dict(schema='gamma.enwiki9.trim-confirmation.v1', candidate_id=ID, experiment=g.reference,
                  raw_bytes=10000000, raw_population='[0,10000000)', initialization='cold',
                  prior_exposure='Historically exposed opening prefix, not globally unseen. No parameter selection or changes to the frozen trimming.',
