...
def classify_observation(before: dict[str, Any], io: dict[str, Any], after: dict[str, Any], *, prior_successful_sample: bool = False) -> str:
    ...
    if not io.get("ok"):
        if (io.get("errno") == errno.EACCES and prior_successful_sample is True
                and before.get("state") in {"Z", "X", "x"}
                and after.get("ok") and after.get("state") in {"Z", "X", "x"}):
            return "sampled_terminal_io_unavailable"
...
def observe_identity_io(pid: int, successful_identities: dict[tuple[int, int], Any] | None = None) -> dict[str, Any]:
    ...
    prior_success = bool(before.get("ok") and successful_identities is not None
                         and (pid, before["start_ticks"]) in successful_identities)
    return {"pid": pid, "before": before, "io": io, "after": after,
            "prior_successful_identity_sample": prior_success,
            "classification": classify_observation(before, io, after, prior_successful_sample=prior_success)}
...
def lifecycle_validation() -> dict[str, Any]:
    ...
    cases = [(*row, False) for row in cases]
    observed_zombie = {"ok": True, "pid": 3180435, "start_ticks": 165333809, "state": "Z", "minor_faults": 333291, "major_faults": 2946}
    cases.extend([
        ("observed_v14_prior_sampled_zombie_eacces", observed_zombie, denied, observed_zombie, "sampled_terminal_io_unavailable", True),
        ("prior_sampled_dead_x_eacces", {**live, "state": "X"}, denied, {**live, "state": "X"}, "sampled_terminal_io_unavailable", True),
        ("prior_sampled_dead_lower_x_eacces", {**live, "state": "x"}, denied, {**live, "state": "x"}, "sampled_terminal_io_unavailable", True),
        ("never_sampled_zombie_eacces", observed_zombie, denied, observed_zombie, "io_unreadable", False),
        ("prior_sampled_live_eacces", live, denied, live, "io_unreadable", True),
        ("prior_sampled_live_to_zombie_eacces", live, denied, {**live, "state": "Z"}, "io_unreadable", True),
        ("prior_sampled_zombie_to_live_eacces", {**live, "state": "Z"}, denied, live, "io_unreadable", True),
        ("prior_sampled_reused_zombie_eacces", observed_zombie, denied, {**observed_zombie, "start_ticks": 165333810}, "identity_reused", True),
        ("prior_sampled_zombie_unreadable_after", observed_zombie, denied, denied, "io_unreadable", True),
        ("prior_sampled_zombie_disappeared_after", observed_zombie, denied, missing, "io_unreadable", True),
        ("prior_sampled_zombie_eperm", observed_zombie, {"ok": False, "errno": errno.EPERM}, observed_zombie, "io_unreadable", True),
        ("prior_sampled_zombie_malformed_io", observed_zombie, {"ok": False, "errno": None}, observed_zombie, "io_unreadable", True),
    ])
    rows = [{"case": name, "observed": classify_observation(a,b,c,prior_successful_sample=prior), "expected": expected, "prior_successful_sample": prior} for name,a,b,c,expected,prior in cases]
    passed = all(row["observed"] == row["expected"] for row in rows)
    if not passed:
        raise RuntimeError("deterministic lifecycle validation failed")
    return {"schema": "gamma.enwiki9.cmix-zombiefix-q0-v1-lifecycle-validation.v1", "validation_only": True,
...
def run_observed(
    ...
    terminal_io_unavailable: list[dict[str, Any]] = []
        ...
        try:
            while True:
                for pid in descendants(process.pid):
                    observation = observe_identity_io(pid, observations)
                    classification = observation["classification"]
                    if classification == "sampled_terminal_io_unavailable":
                        terminal_io_unavailable.append(observation)
                        continue
    ...
    return {
        ...
        "previously_sampled_terminal_io_unavailable": terminal_io_unavailable,
