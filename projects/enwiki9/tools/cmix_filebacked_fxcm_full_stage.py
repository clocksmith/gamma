#!/usr/bin/env python3
"""Execute one guarded q1 full-corpus encode or decode stage."""
...
SCHEMA = "gamma.enwiki9.cmix-filebacked-fxcm-full-stage.v1"
...
def marker(path: Path, phase: str, event: str, detail: str | None = None) -> None:
    value: dict[str, Any] = {"phase": phase, "event": event}
...
def main() -> int:
    ...
    errors: list[str] = []
    if args.mode == "encode":
        if args.package is None or args.head is None or args.archive is not None:
            raise RuntimeError("encode requires package/head and forbids archive")
        package = regular(args.package, "packaged CMIX binary")
        head = regular(args.head, "head blob")
        inputs = {"package": artifact(package), "head": artifact(head)}
        ...
        if args.archive is None or args.package is not None or args.head is not None:
            raise RuntimeError("decode requires archive and forbids package/head")
    ...
    stage_receipt = {
        ...
        "claim_authority": "single_guarded_full_corpus_stage_only",
