#!/usr/bin/env python3
"""Evaluate the production open final RMSNorm affine and input backward."""
...
def require_inputs(experiment: dict[str, Any]) -> None:
    ...
    bound = (
        ("final-hidden-residual-decision", PARENT_DECISION),
        ("final-hidden-residual-reflection", PARENT_REFLECTION),
        ("promoted-final-hidden-residual", PARENT_RESIDUAL),
    ...
    if not (
        ...
        and decision["measurements"]["openGradientDeterministic"] is True
        ...
        raise ValueError("final RMSNorm backward antecedents are not satisfied")
...
def source_package(path: Path) -> None:
    ...
    if len(compressed) > SOURCE_CEILING:
        raise ValueError("open final RMSNorm backward source closure exceeds ceiling")
...
def main() -> int:
    ...
    if output != RESULT / "decision.json" or output.exists():
        raise ValueError("open final RMSNorm backward result boundary is not fresh")
    ...
    if not (
        len(forward_binaries) == len(hidden_binaries) == len(norm_binaries) == 1
    ):
    ...
    measurements: dict[str, bool | int | float] = {
        ...
        "finalNormGainElementCount": replay_a["normGain"].stat().st_size // 2,
        "finalNormGainMismatchCount": comparisons["normGain"][0],
        "maximumFinalNormGainAbsoluteError": comparisons["normGain"][1],
        "finalNormBiasElementCount": replay_a["normBias"].stat().st_size // 2,
        "finalNormBiasMismatchCount": comparisons["normBias"][0],
        "maximumFinalNormBiasAbsoluteError": comparisons["normBias"][1],
        "finalNormInputResidualElementCount": replay_a["normInputResidual"].stat().st_size // 2,
        "topFeedforwardBiasElementCount": replay_a["topFeedforwardBias"].stat().st_size // 2,
