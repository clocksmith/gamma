...
from tools import opcode_previous_word_bounded_gate_v1 as gate
from tools import record_driver_result as recorder

CID = gate.CID
JID = '20260912T194714Z_32ce808ab6'
OUT = ROOT / 'operations/provenance/opcode_previous_word_bounded_terminal_20260912'
...
def main():
    ...
    assert job["administrative_only"] and job["state"] == "cancelled"
    reconciliation_path = ROOT/job["reconciliation"]["path"]
    reconciliation = read(reconciliation_path)
    assert ref(reconciliation_path) == job["reconciliation"]
    ...
    supplement_path = ROOT/'operations/provenance/opcode_word_bounded_runtime_supplement_v1.json'
    supplement = read(supplement_path)
    for row in supplement['additional_runtime_files']:
        path=Path(row['path'])
    ...
    failures.append(dict(arm='P',phase='repeat',returncode=None,timeout=None,memory_stop=None,classification='host-storage-exhaustion',required_execution_record_missing=True))
    ...
    summary=dict(schema='gamma.enwiki9.opcode-word-confirmation-terminal.v1',candidate_id=CID,job=ref(jobpath),guard=ref(gp),decision_policy=plan['decision_policy'],experiment=ref(ROOT/job['experiment']['path']),
        runtime_supplement=ref(supplement_path),administrative_reconciliation=ref(reconciliation_path),original_returncode=None,execution_end_time=None,resource_evidence_complete=False,observed_resource_failure='host-storage-exhaustion (ENOSPC)',peaks_scope='Last retained nonterminal sample; final peaks unknown',
