...
ID='fx2_weight_adaptive_fixture50051_q0_v2'
SPEC='operations/provenance/fx2_weight_adaptive_fixture50051_v2_inputs.json'
...
def validate(gate):
    ...
    require(parent['validity']['classification']=='implementation-failure' and
            parent['decision']['verdict']=='retry' and
            parent['candidateId']==spec['parent']['candidateId'],'parent does not authorize implementation retry')
    ...
    require(economics['all_production_outputs_exact'] and economics['old_dispatch_negative_reproduced'],
            'production entrypoint controls are missing')
    previous=json.loads(gate.buffers[spec['previous_native_terminal']])['result']
    implementation_delta=economics['raw_source_delta']-previous['raw_source_delta']
    require(implementation_delta==62 and implementation_delta<=gate.contract['budget']['maximumAddedPackageBytes'],
            'dispatch implementation increment exceeds inherited budget')
