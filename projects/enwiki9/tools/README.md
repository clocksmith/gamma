...
Before native work, q3 requires the production attribution, bridge, exact
midpoint patch, and retained F comparator to be their direct prospectively
hash-bound inputs. It also requires retained F to match the path, byte count,
and digest in the legacy attribution receipt. Before reading q2's low-precision
summary, q3 requires its decision, detail, and terminal incomplete-evidence
reflection to match the prospectively hash-bound inputs and requires that
reflection to authorize `retry`; the same check repeats before q2's diagnostic
summary is consumed.
