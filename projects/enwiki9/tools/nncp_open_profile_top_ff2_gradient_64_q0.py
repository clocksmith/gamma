#!/usr/bin/env python3
"""Evaluate the production open top-layer FF2 parameter gradient."""
...
def require_inputs(experiment: dict[str, Any]) -> None:
    ...
    bound = (
        ("final-norm-backward-decision", PARENT_DECISION),
        ("final-norm-backward-reflection", PARENT_REFLECTION),
        ("promoted-final-norm-hidden-residual", PARENT_RESIDUAL),
    ...
    if not (
        ...
        raise ValueError("top-layer FF2 gradient antecedents are not satisfied")
...
def source_package(path: Path) -> None:
    ...
    if len(compressed) > SOURCE_CEILING:
        raise ValueError("open top-layer FF2 gradient source closure exceeds ceiling")
...
def main() -> int:
    ...
    if output != RESULT / "decision.json" or output.exists():
        raise ValueError("open top-layer FF2 gradient result boundary is not fresh")
