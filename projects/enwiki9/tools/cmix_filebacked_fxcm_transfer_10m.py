#!/usr/bin/env python3
"""Run guarded opening/distant 10M parent/q1 identity trajectories."""

from __future__ import annotations

import argparse
import hashlib
import os
from pathlib import Path
from typing import Any

import cmix_filebacked_fxcm_scope_identity as scope


SCHEMA = "gamma.enwiki9.cmix-filebacked-fxcm-transfer-10m.v1"
PRIOR_SCHEMA = "gamma.enwiki9.cmix-filebacked-fxcm-cumulative-identity.v1"
SCOPES = ((0, 10_000_000), (500_000_000, 10_000_000))


def guard_values(arm: dict[str, Any]) -> list[dict[str, Any]]:
    values = []
    for phase in ("encode", "decode"):
        _, value = scope.load_json(Path(arm[f"{phase}_guard"]["path"]), f"{phase} guard")
        values.append(value)
    return values


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--build-receipt", type=Path, required=True)
    parser.add_argument("--program-lock-verification", type=Path, required=True)
    parser.add_argument("--allocator-controls", type=Path, required=True)
    parser.add_argument("--allocator-positive-fixture", type=Path, required=True)
    parser.add_argument("--cumulative-identity-receipt", type=Path, required=True)
    parser.add_argument("--corpus", type=Path, required=True)
    parser.add_argument("--resource-guard", type=Path, required=True)
    parser.add_argument("--result-root", type=Path, required=True)
    parser.add_argument("--scratch-root", type=Path, required=True)
    args = parser.parse_args()

    build_path, build = scope.load_json(args.build_receipt, "scope build receipt")
    lock_path, lock = scope.load_json(
        args.program_lock_verification, "program lock verification"
    )
    controls_path, controls = scope.load_json(args.allocator_controls, "allocator controls")
    positive_path, positive = scope.load_json(
        args.allocator_positive_fixture, "allocator positive fixture"
    )
    prior_path, prior = scope.load_json(
        args.cumulative_identity_receipt, "cumulative identity receipt"
    )
    corpus = scope.existing_regular(args.corpus, "canonical corpus")
    guard_tool = scope.existing_regular(args.resource_guard, "resource guard")
    packages, _ = scope.package_paths(build)
    head_blob = scope.verify_artifact_record(build.get("head_blob"), "head blob")
    if (
        lock.get("schema") != scope.LOCK_SCHEMA
        or lock.get("candidate_id") != scope.CANDIDATE_ID
        or lock.get("verified") is not True
    ):
        raise RuntimeError("program lock verification did not pass")
    if controls.get("schema") != scope.CONTROL_SCHEMA or not scope.controls_pass(controls):
        raise RuntimeError("allocator negative controls did not pass")
    if positive.get("pass") is not True or positive.get("return_code") != 0:
        raise RuntimeError("allocator positive fixture did not pass")
    if prior.get("schema") != PRIOR_SCHEMA or prior.get("terminal_pass") is not True:
        raise RuntimeError("cumulative opening-1M antecedent did not pass")
    if (
        prior.get("candidate_id") != scope.CANDIDATE_ID
        or prior.get("decisions", {}).get("cumulative_1m_identity_pass") is not True
    ):
        raise RuntimeError("cumulative opening-1M antecedent identity mismatch")
    if (
        corpus.stat().st_size != scope.CANONICAL_BYTES
        or scope.sha256_file(corpus) != scope.CANONICAL_SHA256
    ):
        raise RuntimeError("canonical enwik9 identity mismatch")

    result_root, result_fs = scope.absent_root(args.result_root, "10M result root")
    scratch_root, scratch_fs = scope.absent_root(args.scratch_root, "10M scratch root")
    if (
        result_root == scratch_root
        or result_root in scratch_root.parents
        or scratch_root in result_root.parents
    ):
        raise RuntimeError("result and scratch roots must be disjoint")
    result_root.mkdir(mode=0o700)
    scratch_root.mkdir(mode=0o700)
    receipt_path = result_root / "transfer-10m-receipt.json"
    cpu = min(os.sched_getaffinity(0))
    scopes: list[dict[str, Any]] = []
    errors: list[str] = []
    aggregate = {arm: hashlib.sha256() for arm in ("parent", "candidate")}

    try:
        for offset, length in SCOPES:
            for arm in aggregate:
                aggregate[arm].update(offset.to_bytes(8, "little"))
                aggregate[arm].update(length.to_bytes(8, "little"))
            parent_slice, parent = scope.run_arm(
                "parent",
                packages["parent"],
                corpus,
                offset,
                length,
                result_root,
                scratch_root,
                guard_tool,
                head_blob,
                cpu,
                aggregate["parent"],
            )
            candidate_slice, candidate = scope.run_arm(
                "candidate",
                packages["candidate"],
                corpus,
                offset,
                length,
                result_root,
                scratch_root,
                guard_tool,
                head_blob,
                cpu,
                aggregate["candidate"],
            )
            if parent_slice != candidate_slice:
                raise RuntimeError("parent/candidate 10M slice identity mismatch")
            probability_identity = (
                parent["trace"]["integer_probability_stream_sha256"]
                == candidate["trace"]["integer_probability_stream_sha256"]
            )
            trace_identity = (
                parent["trace"]["residual_trace"]["sha256"]
                == candidate["trace"]["residual_trace"]["sha256"]
                and parent["trace"]["byte_trace"]["sha256"]
                == candidate["trace"]["byte_trace"]["sha256"]
            )
            payload_identity = parent["payload"]["sha256"] == candidate["payload"]["sha256"]
            raw_slice_inverse_required = offset == 0
            raw_slice_inverse = all(
                arm["raw_slice_inverse_pass"] for arm in (parent, candidate)
            )
            decoded_identity = (
                parent["restored"]["bytes"] == candidate["restored"]["bytes"]
                and parent["restored"]["sha256"] == candidate["restored"]["sha256"]
            )
            diagnostic_decode_pass = decoded_identity and (
                raw_slice_inverse if raw_slice_inverse_required else True
            )
            scope_pass = (
                probability_identity
                and trace_identity
                and payload_identity
                and diagnostic_decode_pass
            )
            scopes.append(
                {
                    "offset": offset,
                    "bytes": length,
                    "slice_sha256": parent_slice,
                    "arms": [parent, candidate],
                    "integer_probability_identity_pass": probability_identity,
                    "full_trace_identity_pass": trace_identity,
                    "payload_identity_pass": payload_identity,
                    "within_scope_state_trajectory_identity_pass": trace_identity,
                    "raw_slice_inverse_required": raw_slice_inverse_required,
                    "raw_slice_inverse_pass": raw_slice_inverse,
                    "parent_candidate_restored_identity_pass": decoded_identity,
                    "diagnostic_decode_identity_pass": diagnostic_decode_pass,
                    "scope_pass": scope_pass,
                }
            )
            if not scope_pass:
                raise RuntimeError(f"10M scope at offset {offset} did not preserve exact behavior")
        if next(scratch_root.iterdir(), None) is not None:
            raise RuntimeError("10M scratch root is not empty")
        scratch_root.rmdir()
    except Exception as exc:
        errors.append(f"{type(exc).__name__}: {exc}")

    complete = len(scopes) == len(SCOPES)
    probability_pass = complete and all(
        item["integer_probability_identity_pass"] for item in scopes
    )
    trace_pass = complete and all(item["full_trace_identity_pass"] for item in scopes)
    payload_pass = complete and all(item["payload_identity_pass"] for item in scopes)
    opening_inverse_pass = complete and all(
        item["raw_slice_inverse_pass"]
        for item in scopes
        if item["raw_slice_inverse_required"]
    )
    distant_decode_identity_pass = complete and all(
        item["parent_candidate_restored_identity_pass"]
        for item in scopes
        if not item["raw_slice_inverse_required"]
    )
    cleanup_pass = complete and not scratch_root.exists() and all(
        all(arm["backing_cleanup_pass"] for arm in item["arms"]) for item in scopes
    )
    resource_pass = complete and all(
        all(arm["encode_guard_pass"] and arm["decode_guard_pass"] for arm in item["arms"])
        for item in scopes
    )
    terminal_pass = (
        not errors
        and complete
        and probability_pass
        and trace_pass
        and payload_pass
        and opening_inverse_pass
        and distant_decode_identity_pass
        and cleanup_pass
        and resource_pass
    )
    guards = [
        guard
        for item in scopes
        for arm in item["arms"]
        for guard in guard_values(arm)
    ]
    receipt = {
        "schema": SCHEMA,
        "candidate_id": scope.CANDIDATE_ID,
        "authoritative_parent_id": scope.PARENT_ID,
        "population": scope.artifact(corpus),
        "fixed_scopes": [
            {"offset": offset, "bytes": length} for offset, length in SCOPES
        ],
        "trajectory_semantics": {
            "opening": "canonical_prefix_from_byte_zero",
            "distant": "cold_reset_fragment_at_corpus_offset",
            "persistent_byte_zero_state_claimed": False,
        },
        "result_filesystem_type": result_fs,
        "scratch_filesystem_type": scratch_fs,
        "selected_logical_cpu": cpu,
        "runner": scope.artifact(Path(__file__).resolve(strict=True)),
        "scope_primitive": scope.artifact(Path(scope.__file__).resolve(strict=True)),
        "antecedents": {
            "scope_build_receipt": scope.artifact(build_path),
            "program_lock_verification": scope.artifact(lock_path),
            "allocator_negative_controls": scope.artifact(controls_path),
            "allocator_positive_fixture": scope.artifact(positive_path),
            "cumulative_identity_receipt": scope.artifact(prior_path),
        },
        "scopes": scopes,
        "aggregate": {
            "parent_probability_stream_sha256": aggregate["parent"].hexdigest(),
            "candidate_probability_stream_sha256": aggregate["candidate"].hexdigest(),
            "identity_pass": complete
            and aggregate["parent"].digest() == aggregate["candidate"].digest(),
        },
        "resources": {
            "guard_count": len(guards),
            "maximum_tree_rss_kib": max(
                (guard["max_sampled_tree_rss_kib"] for guard in guards), default=0
            ),
            "maximum_temporary_disk_bytes": max(
                (guard["max_sampled_temporary_disk_bytes"] for guard in guards), default=0
            ),
            "maximum_allowed_cpu_count": max(
                (guard["max_sampled_allowed_cpu_count"] for guard in guards), default=0
            ),
            "all_guards_pass": resource_pass,
        },
        "decisions": {
            "all_fixed_scopes_complete": complete,
            "exact_integer_probability_identity_pass": probability_pass,
            "full_trace_identity_pass": trace_pass,
            "payload_identity_pass": payload_pass,
            "opening_both_arms_exact_inverse_pass": opening_inverse_pass,
            "distant_parent_candidate_decode_identity_pass": distant_decode_identity_pass,
            "within_scope_state_trajectory_identity_pass": trace_pass,
            "candidate_backing_cleanup_pass": cleanup_pass,
            "resource_guard_pass": resource_pass,
            "opening_distant_10m_transfer_pass": terminal_pass,
            "full_stream_identity_established": False,
            "persistent_full_corpus_identity_established": False,
            "memory_safe_parent_qualified": False,
            "promotion_authorized": False,
        },
        "errors": errors,
        "terminal_pass": terminal_pass,
        "claim_authority": "opening_and_distant_cold_reset_10m_identity_only",
        "claim_boundary": (
            "Exact opening-prefix and distant cold-reset 10M post-head integer probabilities, "
            "complete finite-coder traces, payloads, decoded-stream identity, guarded resources, "
            "and file-backed cleanup. Raw inversion is required for the opening prefix; the "
            "specialized preprocessor is not a raw inverse for an arbitrary interior fragment. "
            "This does not establish a byte-zero persistent full-corpus trajectory, full-1G "
            "resource eligibility, compression gain, package score, or prize qualification."
        ),
        "execution_authority": False,
        "gamma_compression_credit_bytes": 0,
        "gamma_score_credit_bytes": 0,
    }
    scope.write_new(receipt_path, receipt)
    return 0 if terminal_pass else 1


if __name__ == "__main__":
    raise SystemExit(main())
