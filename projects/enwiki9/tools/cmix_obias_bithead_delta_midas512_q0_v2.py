#!/usr/bin/env python3
"""Build and run one frozen 250KB DELTA-MIDAS v2 diagnostic arm."""
...
PROGRAM = ROOT / "programs" / "cmix_obias_bithead_delta_midas512_q0_v2"
...
def main() -> int:
    ...
    parser.add_argument(
        "--ppm-always-purge",
        action="store_true",
        help="compose the sealed PPM budget-zero residency policy (v3)",
    )
    ...
    if args.ppm_always_purge:
        candidate_id = (
            f"cmix_obias_bithead_delta_midas512_ppm0_{arm.lower()}_q0_v3"
        )
    else:
        candidate_id = f"cmix_obias_bithead_delta_midas512_{arm.lower()}_q0_v2"
    ...
    qm0.DEFINES = (
        ...
        f"-DKH_DELTA_MIDAS_ARM={ARMS[arm]}"
        + (
    ...
    decision.update(
        {
            "schema": (
                "gamma.enwiki9.cmix-obias-bithead-delta-midas512-ppm0-q0-v3"
                if args.ppm_always_purge
                else "gamma.enwiki9.cmix-obias-bithead-delta-midas512-q0-v2"
            ),
