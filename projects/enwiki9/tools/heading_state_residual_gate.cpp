...
constexpr uint32_t kBlendDenominator = 32;
...
int main(int argc, char** argv) {
  if (argc != 5) {
    std::cerr << "usage: heading_state_residual_gate TRACE ARCHIVE MAP DECISION\n";
    return 2;
  }
  try {
    ...
    std::array<uint64_t, 10> state_rows{};
    for (int64_t row = 0; row < rows; ++row) {
      ...
      if (state < 0 || state > 9) throw std::runtime_error("invalid state");
      ++state_rows[state];
      ...
      uint16_t corrected = p1;
      if (support >= kMinimumSupport) {
        const uint32_t auxiliary =
            ((counts.ones + kAlpha) * 65536u) /
            (support + 2 * kAlpha);
        uint32_t q =
            ((kBlendDenominator - 1) * p1 + auxiliary) /
            kBlendDenominator;
        q = std::max<uint32_t>(1, std::min<uint32_t>(65535, q));
        corrected = static_cast<uint16_t>(q);
      }
    ...
    out << "{\n"
        << "  \"id\": \"heading_state_residual_apm_v1\",\n"
        ...
        << "  \"decision\": \""
        << (passes_gate ? "authorize_distant_heading_state_residual"
                        : "retire_heading_state_residual_apm")
        << "\"\n"
        << "}\n";
    out.close();
