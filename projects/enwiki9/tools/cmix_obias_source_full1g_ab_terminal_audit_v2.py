...
CANDIDATE_ID = "cmix_obias_source_full1g_ab_terminal_audit_v2"
...
def latest_tmpfs_resource_evidence() -> tuple[Path, dict[str, object]]:
    matches = sorted(
        (ROOT / "operations" / "evidence").glob(f"*_{B_ID}_tmpfs.json")
    )
    if not matches:
        raise FileNotFoundError("missing Arm B tmpfs-aware resource evidence")
    path = matches[-1]
    return path, json.loads(path.read_text())
...
def main() -> int:
    ...
    tmpfs_evidence_path, tmpfs_resource = latest_tmpfs_resource_evidence()
    required_files = [
        A / "archive9",
        B / "archive9",
        A / "out.cmix",
    ...
    tmpfs_accounting = tmpfs_resource.get("accounting", {})
    scratch_evidence = tmpfs_resource.get("scratch", {})
    resource_failed = bool(
        isinstance(observed_hwm, int) and observed_hwm > RSS_LIMIT_KIB
        or tmpfs_accounting.get("tmpfs_aware_memory_within_limit") is False
    )
    correctness_gates = {
    ...
    resource_gates = {
        ...
        "tmpfs_resource_evidence_present": True,
        "scratch_filesystem_not_memory_backed": (
            scratch_evidence.get("memory_backed") is False
        ),
        "tmpfs_aware_memory_within_limit": (
            tmpfs_accounting.get("tmpfs_aware_memory_within_limit") is True
        ),
    ...
    if correctness_pass and not strict_resource_pass:
        next_action = (
            "Execute the sealed dP probability-adjoint gate alone, then run the "
            "disk-backed PPM0 identity/resource diagnostic alone; only exact "
            "positive evidence may promote to a full correction-only replay."
        )
    ...
    result = {
        "schema": "gamma.enwiki9.cmix-obias-source-full1g-ab-terminal-audit.v2",
        ...
        "resource_evidence": {
            ...
            "tmpfs_aware": {
                "path": str(tmpfs_evidence_path),
                "sha256": sha256(tmpfs_evidence_path),
                "scratch": scratch_evidence,
                "accounting": tmpfs_accounting,
            },
