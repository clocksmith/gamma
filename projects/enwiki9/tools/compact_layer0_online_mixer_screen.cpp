...
constexpr std::uint32_t kLayerEndpointCount = 26;
constexpr std::uint32_t kPairEndpointCount = 2;
constexpr std::uint32_t kEndpointCount = kLayerEndpointCount + kPairEndpointCount;
...
struct Inputs {
  Inputs(const std::string& layer_path, const std::string& pair_path,
         const std::string& base_path, const std::string& store_path)
      : layer(ReadFile(layer_path)), base(ReadFile(base_path)),
        pair(ReadFile(pair_path)), store(ReadFile(store_path)) {
    ...
    if (rows == 0 || layer.size() != 16 + rows * kLayerEndpointCount * 2) {
    ...
    const std::array<unsigned char, 8> pair_magic = {
        'C', 'M', 'X', 'A', 'U', 'X', '1', 0};
    if (pair.size() != 16 + rows * kPairEndpointCount * 2 ||
        ReadU64(pair.data() + 8) != rows ||
        !std::equal(pair_magic.begin(), pair_magic.end(), pair.begin())) {
      throw std::runtime_error("invalid compact/FX2 pair-trace dimensions");
    }
  }
  ...
  std::uint16_t Endpoint(std::uint64_t row, std::uint32_t endpoint) const {
    ...
    if (endpoint < kLayerEndpointCount) {
      p = layer.data() + 16 +
          2 * (row * kLayerEndpointCount + endpoint);
    } else {
      p = pair.data() + 16 +
          2 * (row * kPairEndpointCount + endpoint - kLayerEndpointCount);
    }
    return static_cast<std::uint16_t>(p[0] | (std::uint16_t{p[1]} << 8));
  }
...
Args ParseArgs(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    else if (option == "--pair-trace") args.pair_trace = Value(&index, argc, argv);
  ...
  if (args.layer0_trace.empty() || args.pair_trace.empty() || args.base_p1.empty() ||
...
int main(int argc, char** argv) {
  try {
    ...
    const Inputs input(args.layer0_trace, args.pair_trace, args.base_p1,
                       args.wrt_store);
    const Tables tables;
    ...
    output << "  \"feature_contract\": {\"layer0_endpoint_count\": 26, "
           << "\"free_upstream_endpoints\": [\"compact_post_sse\", "
           << "\"fx2lite428_preblend\"], \"feature_count_with_bias\": "
           << kFeatureCount << "},\n";
    output << "  \"configs\": [\n";
