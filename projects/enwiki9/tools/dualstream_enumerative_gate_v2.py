...
driver.SELF = "tools/dualstream_enumerative_gate_v2.py"
...
def classify_failure(error, last):
    if last and last["timeout"]:
        error = subprocess.TimeoutExpired(last["argv"], 90)
...
def main():
    ...
    try:
        ...
        stage.update(status="failed", correctness_pass=False, native_phases=len(stage["commands"]),
                     failure_class=classify_failure(error, last),
