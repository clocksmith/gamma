#!/usr/bin/env python3
"""Replay the post-update forward in a guarded main-filesystem work tree."""
...
CANDIDATE_ID = "nncp_ggml_postupdate_forward_parity_64_q0_retry_v2"
...
TMPFS_REFLECTION = (
    ROOT / "operations/adaptive/reflections/20260816T010837Z_9afc965d1a.json"
)
EXPORT_RECEIPTS = RESULT / "export-receipts.json"
WORK_ROOT = RESULT / "work"
...
def configure_base() -> tuple[Any, list[dict[str, Any]]]:
    ...
    original_temporary_directory = base.tempfile.TemporaryDirectory

    def guarded_temporary_directory(*args: object, **kwargs: object):
        kwargs["dir"] = WORK_ROOT
        return original_temporary_directory(*args, **kwargs)

    base.tempfile.TemporaryDirectory = guarded_temporary_directory
...
def require_inputs(experiment: dict[str, Any]) -> None:
    ...
    for identifier, path in (
        ...
        ("tmpfs-quota-reflection", TMPFS_REFLECTION),
    ):
    ...
    tmpfs_reflection = json.loads(TMPFS_REFLECTION.read_text())
    if not (
        ...
        and tmpfs_reflection.get("decision", {}).get("verdict") == "retry"
        and tmpfs_reflection.get("attribution", {}).get("failureClass")
...
def main() -> int:
    ...
    measurements: dict[str, bool | int | float] = {
        ...
        "guardedWorkRootPass": not WORK_ROOT.exists(),
