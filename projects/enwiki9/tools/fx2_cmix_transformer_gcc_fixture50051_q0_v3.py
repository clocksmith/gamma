...
ID = "fx2_cmix_transformer_gcc_fixture50051_q0_v3"
...
FAST = "-DSEED=923 -DUPDATE_LIMIT=3000 -m64 -Wall -std=c++17 -include cstdint -ffast-math -fno-exceptions -fno-threadsafe-statics -march=x86-64-v3 -mtune=generic -mrecip=none -fdata-sections -ffunction-sections"
...
def stage() -> int:
    ...
    audit = verify()
    work = RESULT / "work"
    materializer_path = PROJECT / "tools/fx2_cmix_transformer_gcc_fixture50051_q0_v3_materializer.py"
    ...
    spec = importlib.util.spec_from_file_location("public_fx2_v3_materializer", materializer_path)
    ...
    try:
        ...
        mapping_pass = len(payload) >= 10000 and len(vocab) == 205 and payload[0] == 7 and payload[5] == 7 and first_separator_indices == SEPARATOR
...
def main() -> int:
    ...
    expected_cgroup = Path("/sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/app.slice/gamma-fx2-transformer-fixture50051-q0-v3")
