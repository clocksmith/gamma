...
SCHEMA = "gamma.enwiki9.cmix-obias-source-ppm-rss-env8192-opening1m-decision.v7"
...
CGROUP_BASE = Path("/sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/app.slice/gamma-cmix-obias-env8192-opening1m-q0-v7")
...
V3 = load_module(V3_PATH, "cmix_q0_v7_v3_helpers")
BASE = V3.BASE
...
def verify_activation(expected_sha256: str | None, cpu: int) -> dict[str, Any]:
    ...
    if value.get("schema")!="gamma.enwiki9.cmix-obias-opening1m-q0-runtime-authority.v1" or value.get("candidate_id")!=CANDIDATE_ID or value.get("scope_bytes")!=POPULATION_BYTES or value.get("selected_cpu")!=cpu or value.get("terminal_authority") is not True or value.get("runtime_paths")!=expected_paths or value.get("authority_root")!=str(AUTHORITY_ROOT) or value.get("producer")!=producer: raise RuntimeError("q0 activation identity mismatch")
    artifacts=value.get("artifacts"); required={"raw_geekbench5_report","host_fingerprint","lease_transition","lease_terminal","lease_verification","guard_receipt"}
    ...
    terminal=json.loads(paths["lease_terminal"].read_text()); verification=json.loads(paths["lease_verification"].read_text()); guard=json.loads(paths["guard_receipt"].read_text())
    if terminal.get("candidate_id")!=CANDIDATE_ID or terminal.get("result_path")!=str(AUTHORITY_ROOT) or terminal.get("scratch_path")!=str(AUTHORITY_ROOT/"scratch") or terminal.get("signal_authority") is not False: raise RuntimeError("q0 authority lease terminal mismatch")
    ...
    if guard.get("status")!="complete" or guard.get("returncode")!=0 or guard.get("cgroup",{}).get("path")!=str(CGROUP_BASE) or guard.get("cgroup",{}).get("memory_max_bytes")>MEMORY_MAX_BYTES or guard.get("peak_sample",{}).get("allowed_cpu_union")!=[cpu] or guard.get("geekbench5_single_core_score")!=score: raise RuntimeError("q0 authority guard contract mismatch")
...
def verify_source_closure(expected_sha256: str | None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    required = {
        ...
        "python_runtime_closure", "authority_producer", "authority_schema",
...
def preflight(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Path], dict[str, Any] | None]:
    ...
    dependencies.update({
        ...
        "stage_v3": STAGE_PATH.resolve(strict=True), "python_runtime_closure": PYTHON_RUNTIME_CLOSURE.resolve(strict=True), "authority_producer": AUTHORITY_PRODUCER.resolve(strict=True), "authority_schema": AUTHORITY_SCHEMA.resolve(strict=True),
    ...
    report = {
        "schema": "gamma.enwiki9.cmix-obias-opening1m-preflight.v7",
...
def main() -> int:
    ...
    lease_module = load_module(dependencies["managed_lease"], "cmix_q0_v7_outer_lease")
    verifier_module = load_module(dependencies["managed_lease_verifier"], "cmix_q0_v7_lease_verifier")
