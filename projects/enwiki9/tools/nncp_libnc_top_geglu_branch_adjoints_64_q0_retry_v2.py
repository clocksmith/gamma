#!/usr/bin/env python3
"""Correct the sealed GEGLU capture fixture-manifest accounting."""
...
import nncp_libnc_top_geglu_branch_adjoints_64_q0_retry_v1 as parent
...
WORK = RESULT / "work"
SOURCE_ID = "nncp_libnc_top_geglu_branch_adjoints_64_q0_retry_v1"
...
MATERIALIZER = ROOT / (
    "tools/nncp_libnc_top_geglu_branch_adjoints_64_q0_retry_v2_materializer.py"
)
PROGRAM_DESCRIPTOR = PROGRAM / "program.py"
SOURCE_CEILING = 2_000_000
evaluate_predicates = parent.parent.parent.capture.open_parent.evaluate
...
def expected_probe_paths() -> set[str]:
    return {
        f"top_geglu_{branch}_{kind}_s{state:03d}.{extension}"
        for branch in ("gate", "value")
...
def source_package(path: Path) -> None:
    ...
    if path.stat().st_size > SOURCE_CEILING:
        raise ValueError("GEGLU manifest-correction source closure exceeds ceiling")
...
def require_inputs(experiment: dict[str, Any]) -> None:
    ...
    if not (
        ...
        and decision["measurements"]["fixturePayloadMismatchCount"] == 1024
        and decision["measurements"]["gateAdjointMismatchCount"] == 114
        and decision["measurements"]["valueAdjointMismatchCount"] == 0
        ...
        raise ValueError("GEGLU manifest-correction antecedents are not satisfied")
...
def main() -> int:
    ...
    if output != RESULT / "decision.json" or output.exists():
        raise ValueError("GEGLU manifest-correction result boundary is not fresh")
    if not WORK.is_dir() or any(WORK.iterdir()):
        raise ValueError("GEGLU manifest-correction work root is not fresh")
    ...
    for manifest in source_execution["captureManifests"]:
        probe_rows = {
            ...
            if row["path"].startswith("top_geglu_")
        }
        observed_rows = {
            row["path"]: (row["bytes"], row["sha256"])
            for row in manifest["files"]
            if not row["path"].startswith("top_geglu_")
        }
        ...
        json.dumps(
            {
                ...
                "declaredProbeNamespace": "top_geglu_",
            },
            indent=2,
            sort_keys=True,
        )
    ...
    result = {
        ...
        "experiment": {
            key: value
            for key, value in reference(experiment_path, "experiment").items()
            if key != "id"
        },
