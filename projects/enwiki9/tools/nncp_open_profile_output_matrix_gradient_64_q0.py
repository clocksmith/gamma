#!/usr/bin/env python3
"""Evaluate the production open output-matrix gradient boundary."""
...
REDUCER = PROGRAM / "output_matrix_gradient.cpp"
...
def require_inputs(experiment: dict[str, Any]) -> None:
    ...
    if not (
        ...
        raise ValueError("open output-matrix antecedents are not satisfied")
...
def source_package(path: Path) -> None:
    ...
    if len(compressed) > SOURCE_CEILING:
        raise ValueError("open output-matrix source closure exceeds ceiling")
...
def main() -> int:
    ...
    if output != RESULT / "decision.json" or output.exists():
        raise ValueError("open output-matrix result boundary is not fresh")
    ...
    executions["materializeForward"] = execute(
        [
            ...
            str(source / "profile_output_matrix_forward.cpp"),
        ],
        ROOT,
    )
    shutil.copyfile(CMAKE, source / "CMakeLists.txt")
    shutil.copyfile(REDUCER, source / "output_matrix_gradient.cpp")
    ...
    reducer_binaries = [
        path for path in build.rglob("nncp_open_output_matrix_gradient") if path.is_file()
    ]
    ...
    try:
        ...
        def run_population(label: str, root: Path) -> dict[str, Any]:
            ...
            shifted_control = WORK / f"{label}-shifted-embed-out-control.bf16"
    ...
    control_bytes = replay_a["shiftedControl"].read_bytes()
    shifted_differs = control_bytes != replay_a["matrixGradient"].read_bytes()[
        : len(control_bytes)
    ]
