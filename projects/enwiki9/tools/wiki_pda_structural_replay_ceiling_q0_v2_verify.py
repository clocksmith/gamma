#!/usr/bin/env python3
"""Independently rederive a WIKI-PDA v2 causal ceiling decision."""
...
import wiki_pda_structural_replay_ceiling_q0_v2 as contract


PROJECT = Path(__file__).resolve().parents[1]
RESULT = PROJECT / "results/wiki_pda_structural_replay_ceiling_q0_v2"
...
def verify_manifest(manifest: dict[str, Any]) -> dict[str, Path]:
    ...
    for record in manifest["artifacts"]:
        ...
        roles[role] = path
    expected_roles = {
        "lease_evidence", "lease_transitions", "parent_reverification",
        "compile_stdout", "compile_stderr", "scanner_binary",
        "scan_a_receipt", "scan_a_guard", "scan_a_stdout", "scan_a_stderr",
        "scan_b_receipt", "scan_b_guard", "scan_b_stdout", "scan_b_stderr",
        "resource_summary", "decision",
    }
...
def check_command(record: dict[str, Any], argv: list[str], environment: dict[str, str]) -> None:
    expected = {"argv": argv, "cwd": str(PROJECT), "environment": environment}
    if (
        record["argv"] != argv
...
def derive_resource_summary(guards: dict[str, dict[str, Any]], scans: dict[str, dict[str, Any]]) -> dict[str, Any]:
...
def derive_gates(scan: dict[str, Any], measurements: dict[str, Any], resources: dict[str, Any], repeat: bool) -> dict[str, bool]:
    gates = {
        ...
        "target_scale_correct_ceiling_pass": measurements["treatment_correct_bytes"] >= contract.REQUIRED_CORRECT_BYTES,
...
def verify(args: argparse.Namespace) -> tuple[dict[str, Any], bool]:
    ...
    try:
        ...
        require(
            not contract.LEASE.exists() and not contract.LEASE_LOCK.exists()
            and not contract.CGROUP_A.exists() and not contract.CGROUP_B.exists(),
            "execution namespace was not cleaned", checks, "namespace_cleanup_pass",
        )
        decision = load_json(decision_path)
        manifest = load_json(manifest_path)
        contract.validate_with_schema(decision, contract.DECISION_SCHEMA)
        roles = verify_manifest(manifest)
        require(roles["decision"] == decision_path, "manifest does not bind decision", checks, "manifest_decision_binding_pass")
        require(decision["operational_status"] == "terminal" and decision["error"] is None, "decision is not a clean terminal result", checks, "terminal_status_pass")

        plan = load_json(contract.PLAN)
        bindings = contract.validate_plan(plan)
        require(decision["bindings"] == bindings, "decision implementation bindings differ", checks, "source_binding_pass")
        ...
        parent_receipt, parent_verification, reverified_raw = contract.validate_parent(receipt_path, verification_path)
        require(
            decision["parent_qualification"]["receipt"] == parent_receipt
            and decision["parent_qualification"]["verification"] == parent_verification
            and roles["parent_reverification"].read_bytes() == reverified_raw
            and decision["parent_qualification"]["fully_positive"] is True,
            "parent qualification binding mismatch", checks, "parent_v4_qualification_pass",
        )
        require(
            decision["population"] == {"path": str(contract.INPUT), "bytes": contract.INPUT_BYTES, "sha256": contract.INPUT_SHA256, "verified": True},
            "population binding mismatch", checks, "population_binding_pass",
        )
        ...
        check_command(
            compile_record,
            [str(contract.COMPILER), *contract.COMPILE_FLAGS, str(contract.SOURCE), "-o", str(binary)],
        ...
        for arm, cgroup in (("a", contract.CGROUP_A), ("b", contract.CGROUP_B)):
            ...
            check_command(command, contract.scan_command(binary, scan_path, guard_path, cgroup, arm), contract.BASE_ENVIRONMENT)
            if (
                record["receipt"] != contract.artifact(scan_path)
                or record["guard"] != contract.artifact(guard_path)
                or record["sampled_cgroup_file_peak_bytes"] is None
                or record["sampled_cgroup_current_peak_bytes"] is None
                or record["sampled_cgroup_current_peak_bytes"] > contract.CGROUP_LIMIT_BYTES
            ):
                raise RuntimeError(f"scan artifact or resource mismatch: {arm}")
            ...
            expected_child = [str(contract.TASKSET), "--cpu-list", "0", str(binary), str(contract.INPUT), str(scan_path)]
            if (
        ...
        repeat = roles["scan_a_receipt"].read_bytes() == roles["scan_b_receipt"].read_bytes() and scans["a"] == scans["b"]
        require(repeat, "scan repeat identity failed", checks, "repeat_identity_pass")
        resources = derive_resource_summary(guards, decision["scans"])
        ...
        require(
            ...
            "resource summary rederivation mismatch", checks, "resource_rederivation_pass",
        )
        measurements = derive_measurements(scans["a"], resources, repeat)
        gates = derive_gates(scans["a"], measurements, resources, repeat)
        require(decision["measurements"] == measurements and decision["gates"] == gates, "measurement or gate rederivation mismatch", checks, "gate_rederivation_pass")
        passed = gates["all_promotion_predicates_pass"]
        verdict = "authorize_retained_parent_donor_surprise_trace_zero_credit" if passed else "retire_exact_wiki_pda_information_source"
        require(
            decision["scientific_verdict"] == verdict
            ...
            "scientific verdict boundary mismatch", checks, "verdict_boundary_pass",
        )

        lease_args = argparse.Namespace(transition_log=roles["lease_transitions"], terminal_lease=roles["lease_evidence"], output=None)
        lease_verification, lease_pass = managed_exclusive_lease_verify.verify(lease_args)
        require(
            lease_pass and lease_verification["verified"] is True
            and lease_verification["candidate_id"] == contract.CANDIDATE_ID
            and decision["exclusive_lease"]["release_pass"] is True
            and decision["exclusive_lease"]["lease_id"] == lease_verification["lease_id"],
            "managed lease evidence failed", checks, "lease_verification_pass",
        )
    ...
    output = {
        "schema": "gamma.enwiki9.wiki-pda-ceiling-verification.v1",
        "candidate_id": contract.CANDIDATE_ID,
        "verified": verified,
