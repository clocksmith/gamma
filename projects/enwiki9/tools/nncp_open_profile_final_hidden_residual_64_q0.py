#!/usr/bin/env python3
"""Evaluate the production open final-hidden residual projection."""
...
MATRIX_DECISION = ROOT / (
    "results/nncp_open_profile_output_matrix_gradient_64_q0_v1/decision.json"
)
...
REDUCER = PROGRAM / "final_hidden_residual.cpp"
...
def require_inputs(experiment: dict[str, Any]) -> None:
    ...
    bound = (
        ...
        ("output-bias-tail-decision", TAIL_DECISION),
        ("output-bias-tail-reflection", TAIL_REFLECTION),
        ("output-matrix-tail-decision", MATRIX_DECISION),
        ("output-matrix-tail-reflection", MATRIX_REFLECTION),
    )
    ...
    matrix = json.loads(MATRIX_DECISION.read_text())
    reflections = [
        json.loads(PARENT_REFLECTION.read_text()),
        json.loads(JOINT_REFLECTION.read_text()),
        json.loads(Q3_REFLECTION.read_text()),
        json.loads(TAIL_REFLECTION.read_text()),
        json.loads(MATRIX_REFLECTION.read_text()),
    ]
    if not (
        ...
        and tail["measurements"]["outputBiasMismatchCount"] == 0
        and tail["measurements"]["openGradientDeterministic"] is True
        and matrix["promotionPass"] is True
        and matrix["measurements"]["outputMatrixMismatchCount"] == 0
        and matrix["measurements"]["openGradientDeterministic"] is True
        and all(reflection["validity"]["valid"] is True for reflection in reflections)
    ):
        raise ValueError("open final-hidden residual antecedents are not satisfied")


def verify_used_fixture(relative_paths: tuple[str, ...]) -> dict[str, str]:
...
def source_package(path: Path) -> None:
    ...
    if len(compressed) > SOURCE_CEILING:
        raise ValueError("open final-hidden residual source closure exceeds ceiling")
...
def main() -> int:
    ...
    if output != RESULT / "decision.json" or output.exists():
        raise ValueError("open final-hidden residual result boundary is not fresh")
    ...
    executions["materializeForward"] = execute(
        [
            ...
            str(source / "profile_final_hidden_forward.cpp"),
        ],
        ROOT,
    )
    shutil.copyfile(CMAKE, source / "CMakeLists.txt")
    shutil.copyfile(REDUCER, source / "final_hidden_residual.cpp")
    ...
    try:
        ...
        def run_population(label: str, root: Path) -> dict[str, Any]:
            ...
            bias_gradient = WORK / f"{label}-out-bias-gradient.bf16"
            matrix_gradient = WORK / f"{label}-embed-out-gradient.bf16"
            hidden_residual = WORK / f"{label}-final-hidden-residual.bf16"
            norm_bias_gradient = WORK / f"{label}-final-norm-bias-gradient.bf16"
            shifted_control = WORK / f"{label}-shifted-final-norm-bias.bf16"
            receipts["reducer"] = execute(
                [
                    str(reducer_binary),
                    str(Q3_FIXTURE / "state_initial.params"),
                    str(Q3_FIXTURE / "parameters_initial.coefs"),
                    str(root),
                    str(bias_gradient),
                    str(matrix_gradient),
                    str(hidden_residual),
                    str(norm_bias_gradient),
                    str(shifted_control),
                ],
                WORK,
                environment,
            ...
            return {
                ...
                "hiddenResidual": hidden_residual,
                "normBiasGradient": norm_bias_gradient,
                "shiftedControl": shifted_control,
    ...
    open_bias_gradient = RESULT / "open-out-bias-gradient.bf16"
    open_matrix_gradient = RESULT / "open-embed-out-gradient.bf16"
    open_hidden_residual = RESULT / "open-final-hidden-residual.bf16"
    open_norm_bias_gradient = RESULT / "open-final-norm-bias-gradient.bf16"
    shutil.copyfile(replay_a["biasGradient"], open_bias_gradient)
    shutil.copyfile(replay_a["matrixGradient"], open_matrix_gradient)
    shutil.copyfile(replay_a["hiddenResidual"], open_hidden_residual)
    shutil.copyfile(replay_a["normBiasGradient"], open_norm_bias_gradient)
    used_fixture.update(
        verify_used_fixture(
            (
    ...
    bias_mismatch_count, maximum_bias_error = compare_bf16(
        open_bias_gradient, Q3_FIXTURE / "gradients/0245_out_bias.bin"
    )
    matrix_mismatch_count, maximum_matrix_error = compare_bf16(
        open_matrix_gradient, Q3_FIXTURE / "gradients/0000_embed_out.bin"
    )
    norm_bias_mismatch_count, maximum_norm_bias_error = compare_bf16(
        open_norm_bias_gradient, Q3_FIXTURE / "gradients/0244_ln_b_40.bin"
    )
    deterministic = (
        replay_a["aggregate"] == replay_b["aggregate"]
        and replay_a["biasGradient"].read_bytes()
        == replay_b["biasGradient"].read_bytes()
        and replay_a["matrixGradient"].read_bytes()
        == replay_b["matrixGradient"].read_bytes()
        and replay_a["hiddenResidual"].read_bytes()
        == replay_b["hiddenResidual"].read_bytes()
        and replay_a["normBiasGradient"].read_bytes()
        == replay_b["normBiasGradient"].read_bytes()
        and replay_a["shiftedControl"].read_bytes()
        == replay_b["shiftedControl"].read_bytes()
    ...
    shifted_differs = (
        replay_a["shiftedControl"].read_bytes()
        != replay_a["normBiasGradient"].read_bytes()
    )
    ...
    measurements: dict[str, bool | int | float] = {
        ...
        "outputBiasElementCount": open_bias_gradient.stat().st_size // 2,
        "outputBiasMismatchCount": bias_mismatch_count,
        "maximumOutputBiasAbsoluteError": maximum_bias_error,
        "outputMatrixElementCount": open_matrix_gradient.stat().st_size // 2,
        "outputMatrixMismatchCount": matrix_mismatch_count,
        "maximumOutputMatrixAbsoluteError": maximum_matrix_error,
        "finalHiddenResidualElementCount": open_hidden_residual.stat().st_size // 2,
        "finalNormBiasElementCount": open_norm_bias_gradient.stat().st_size // 2,
        "finalNormBiasMismatchCount": norm_bias_mismatch_count,
        "maximumFinalNormBiasAbsoluteError": maximum_norm_bias_error,
    ...
    result = {
        ...
        "artifacts": [
            ...
            reference(open_bias_gradient, "open-output-bias-gradient"),
            reference(open_matrix_gradient, "open-output-matrix-gradient"),
            reference(open_hidden_residual, "open-final-hidden-residual"),
            reference(open_norm_bias_gradient, "open-final-norm-bias-gradient"),
