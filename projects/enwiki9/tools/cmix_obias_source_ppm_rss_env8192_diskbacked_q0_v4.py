...
CANDIDATE_ID = "cmix_obias_source_ppm_rss_env8192_diskbacked_q0_v4"
SCHEMA = "gamma.enwiki9.cmix-obias-source-ppm-rss-env8192-opening1m-decision.v4"
...
AUTH_RUNTIME_VERIFY_PATH = PROJECT / "tools/cmix_filebacked_fxcm_runtime_qualification_verify.py"
...
V3 = load_module(V3_PATH, "cmix_q0_v4_v3_helpers")
BASE = V3.BASE
AUTH_RUNTIME = load_module(AUTH_RUNTIME_VERIFY_PATH, "cmix_q0_v4_runtime_authority")
...
def verify_activation(expected_sha256: str | None, cpu: int) -> dict[str, Any]:
    ...
    if expected_sha256 is None or receipt_record["sha256"] != expected_sha256:
        raise RuntimeError("future adaptive job does not bind the exact activation SHA-256")
    value = json.loads(ACTIVATION_RECEIPT.read_text())
    if (
        value.get("schema") != ACTIVATION_SCHEMA
        or value.get("candidate_id") != CANDIDATE_ID
        or value.get("scope_bytes") != POPULATION_BYTES
        or value.get("selected_cpu") != cpu
        or value.get("terminal_authority") is not True
    ):
        raise RuntimeError("runtime activation identity or selected CPU mismatch")
    artifacts = value.get("artifacts")
    required = {
        "geekbench5_report", "host_fingerprint", "runtime_qualification",
        "runtime_qualification_verification", "managed_lease_verification",
    }
    if not isinstance(artifacts, dict) or set(artifacts) != required:
        raise RuntimeError("runtime activation artifact closure is incomplete")
    paths = {name: verify_record(record, name)[0] for name, record in artifacts.items()}
    report_score = AUTH_RUNTIME.parse_geekbench5_score(paths["geekbench5_report"])
    host_value = json.loads(paths["host_fingerprint"].read_text())
    if host_value != AUTH_RUNTIME.current_host_fingerprint():
        raise RuntimeError("authoritative current-host fingerprint mismatch")
    source = json.loads(paths["runtime_qualification"].read_text())
    verification = json.loads(paths["runtime_qualification_verification"].read_text())
    if (
        source.get("schema") != AUTH_RUNTIME.SOURCE_SCHEMA_ID
        or source.get("contract_revision") != 2
        or source.get("benchmark", {}).get("raw_report") != artifacts["geekbench5_report"]
        or source.get("benchmark", {}).get("host_fingerprint") != artifacts["host_fingerprint"]
        or source.get("benchmark", {}).get("single_core_score") != report_score
        or verification.get("schema") != AUTH_RUNTIME.OUTPUT_SCHEMA_ID
        or verification.get("source_receipt") != artifacts["runtime_qualification"]
        ...
        or not all(check is True for check in verification["checks"].values())
        or verification.get("derived", {}).get("geekbench5_single_core_score") != report_score
    ):
        raise RuntimeError("independent Geekbench/runtime provenance verification failed")
    lease_value = BASE.managed_lease_verification(paths["managed_lease_verification"])
    return {
        "receipt": receipt_record,
        "value": value,
        "score": report_score,
        "host_fingerprint": artifacts["host_fingerprint"],
        "runtime_verification": artifacts["runtime_qualification_verification"],
        "managed_lease_verification": lease_value,
    }
...
def verify_source_closure(expected_sha256: str | None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    required = {
        ...
        "runtime_authority_verifier", "runtime_host_schema", "runtime_verification_schema",
...
def preflight(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Path], dict[str, Any] | None]:
    ...
    dependencies.update({
        ...
        "stage_v3": STAGE_PATH.resolve(strict=True), "runtime_authority_verifier": AUTH_RUNTIME_VERIFY_PATH.resolve(strict=True),
    ...
    if not SOURCE_CLOSURE.is_file():
        blockers.append("sealed v4 source closure is missing")
    else:
        try:
    ...
    if LEASE_PATH.exists() or LEASE_PATH.is_symlink() or LEASE_LOCK_PATH.exists() or LEASE_LOCK_PATH.is_symlink():
        blockers.append("pinned canonical managed exclusive lease namespace is occupied")
    report = {
        "schema": "gamma.enwiki9.cmix-obias-opening1m-preflight.v4",
...
def main() -> int:
    ...
    lease_module = load_module(dependencies["managed_lease"], "cmix_q0_v4_outer_lease")
    verifier_module = load_module(dependencies["managed_lease_verifier"], "cmix_q0_v4_lease_verifier")
