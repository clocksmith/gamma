...
ARMS = ("C", "P", "K", "O", "R", "D", "S")
RECEIPT = re.compile(
    r"KH_BITHEAD_FTRL_RECEIPT arm=(?P<arm>[PKORDS]) bits=(?P<bits>[0-9]+) "
...
def candidate_id(arm: str, ppm_always_purge: bool) -> str:
    if ppm_always_purge:
        return f"cmix_obias_bithead_ftrl512_ppm0_disk_{arm.lower()}_q0_v5"
    return f"cmix_obias_bithead_ftrl512_{arm.lower()}_q0_v2"
...
def main() -> int:
    ...
    parser.add_argument("--ppm-always-purge", action="store_true", required=True)
    args = parser.parse_args()
    if args.ppm_always_purge:
        result = ROOT / "results" / "cmix_obias_bithead_ftrl512_ppm0_joint_q0_v3"
    else:
        result = ROOT / "results" / "cmix_obias_bithead_ftrl512_joint_q0_v2"
    if result.exists():
    ...
    for arm in ARMS:
        ...
        receipt_sync[arm] = bool(
            complete
            and all(rows[stage]["finite"] for stage in rows)
            ...
            and rows["encode1"]["adapter_hash"]
            == rows["encode2"]["adapter_hash"]
            and rows["encode1"]["adapter_hash"]
            == rows["bare_decode"]["adapter_hash"]
            and rows["encode1"]["max_gate_q20"]
            == rows["encode2"]["max_gate_q20"]
            == rows["bare_decode"]["max_gate_q20"]
            and rows["encode1"]["max_logit_q20"]
            == rows["encode2"]["max_logit_q20"]
            == rows["bare_decode"]["max_logit_q20"]
        )

    clean_parent_identity = payload_identity(decisions["C"], decisions["P"])
    p_k_identity = bool(
        receipt_sync.get("P")
        ...
        and payload_identity(decisions["P"], decisions["K"])
    )
    finite = all(
        receipts[arm][stage] is not None
        and receipts[arm][stage]["finite"]
        for arm in ARMS
        if arm != "C"
        for stage in ("encode1", "encode2", "bare_decode")
    )
    k_shadow_live = bool(
        receipts["K"]["encode1"] is not None
        and receipts["K"]["encode1"]["max_gate_q20"] > 0
        and receipts["K"]["encode1"]["max_logit_q20"] > 0
    )
    control_fields = {
        "O": "max_logit_q20",
        "R": "max_gate_q20",
    ...
    gates = {
        ...
        "k_full_shadow_arithmetic_live": k_shadow_live,
    ...
    result = {
        "schema": (
            "gamma.enwiki9.cmix-obias-bithead-ftrl512-ppm0-joint-disk-q0-v7"
            if args.ppm_always_purge
            else "gamma.enwiki9.cmix-obias-bithead-ftrl512-joint-q0-v2"
        ),
        "candidate_id": (
            "cmix_obias_bithead_ftrl512_ppm0_joint_disk_q0_v7"
            if args.ppm_always_purge
            else "cmix_obias_bithead_ftrl512_joint_q0_v2"
        ),
