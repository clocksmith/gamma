#!/usr/bin/env python3
"""Seal the exact-10M v12 FX2-lite PPMD context-restore crash."""
...
if __package__:
    ...
    from .seal_cmix21_lstm200_fx2lite428_allocator_failure import (
...
EXPECTED_FAILURE_FUNCTION = "Fx2LitePPMD::ppmd_Model::RestoreModelRare"
EXPECTED_CALLER_FUNCTION = "Fx2LitePPMD::ppmd_Model::ppmd_UpdateByte"


def require_restore_mapping(source_mapping: str) -> None:
    required = (EXPECTED_FAILURE_FUNCTION, EXPECTED_CALLER_FUNCTION)
    missing = [function for function in required if function not in source_mapping]
    if missing:
        raise RuntimeError(f"fault mapping misses context-restore chain: {missing}")
...
def main() -> int:
    ...
    if recovery.get("schema") != "cmix21_lstm200_fx2lite428_allocator_recovery_v1":
    ...
    if recovery.get("decision", {}).get("exact_10m_confirmation_authorized") is not True:
        raise RuntimeError("allocator recovery did not authorize exact 10M")
    ...
    require_restore_mapping(source_mapping)

    receipt = {
        "schema": "cmix21_lstm200_fx2lite428_context_restore_failure_v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "evidence_level": "exact_guarded_10m_terminal_symbolized_context_restore_failure",
        ...
        "diagnosis": {
            ...
            "root_boundary": (
                "the selective allocator checks moved the failure beyond the old "
                "free-list boundary, but RestoreModelRare still followed an invalid "
                "FX2-lite PPMD suffix/context link while walking MaxContext to root"
            ),
            "existing_proven_fix_source": (
                "the primary CMIX21 PPMD implementation in the same candidate tree "
                "already bounds RestoreModelRare suffix walks and resets on invalid "
                "context links"
            ),
        },
        "proof": {
        ...
        "decision": {
            ...
            "selective_context_restore_replay_authorized": True,
            "verdict": "allocatorguard_v12_rejected_at_context_restore_boundary",
            "next_action": (
                "port only bounded context/suffix validation into FX2-lite "
                "RestoreModelRare, require exact 1M archive identity, roundtrip, and "
                "determinism, then reconsider exact 10M"
            ),
        },
        "claim_boundary": (
            "No completed exact-10M archive or compression score exists. This "
            "receipt localizes a codec-correctness failure and cannot promote the "
            "forecast or support a 10.95 percent claim."
        ),
    }
