...
class SourcePopulation {
 public:
  void inject(const std::vector<uint64_t>& pointers) {
    ...
    for (uint64_t pointer : pointers) states_.push_back(State{pointer, each, 0});
    merge_and_cap();
  }
  ...
  void observe(int truth, const std::vector<uint8_t>& stream) {
    ...
    for (auto& state : states_) {
      ++state.pointer_bit;
      if (state.age != std::numeric_limits<uint32_t>::max()) ++state.age;
    }
    merge_and_cap();
  }
  ...
  int dominant_age_bucket() const {
    if (states_.empty()) return 0;
    const State* dominant = &states_.front();
    for (const auto& state : states_) {
      if (state.weight > dominant->weight) dominant = &state;
    }
    uint32_t value = dominant->age + 1;
    int bucket = 0;
    while (value > 1 && bucket < 11) {
      value >>= 1;
      ++bucket;
    }
    return bucket;
 ...
 private:
  ...
  void merge_and_cap() {
    ...
    for (const auto& state : states_) {
      if (!merged.empty() && merged.back().pointer_bit == state.pointer_bit) {
        merged.back().weight += state.weight;
        merged.back().age = std::max(merged.back().age, state.age);
      }
...
struct Hazard {
  ...
  void observe(bool prediction_correct) {
    if (prediction_correct) ++correct;
    else ++wrong;
    if (correct + wrong >= (1u << 20)) {
      correct = std::max<uint64_t>(1, (correct + 1) / 2);
      wrong = std::max<uint64_t>(1, (wrong + 1) / 2);
    }
  }
};
...
struct Arm {
  ...
  std::array<Mixer, 12 * 6 * 4 * 8> mixers{};
  std::array<Hazard, 12 * 6 * 4 * 8> hazards{};
  ...
  void bit(uint64_t current_bit, int truth, uint16_t p1, const std::vector<uint8_t>& stream, double base_cost, int third) {
    ...
    ++active_bits;
    const double vote_p1 = population.probability_one(stream);
    const bool majority_one = vote_p1 >= 0.5;
    const double agreement = std::abs(vote_p1 - 0.5) * 2.0;
    int agreement_bucket = 0;
    if (agreement >= 0.5) agreement_bucket = 1;
    if (agreement >= 0.75) agreement_bucket = 2;
    if (agreement >= 0.9375) agreement_bucket = 3;
    const int bucket =
        (((population.dominant_age_bucket() * 6 +
           population.support_bucket()) *
              4 +
          agreement_bucket) *
             8 +
         static_cast<int>(current_bit & 7));
    Hazard& hazard = hazards[bucket];
    const double reliability = hazard.reliability();
    const double echo_p1 = majority_one ? reliability : 1.0 - reliability;
    ...
    const double raw_echo_truth = truth ? vote_p1 : 1.0 - vote_p1;
    const double echo_cost =
        raw_echo_truth > 0.0
            ? -std::log2(raw_echo_truth)
            : std::numeric_limits<double>::infinity();
    oracle_bits += std::min(base_cost, echo_cost);
    mixer.observe(base_truth, echo_truth);
    hazard.observe(majority_one == static_cast<bool>(truth));
...
int main(int argc, char** argv) {
  try {
    ...
    Arm sigma{"SIGMA_HAZARD"}, recency{"RECENCY"}, shuffled{"SHUFFLED"}, byte8{"BYTE8"};
