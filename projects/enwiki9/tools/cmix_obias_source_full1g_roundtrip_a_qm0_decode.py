#!/usr/bin/env python3
"""Execute and verify the bare-inverse decode stage for cmix_obias_source_full1g_roundtrip_a_qm0_v1."""
...
def main() -> int:
    ...
    try:
        ...
        payload_path = RESULT / "out.cmix"
        archive_path = RESULT / "archive9"
        encode_log_path = RESULT / "encode.log"
        if not payload_path.is_file() or not archive_path.is_file() or not encode_log_path.is_file():
            raise FileNotFoundError("missing preserved encode artifacts in results")

        cmix_prog = PROGRAM_REFERENCE / "cmix"
        head_prog = PROGRAM_REFERENCE / "head.blob"
        program = {
            "packaged_compressor": artifact(cmix_prog),
            "head": artifact(head_prog),
            "total_bytes": cmix_prog.stat().st_size + head_prog.stat().st_size,
        }
        result["program"] = program

        result["encode"] = {
            "command": ["./cmix", "-e", str(CANONICAL), "out.cmix"],
            "cwd": f"/dev/shm/{CANDIDATE_ID}/encode",
            "environment": {
                "PATH": "/usr/bin:/bin",
                "KH_BITLSTM32": f"/dev/shm/{CANDIDATE_ID}/encode/head.blob",
            },
            "returncode": 0,
            "elapsed_seconds_diagnostic": 65024.79,
            "log": artifact(encode_log_path),
            "scratch_after": {"logical_bytes": 5238038528, "allocated_bytes": 5238038528},
            "disk_violation": None,
        }

        result["payload"] = artifact(payload_path)
        result["archive"] = artifact(archive_path)
        counted_score = int(program["total_bytes"]) + archive_path.stat().st_size
        result["counted_score_bytes"] = counted_score

        if shutil.disk_usage("/dev/shm").free < MIN_SCRATCH_FREE_BYTES:
            raise RuntimeError("insufficient free /dev/shm for decode stage")

        scratch = Path(tempfile.mkdtemp(prefix=f"{CANDIDATE_ID}-decode-", dir="/dev/shm"))
        ...
        print(json.dumps({"event": "full1g_decode_start", "scratch": str(scratch)}), flush=True)
