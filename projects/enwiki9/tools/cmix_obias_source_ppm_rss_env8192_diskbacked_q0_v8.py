...
CANDIDATE_ID = "cmix_obias_source_ppm_rss_env8192_diskbacked_q0_v8"
SCHEMA = "gamma.enwiki9.cmix-obias-source-ppm-rss-env8192-opening1m-decision.v8"
...
CGROUP_BASE = Path("/sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/app.slice/gamma-cmix-obias-env8192-opening1m-q0-v8")
...
STDLIB_RESOURCE_GUARD = PROJECT / "tools/run_with_resource_guard_q0_v8.py"
...
V3 = load_module(V3_PATH, "cmix_q0_v8_v3_helpers")
BASE = V3.BASE
...
def verify_activation(expected_sha256: str | None, cpu: int) -> dict[str, Any]:
    ...
    verifier=load_module((PROJECT/"tools/managed_exclusive_lease_verify.py").resolve(strict=True),"q0_v8_activation_lease_verify"); replay,verified=verifier.verify(argparse.Namespace(transition_log=paths["lease_transition"],terminal_lease=paths["lease_terminal"],output=None))
...
def preflight(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Path], dict[str, Any] | None]:
    ...
    report = {
        "schema": "gamma.enwiki9.cmix-obias-opening1m-preflight.v8",
...
def main() -> int:
    ...
    lease_module = load_module(dependencies["managed_lease"], "cmix_q0_v8_outer_lease")
    verifier_module = load_module(dependencies["managed_lease_verifier"], "cmix_q0_v8_lease_verifier")
