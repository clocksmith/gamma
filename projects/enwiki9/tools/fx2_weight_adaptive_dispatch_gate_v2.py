...
PLAN = 'operations/provenance/fx2_weight_adaptive_dispatch_gate_v2_plan.json'
...
def run(plan, inputs, output):
    ...
    for arm in ('P', 'D'):
        ...
        names = ('weights_io', 'weights_io_compressed', 'qmat_dense', 'qmat_sparse',
                 'attn', 'kda', 'glue', 'arena_build', 'model_opt')
        invoke('objects-'+arm, ['/usr/bin/make', '-j1', *['tf_'+n+'.o' for n in names],
                               'CC=/usr/bin/g++'], tree)
        ...
        probes[arm] = probe
        # Upstream cmix removes objects after linking. Link the probe first,
        # then let the unmodified production recipe consume those same objects.
