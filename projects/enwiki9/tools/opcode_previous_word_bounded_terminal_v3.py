...
from tools import opcode_previous_word_bounded_gate_v3 as gate
from tools import record_driver_result as recorder

CID = gate.CID
JID = '20260912T220257Z_b591d391ac'
OUT = ROOT / 'operations/provenance/opcode_previous_word_confirmation_v4_terminal_20260912'
...
def main():
    ...
    guard_schema = recorder._validate_guard_receipt(gp, job, guard)
    assert guard['returncode'] is not None
    ...
    for arm in ['P','K','D','S','release-D']:
        ...
        rows[arm]=dict(archive_bytes=size,exact_inverse=inverse,repeat_byte_identity=repeat,state_synchronization=state,observed_plain_identity=plain if a=='D' else 'not-required',artifacts=artifacts,audits={m:ref(where/(a+'-'+m+'.audit.json')) for m in observed},phase_resources=resources)
        audits[arm]=observed
    ...
    resource_complete=all(guard['measurements'].values())
    resource=False if resource_failure else True if resource_complete else None
    ...
    index=dict(schema='gamma.enwiki9.terminal-result-index.v1',job=ref(jobpath),guard=ref(gp),arms=[],evidence=[ref(terminal),plan['decision_policy']])
    for arm,row in rows.items():
        ...
        result=dict(schema='gamma.enwiki9.driver-result.v2',arm=arm,program_id=CID,program_name='Cold 1MB previous-word '+arm,
            ...
            closed_guard=ref(gp),source_terminal=ref(terminal),audit_files=row['audits'],compressed_size=size,compressed_sha256=arts.get('archive',{}).get('sha256'),data_size=1000000,data_path=plan['input']['path'],data_sha256=plan['input']['sha256'],data_md5=hashlib.md5(raw).hexdigest(),
