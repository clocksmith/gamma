#!/usr/bin/env python3
"""Native same-capacity attention retention comparison with fixed paid weights."""
import io
import json
...
from lib.fx2_attention_anchor_v1 import materialize, SOURCE, HEADER
from tools.fx2_expert_release250k_v3 import source_zip, equal, cleanup

ID='fx2_attention_anchor250k_v2'
...
def verify_compile_mode(g, arm, label):
    lines=(g.result/(label+'.stdout')).read_text().splitlines()
    commands=[shlex.split(line) for line in lines if ' -c cpp_infer/src/opt/model_opt.cpp ' in line]
    require(len(commands)==1,'attention compilation command absent or ambiguous')
    flags=[x for x in commands[0] if x.startswith('-DGAMMA_ANCHOR_MODE')]
    expected=[] if arm=='P' else ['-DGAMMA_ANCHOR_MODE='+str({'K':0,'D':1,'S':2}[arm])]
    require(flags==expected,'attention compilation mode differs')
...
def execute(g):
    ...
    child=materialize(members,g.buffers['lib/fx2_attention_anchor_v2.h'])
    require(set(child)-set(members)=={HEADER} and {n for n in members if members[n]!=child[n]}=={SOURCE},'changed member set')
    package=json.loads(g.buffers[RELEASE+'package.json'])
    arms={};deliveries={}
    for arm,source in [('P',members),('K',child),('D',child),('S',child)]:
        ...
        verify_compile_mode(g,arm,arm+'-compile')
        ...
        verify_compile_mode(g,arm,arm+'-compile-repeat')
    ...
    comparison={a:neural_compare(native/'P-encode.probs',native/(a+'-encode.probs'),json.loads(g.buffers[VALIDITY])['missing_feature_byte_coordinates']) for a in 'KDS'}
    require(comparison['K']['changed_rows']==0,'bookkeeping changes neural probabilities')
    ...
    required_option=plan['compile_commands']['D'][-1]
    require(required_option.startswith('CPPFLAGS_TRANSFORMER=') and plan['compile_commands']['D'][:-1]==plan['compile_commands']['P'],'build option accounting differs')
    option=len(required_option.encode('ascii'))
    ...
    return dict(arms=arms,neural_comparison=g.artifact(g.result/'neural-comparison.json'),
        ...
        changed_source_members=[SOURCE,HEADER],confirmation_authorized=confirm,
        ...
        scientific_verdict='Native archive, control and source-component gains authorize unchanged independent confirmation.' if confirm else 'This fixed four-anchor retention does not pass every archive/control/source-component requirement.')
...
def main():
    ...
    result=dict(schema='gamma.enwiki9.attention-anchor-terminal.v2',candidate_id=ID,experiment=g.reference,
    ...
    except Exception as e:result.update(status='execution_failed',failure_class=getattr(e,'category','correctness_or_evidence_failure'),error=str(e),scientific_verdict='Incomplete native anchor comparison; preserve completed evidence.')
