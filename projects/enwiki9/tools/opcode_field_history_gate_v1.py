#!/usr/bin/env python3
"""Bounded field-history comparison; reuse published guard and identity checks."""
import argparse
import json
import os
...
phase, binding = authority.phase, authority.binding
compare, require_phase, BudgetStop = authority.compare, authority.require_phase, authority.BudgetStop
require = phase.require
CID = 'opcode_field_history250k_q0_v1'
SELF = 'tools/opcode_field_history_gate_v1.py'
CLI = 'tools/opcode_field_history_corpus_v1.py'
CAPS, PHASES = dict(authority.CAPS), dict(authority.PHASES)
SOURCES = {SELF, CLI, 'tools/opcode_wiki_slot_gate_v1.py',
           'tools/opcode_field_history_v1.py', 'lib/opcode_field_history_v1.py',
           'tools/opcode_field_history_build_v1.py',
           ...
           'tools/research_contracts.py', 'tests/test_opcode_field_history_gate_v1.py'}
...
def validate_plan(plan):
    ...
    require(len(plan['package_files']) == 2 and
            {r['path'] for r in plan['package_files']} ==
            {f'programs/{CID}/p', f'programs/{CID}/program.py'}, 'package ownership differs')
    require(sum(r['bytes'] for r in plan['package_files']) == 6606, 'measured bundle size differs')
    require(plan['runtime_files'] and plan['evidence'], 'runtime or evidence missing')


# Reuse the exact published authentication body in a private globals mapping.
# Its frozen-input, revision, snapshot, cgroup and affinity checks stay intact;
# the original gate module and measured files are never modified.
...
def run_comparison(directory, plan, snapshot, marker, limits=PHASES):
    ...
    gains = {key: rows[arm]['archive_bytes'] - rows['D']['archive_bytes']
             for key, arm in [('archive_saving_bytes', 'P'), ('aligned_vs_global_bytes', 'G'),
                              ('aligned_vs_randomized_bytes', 'S')]}
    passed = all(value > 0 for value in gains.values())
    return dict(arms=rows, commands=commands, **gains, correctness_pass=True,
                controls_equivalent=True, prediction_gate_pass=passed,
                interpretation='strict-parent-and-control-gain' if passed else 'comparison-rejected')
...
def main():
    ...
    stage = dict(schema='gamma.enwiki9.opcode-field-history-gate.v1', candidate_id=CID,
                 experiment=reference, input=plan['input'], correctness_pass=False, status='running',
                 package_files=plan['package_files'], local_source_bytes=6606, source_delta_bytes=860,
