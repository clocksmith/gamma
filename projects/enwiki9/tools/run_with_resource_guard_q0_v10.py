...
OBJECTIVE_CONTRACT = pathlib.Path(__file__).resolve().parents[1] / "contracts/research/v1/objective-contract.json"
OBJECTIVE_CONTRACT_SHA256 = "774aab7b321c606410b58de95be93b379331e6f3c0f926c66ed36d10743c1967"
...
def _terminate(proc: subprocess.Popen[Any], cgroup_path: pathlib.Path) -> int:
    ...
    cgroup_kill = cgroup_path / "cgroup.kill"
    try:
        residual = cgroup_procs.read_text().split()
    except OSError:
        residual = []
    if residual and cgroup_kill.exists():
        try:
            cgroup_kill.write_text("1\n")
        except OSError:
            pass
...
def main() -> int:
    ...
    parser.add_argument("--phase", choices=("identity",), default="identity")
    ...
    if args.limit_mode != "tree":
        raise SystemExit("identity phase requires --limit-mode tree")
    ...
    if hashlib.sha256(OBJECTIVE_CONTRACT.read_bytes()).hexdigest() != OBJECTIVE_CONTRACT_SHA256:
        raise SystemExit("bound objective contract identity drift")
    objective = {"objectiveId":"gamma-enwiki9-hutter-105m-v1","objectiveDigest":"sha256:ce4c435c0f398caf65a09050c8518d9c5ea63239f9156048ea2aaaf9b8ffa7e8","objectivePath":"contracts/research/v1/objective-contract.json","targetScoreBytes":105000000,"corpusBytes":1000000000,"corpusSha256":"159b85351e5f76e60cbe32e04c677847a9ecba3adc79addab6f4c6c7aa3744bc"}
