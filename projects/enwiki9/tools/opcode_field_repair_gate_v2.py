...
SELF = 'tools/opcode_field_repair_gate_v2.py'
TESTS = 'tests/test_opcode_field_repair_gate_v2.py'
...
def validate_plan(plan, candidate):
    fields = {'schema', 'candidate_id', 'stage', 'population', 'arms', 'resources', 'runtime_files',
              ...
              'historical_parent', 'selection_receipt', *PHASE_CAPS}
    require(set(plan) == fields and plan['schema'] == SCHEMA and plan['candidate_id'] == candidate, 'plan fields differ')
    ...
    require(plan['stage'] in populations and plan['arms'] == ARMS, 'unselected stage or controls')
    size, digest = populations[plan['stage']]
    reference_shape(plan['population'])
    require(plan['population']['bytes'] == size and plan['population']['sha256'] == digest
            and plan['historical_parent'] is None, 'reserved population or fresh parent policy differs')
    caps = CAPS if size == 250000 else dict(cpus=[2], memory_bytes=12884901888, scratch_bytes=2147483648, swap_bytes=0, wall_seconds=12000)
    phases = PHASE_CAPS if size == 250000 else dict(phase_cpu_seconds=1200, phase_wall_seconds=1500, phase_address_bytes=8589934592)
    require(plan['resources'] == caps and all(type(plan['resources'][k]) is int for k in caps if k != 'cpus')
            and all(type(plan[k]) is int and plan[k] == v for k,v in phases.items()), 'resource bounds differ')
    reference_shape(plan['cli']); reference_shape(plan['selection_receipt'])
    for key in ('source_files','package_files','parent_package_files','kernel_basis','runtime_files'):
        ...
        for row in rows: reference_shape(row, absolute=key == 'runtime_files')
    sources = {r['path']:r for r in plan['source_files']}
    require({SELF,TESTS,SHARED_DRIVER,HARNESS_BASE,plan['cli']['path']} <= set(sources)
            and sources[plan['cli']['path']] == plan['cli'], 'source closure missing')
    require({r['path'] for r in plan['package_files']} <= set(sources), 'codec unbound')
    expected = {'p':'3e9c9ed25997ad10bac94575fb3da5009530ba04fa961da14a016fe89eec9a15',
    ...
    require({Path(r['path']).name:r['sha256'] for r in plan['package_files']} == expected, 'selected codec changed')


def validate_selection(plan, contract):
    row = plan['selection_receipt']; check_file(row)
    require(any(r['path'] == row['path'] and r['sha256'].removeprefix('sha256:') == row['sha256']
                for r in contract['inputs']), 'selection not contract-bound')
    receipt = driver.read_json(ROOT/row['path'])
    require(receipt['schema'] == 'gamma.enwiki9.reflection-receipt.v1' and receipt['validity']['valid']
            and receipt['hypothesis']['verdict'] == 'supported'
            and receipt['attribution']['failureClass'] == 'algorithmic-gain'
            and receipt['attribution']['controlsEquivalent'] is True
            and receipt['decision']['verdict'] == 'next-gate'
            and receipt['decision']['nextGateBytes'] == plan['population']['bytes']
            and receipt['measurements']['netBytesSaved'] > 0, 'prior selection did not pass')
    require(receipt['candidateId'] == contract['parent']['candidateId'], 'selection parent differs')
...
def authenticate(candidate, validate_only=False):
    ...
    validate_selection(plan, contract)
    ...
    if not validate_only:
        ...
        require((group / 'memory.max').read_text().strip() == str(plan['resources']['memory_bytes'])
...
def comparison_table(rows, plan):
    ...
    return dict(schema='gamma.enwiki9.opcode-field-costs.v1', archive_bytes=sizes, archive_saving_bytes=saving,
                ...
                strict_archive_improvement=saving > 0, validation_eligible=False,
                confirmation_eligible=plan['stage'] == 'validation' and saving > 0, confirmation_requires_positive_separately_frozen_validation=True,
...
def main(argv=None):
    ...
    try:
        for arm in ['legacy', 'P', 'K', 'D']:
            ...
            if arm == 'legacy':
                require(driver.sha(paths['archive']) == report['archive_sha256']
                        and paths['archive'].stat().st_size == report['archive_bytes']
                        and report['raw_bytes'] == len(raw) and report['raw_sha256'] == plan['population']['sha256'],
                        'fresh retained parent identity differs')
                if plan['historical_parent'] is not None:
                    require(report['archive_sha256'] == plan['historical_parent']['archive_sha256']
                            and report['archive_bytes'] == plan['historical_parent']['archive_bytes'], 'historical reference differs')
