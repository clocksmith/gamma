...
STUDENT_MODEL="${STUDENT_MODEL:-google/gemma-3-1b-it}"
...
SAVE_TAIL_START="${SAVE_TAIL_START:-0}"
SAVE_EVERY_TAIL="${SAVE_EVERY_TAIL:-0}"
SAVE_TAIL_MAX_CHECKPOINTS="${SAVE_TAIL_MAX_CHECKPOINTS:-0}"
...
if [[ "$SKIP_TRAIN" == "0" ]]; then
  cmd=(
    ...
    --save-tail-start "$SAVE_TAIL_START"
    --save-every-tail "$SAVE_EVERY_TAIL"
    --save-tail-max-checkpoints "$SAVE_TAIL_MAX_CHECKPOINTS"
