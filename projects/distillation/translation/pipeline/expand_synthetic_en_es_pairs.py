"""Expand and rewrite the EN/ES synthetic translation training shards."""
...
DEFAULT_SHARD_DIR = Path(
    "projects/distillation/translation/training_data/"
    "translate_distill_pairs_en_es_2way.train.shards"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--new-rows", type=positive_int, default=9220)
    parser.add_argument("--rows-per-shard", type=positive_int, default=5000)
...
def main() -> int:
    ...
    seen_sources = set()
    fixed_existing = []
    shard_pattern = f"{SHARD_PREFIX}.shard-*.jsonl"

    for shard_path in sorted(args.shard_dir.glob(shard_pattern)):
        with shard_path.open("r", encoding="utf-8") as rows:
            for line in rows:
    ...
    target_en_es = args.new_rows // 2
    target_es_en = args.new_rows - target_en_es
    new_rows = []
    ...
    for old_path in sorted(args.shard_dir.glob(shard_pattern)):
        old_path.unlink()

    shards_meta = []
    for offset in range(0, len(all_rows), args.rows_per_shard):
        shard_rows = all_rows[offset : offset + args.rows_per_shard]
        shard_index = offset // args.rows_per_shard + 1
        shard_name = f"{SHARD_PREFIX}.shard-{shard_index:05d}.jsonl"
        shard_path = args.shard_dir / shard_name
    ...
    manifest = {
        ...
        "generator": "projects/distillation/translation/pipeline/expand_synthetic_en_es_pairs.py",
        "seed": args.seed,
        "new_rows": args.new_rows,
        "out_dir": str(args.shard_dir),
        "prefix": SHARD_PREFIX,
    ...
    manifest_path = args.shard_dir / f"{SHARD_PREFIX}.shards.json"
