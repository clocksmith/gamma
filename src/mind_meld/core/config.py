...
class MeldConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'MeldConfig':
        ...
        _ENUM_FIELDS: Dict[str, Dict[str, type]] = {
            'swap_config': {'strategy': SwapStrategy},
            'translation_config': {'mode': TranslationMode, 'vocabulary_strategy': VocabularyStrategy},
        }

        kwargs: Dict[str, Any] = {}
        for key, value in data.items():
            if key in _DATACLASS_FIELDS:
                sub_cls = _DATACLASS_FIELDS[key]
                sub_data = dict(value)
                for enum_field, enum_cls in _ENUM_FIELDS.get(key, {}).items():
                    if enum_field in sub_data:
                        sub_data[enum_field] = enum_cls(sub_data[enum_field])
