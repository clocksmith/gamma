...
Usage:
  node tools/codegen-bench/index.js [options]

Quick start:
  node tools/codegen-bench/index.js              # Basic JS vs TS comparison (default)
  node tools/codegen-bench/index.js --extended   # Add scripting and backend tasks
...
Examples:
  ...
  node tools/codegen-bench/index.js --category foundations --language js,ts --all-prompt-levels --provider ollama-qwen3-30b --runs 5

  # Test prompt effectiveness: novice vs expert
  node tools/codegen-bench/index.js --category foundations --language js --prompt-level novice,expert --provider ollama-gpt-oss-20b --runs 3

  # High temperature for code variation analysis
  node tools/codegen-bench/index.js --category foundations --language ts --temperature 1.0 --provider ollama-qwen3-30b --runs 5

  # Deterministic testing (zero temperature)
  node tools/codegen-bench/index.js --category foundations --language js --temperature 0.0 --provider ollama-gpt-oss-120b

  # Test multiple temperatures (multiplies test runs)
  node tools/codegen-bench/index.js --task fibonacci --language js --temperatures 0.0,0.5,1.0,1.5 --provider ollama-qwen3-30b --runs 3

  # Temperature range testing
  node tools/codegen-bench/index.js --task expression-evaluator --language js,ts --temperature-range 0.0,2.0,0.5 --provider ollama-gpt-oss-20b

  # Use mock responses for testing (dry mode)
  node tools/codegen-bench/index.js --task fibonacci --language js,ts --dry

  # Legacy variant-based API still works
  node tools/codegen-bench/index.js --task fibonacci --variant javascript-expert,typescript-expert --provider ollama-qwen3-30b
`);
}
