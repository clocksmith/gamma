"""Export a prompt-tree attention and hidden-state dataset."""
...
OUTPUT_FILE = Path("reports/model_analysis/tree_attention_data.json")
...
def main():
    ...
    for i, prompt in enumerate(prompts):
        ...
        for token_vec in last_layer_hidden:
            ...
            rgb = (
                ((rgb - rgb.min()) / (rgb.max() - rgb.min() + 1e-6) * 255)
                .astype(int)
                .tolist()
            )
    ...
    print(f"\nWriting {len(export_data)} artifacts to {args.output}...")
