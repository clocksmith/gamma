...
try:
    if not (platform.system() == "Darwin" and platform.machine().startswith("arm")):
        print(
            "MLXEngine WARNING: MLX is designed for Apple Silicon. May not function correctly or optimally elsewhere."
        )
    ...
    raise ImportError(
        "MLX libraries (mlx, mlx-lm) not found. Install with `pip install -r requirements-mlx.txt` (Apple Silicon recommended)."
    )
...
class MLXEngine(LLMEngine):
    """LLMEngine using Apple's MLX via mlx-lm, supports KV caching inherently."""
    ...
    def __init__(
        ...
        self._model_args: Optional[Dict[str, Any]] = None  # From mlx_load_model

    def load(self):
        model_id = self.model_name
        print(f"MLXEngine: Loading model '{model_id}'...")
        load_cfg_args = self.engine_config.get(
            "mlx_load_config", game_config.MLX_LOAD_CONFIG
        )
        ...
        try:
            self._mlx_model, self.tokenizer, self._model_args = mlx_load_model(
                model_id, config=load_cfg_args, adapter_path=adapter_path_arg
            )
            mx.eval(
                self._mlx_model.parameters()
            )  # Ensure model parameters are evaluated on device
    ...
    def encode(self, text: str, add_special_tokens: bool = True) -> Tuple[mx.array, Optional[mx.array]]:  # type: ignore
        if not self.tokenizer:
            raise RuntimeError("MLXEngine: Tokenizer not loaded.")
        # mlx-lm tokenizer usually expects HF-style input, returns dict of numpy arrays
        encoded_np = self.tokenizer(
    ...
    def decode(self, ids: Any, skip_special_tokens: bool = False) -> str:
        if not self.tokenizer:
            raise RuntimeError("MLXEngine: Tokenizer not loaded.")
        # Ensure IDs are Python list for tokenizer.decode
        ...
        elif isinstance(ids, (list, tuple, np.ndarray)):
            ids_list = list(ids)
        else:
            try:
                ids_list = [int(ids)]
                ...
                raise TypeError(
                    f"Unsupported token_ids type for MLX decode: {type(ids)}"
                )
        return self.tokenizer.decode(ids_list, skip_special_tokens=skip_special_tokens)

    def _s(self, l: mx.array) -> mx.array:
        return mx.softmax(l, axis=-1)  # type: ignore

    def _t(self, l: mx.array, temp: float) -> mx.array:
        return l / mx.maximum(mx.array(temp, dtype=l.dtype), 1e-6) if temp > 0 else l  # type: ignore

    def _k(self, l: mx.array, k_val: int) -> mx.array:  # type: ignore
        vocab_size = l.shape[-1]
        k = min(k_val, vocab_size)
        if k <= 0 or k >= vocab_size:
            return l
        # Get top_k values, then create mask based on threshold
        top_k_values = mx.sort(l, axis=-1)[..., -k:]  # type: ignore
        threshold = top_k_values[
            ..., 0:1
        ]  # Smallest of the top_k values (using 0:1 to keep dim)
    ...
    def _top(self, l: mx.array, k_show: int) -> Tuple[List[str], List[float], List[int]]:  # type: ignore
        if l.size == 0 or mx.all(mx.isinf(l)):
            return ["<No Valid Tokens>"], [1.0], [-1]  # type: ignore
        probs_mx = self._s(l)
        vocab_size = probs_mx.shape[-1]
        effective_k = min(k_show if k_show > 0 else vocab_size, vocab_size)

        # Get top_k from probabilities
        top_probs_vals = mx.sort(probs_mx, axis=-1)[..., -effective_k:][..., ::-1]  # type: ignore Sort descending
        # Get indices corresponding to these top_k probabilities
        top_indices_vals = mx.argsort(probs_mx, axis=-1)[..., -effective_k:][..., ::-1]  # type: ignore

        if top_probs_vals.ndim > 1:  # Squeeze if batch dim was present
            top_probs_vals = mx.squeeze(top_probs_vals, axis=0)
            top_indices_vals = mx.squeeze(top_indices_vals, axis=0)  # type: ignore

        top_indices_list = np.array(top_indices_vals).tolist()  # Convert to Python list
        return (
            [self.get_token_text(idx) for idx in top_indices_list],
            np.array(top_probs_vals).tolist(),
            top_indices_list,
        )
    ...
    def predict_next(self, input_ids: mx.array, attention_mask: Optional[mx.array], temperature: float, top_k: int, top_p: float, output_attentions: bool = False, output_hidden_states: bool = False) -> Dict[str, Any]:  # type: ignore
        if not self._mlx_model:
            raise RuntimeError("MLXEngine: Model not loaded.")
        st = time.time()
        # mlx-lm models handle cache internally. Pass current cache (_kv_cache).
        # input_ids for subsequent steps should be just the new token, e.g., shape (1,1).
        # The model call returns (logits_for_all_input_tokens, updated_cache_tuple).
        current_cache_to_pass = self._kv_cache if input_ids.shape[-1] == 1 else None

        try:
            model_outputs_tuple = self._mlx_model(
                input_ids, cache=current_cache_to_pass
            )
            mx.eval(model_outputs_tuple)  # Ensure computation completes
        except Exception as e:
            raise RuntimeError(
                f"MLX model execution failed for input shape {input_ids.shape}: {e}"
            ) from e

        logits_all_steps, updated_kv_cache = model_outputs_tuple  # Unpack
        self._kv_cache = updated_kv_cache  # Store updated cache for next step

        logits_raw = logits_all_steps[:, -1, :]  # Logits for the last token
        ...
        next_token_id_mx = mx.argmax(probs_proc, axis=-1)
        next_token_id_val = np.array(next_token_id_mx).item()  # type: ignore
        ...
        return {
            ...
            "attention": None,
            "hidden_states": None,  # Not standard from mlx-lm generate/call
    ...
    def get_vocabulary_size(self) -> int:
        if not self.tokenizer:
            raise RuntimeError("MLXEngine: Tokenizer not loaded.")
            return -1
        # mlx-lm tokenizer (usually HF AutoTokenizer) has vocab_size
    ...
    def get_attention_for_visualization(
        ...
        # mlx-lm's model.__call__ does not typically return attention weights by default.
        # This would require modifying the underlying model or using a specific model variant.
        if game_config.DEFAULT_VERBOSE or self.engine_config.get("verbose", False):
            print(
                "(MLXEngine: Attention heatmap visualization is not generally available with standard mlx-lm model calls.)"
            )
        return None
    ...
    def get_probabilities_at_step(
        ...
        if not isinstance(data, mx.array):
            raise TypeError(f"Expected mx.array for MLX probabilities, got {type(data)}")  # type: ignore
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "MLX Model Path/ID": self.model_name,
            "Model Type (from mlx-lm)": (
                self._model_args.get("model_type", "N/A") if self._model_args else "N/A"
            ),
        }
        if cfg_args.get("mlx_adapter_path"):
            summary["Adapter Path"] = cfg_args.get("mlx_adapter_path")
        if cfg_args.get("mlx_load_config"):
            summary["Load Config Used"] = str(
                cfg_args.get("mlx_load_config")
            )  # Stringify dict for display
        return summary
