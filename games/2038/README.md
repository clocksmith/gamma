...
`0.8.0-rc.16-test`.
Executable game `0.14.15` implements that candidate under
...
- `http://localhost:8038/` — play the synchronized `0.14.15` game and export its
  replay.
- `http://localhost:8038/lab` — run `0.14.15` tournaments, strategy evolution,
...
`npm run check` is the release gate. It verifies the executable `0.14.15`
bundle, its synchronized `0.8.0-rc.16-test` physical-rules candidate, both
