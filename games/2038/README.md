...
`0.7.0-rc.6-test`.
Executable game `0.13.0` implements that candidate under
...
- `http://localhost:8038/` — play the synchronized `0.13.0` game and export its
  replay.
- `http://localhost:8038/lab` — run `0.13.0` tournaments, strategy evolution,
...
`npm run check` is the release gate. It verifies the executable `0.13.0`
bundle, its synchronized `0.7.0-rc.6-test` physical-rules candidate, both
