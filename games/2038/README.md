...
`0.8.0-rc.6-test`.
Executable game `0.14.5` implements that candidate under
...
- `http://localhost:8038/` — play the synchronized `0.14.5` game and export its
  replay.
- `http://localhost:8038/lab` — run `0.14.5` tournaments, strategy evolution,
...
`npm run check` is the release gate. It verifies the executable `0.14.5`
bundle, its synchronized `0.8.0-rc.6-test` physical-rules candidate, both
