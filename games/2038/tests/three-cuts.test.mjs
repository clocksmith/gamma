...
test("Build supplies an Era III production engine and still leaves Era IV for Fusion", async () => {
  ...
  assert.equal(player.projects.filter(p => p.projectId === "mega_cluster").length, 1);
  assert.ok(match.operatingProjects(player).some(p => p.projectId === "mega_cluster"));
  ...
  assert.ok(player.projects.some(p => p.projectId === "fusion_demonstrator"));
  assert.equal(player.projects.find(p => p.projectId === "mega_cluster").builtEra, 3);
});
...
test("personal project host ownership and stale plans remain authoritative under Build", async () => {
  ...
  const tile = match.board.find(t => t.id === "renewable_basin");
  player.facilities = [{ id: "host", tileId: tile.instanceId, category: tile.category }];
  const choice = match.legalResolutions(0, "build").find(c => !c.parameters.facility && c.parameters.project?.id === "mega_cluster");
  assert.ok(choice);
  player.projects.push({projectId:"mega_cluster",hostId:"host",builtEra:3});
  assert.throws(() => match.applyResolution(0, choice), /no longer legal/);
...
test("Fusion has a personal limit and consumes no Generator slot", async () => {
  const match = await game(); match.round = 3; await match.beginRound([]);
  const tiles = match.board.filter(t=>t.category!=="frontier");
  for (const player of match.players) {
    player.runway=10;player.compute=10;
    player.facilities=[{id:`host-${player.seat}`,tileId:tiles[player.seat].instanceId,category:tiles[player.seat].category}];
