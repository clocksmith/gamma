...
focused regression or blind-play contract in executable `0.14.18` and rules
candidate `0.8.0-rc.19-test`.
...
- [x] **G2038-001 — Training Run banking is chosen before the run instead of after each safe reveal.**
  - Evidence: `rules.md:710-717` lets the player bank or continue after any non-duplicate card. `lab/environment/core-economy-match.js:422-433` offers only a preselected `stopAt` value from 2 through 7, and `web/src/engine.js:255-321` banks automatically when `seen.size` reaches it.
...
- [x] **G2038-002 — Curated Corpus does not become a duplicate when every ordinary domain is already present.**
  - Evidence: `rules.md:730-732` says Curated Corpus is a duplicate when no ordinary domain remains. `web/src/engine.js:290-295` simply does nothing when `firstMissingDomain()` returns no domain.
...
- [x] **G2038-003 — The second Synthetic Loop does not resolve as its own unprotectable duplicate.**
  - Evidence: `rules.md:737-741` makes the second Synthetic Loop the duplicate. `web/src/engine.js:306-311` no-ops on the second Loop and only marks the next duplicate as unprotected.
...
- [x] **G2038-004 — Licensed Dataset is forced whenever Runway is available.**
  - Evidence: `rules.md:735-736` gives the player a choice between paying one Runway to continue and declining to bank. `web/src/engine.js:299-305` always pays when Runway is positive.
...
- [x] **G2038-005 — Duplicate protection is automatic and its competing sources cannot be chosen.**
  - Evidence: duplicate protection is optional in `rules.md:722-726`; Scientific Method is also an optional paid reaction in `components/factions.json`. `web/src/engine.js:274-285` automatically spends Safety, while `lab/environment/selected-rules-match.js:2572-2603` makes Scientific Method available only when Safety is zero and the acting piece is not at Research.
...
- [x] **G2038-006 — Unique-domain effects use Capability gained instead of the domains actually banked.**
  - Evidence: the Training result already returns `distinctDomains` in `web/src/engine.js:324-338`. The Demonstration Holds Mandate, Scaling-Law Breakthrough, and The Matter Compiler Enters Public Maintenance count domains in `components/mandates.json`, `components/factions.json`, and `components/headlines.json`. `lab/environment/selected-rules-match.js:2642-2658,2677-2680` uses the Capability delta instead.
...
- [x] **G2038-008 — The Fund Mandate records the printed gain rather than Runway actually gained after the cap.**
  - Evidence: caps apply immediately after gains in `rules.md:231-239`; the Mandate asks for Runway gained through Fund in `components/mandates.json`. `lab/environment/selected-rules-match.js:2635-2640` adds the requested `actual` amount to `fundRunway` even if `addResource()` clipped the gain.
...
- [x] **G2038-009 — Immediate Facility effects are counted as Compute produced during Production.**
  - Evidence: `rules.md:871-873` says immediate Facility production is not a second Production. `lab/environment/selected-rules-match.js:3458-3527` increments `roundMetrics.computeProduced` inside `produceFacility()` regardless of its `stage`; that helper is also called by immediate effects at `lab/environment/selected-rules-match.js:1392-1397,4670-4675`.
...
- [x] **G2038-010 — Joint Venture Compute is omitted from Compute produced during Production.**
  - Evidence: Joint Ventures produce in the Partner box under `rules.md:608-611`, and the Mandate counts Compute produced during Production in `components/mandates.json`. `lab/environment/selected-rules-match.js:3862-3889` grants the exchanged resource without incrementing `roundMetrics.computeProduced` when that resource is Compute.
...
- [x] **G2038-012 — Organize Recruit omits its additional movement.**
  - Evidence: `rules.md:776-777` recruits and then moves one CEO or Team up to two additional adjacent hexes. `lab/environment/selected-rules-match.js:2476-2504` recruits and returns immediately.
...
- [x] **G2038-013 — Organize Redistribute cannot divide five steps among pieces.**
  - Evidence: `rules.md:778` allows a combined total of five adjacent steps across CEOs and Teams. `lab/environment/selected-rules-match.js:1757-1778,2506-2513` selects one extra piece and teleports it to a tile at axial distance at most five.
...
- [x] **G2038-014 — Reorganization makes arbitrary movement and return choices for the player.**
  - Evidence: `rules.md:382-389` lets the player move each Team up to one hex and optionally choose one Team to return. `lab/environment/selected-rules-match.js:3258-3273` moves every Team to the first enumerated board option and returns the first Team found.
...
- [x] **G2038-015 — The Company Brain Survives Bankruptcy replaces Organize instead of modifying it.**
  - Evidence: the Headline says “During Organize” in `components/headlines.json`. `lab/environment/selected-rules-match.js:1885-1903,2529-2537` creates a standalone `employee_free` resolution, returns the first N Teams, and performs none of Organize's three modes.
...
- [x] **G2038-016 — Cost reductions are applied after base legality has already removed actions.**
  - Evidence: replacement, surcharge, and discount order is defined in `rules.md:241-242`. Base generation requires one Compute for Research and two Runway for a Facility in `lab/environment/core-economy-match.js:422-445`; selected-rule adjustments occur later in `lab/environment/selected-rules-match.js:2058-2144`.
...
- [x] **G2038-017 — The deepfake Regulate surcharge can resolve without being paid.**
  - Evidence: `components/headlines.json` adds one Compute to Deploy. `lab/environment/selected-rules-match.js:2759-2761` subtracts it only after resolution and clamps the balance at zero, rather than including it in legal cost calculation.
...
- [x] **G2038-018 — The deepfake vote effect expires after its reveal cycle instead of lasting to Era end.**
  - Evidence: `components/headlines.json` says the selected regime lasts from the vote until Era end. `lab/environment/selected-rules-match.js:1400-1408` stores the result in `regime.cycle`, and later Deploy handling reads only that cycle at `lab/environment/selected-rules-match.js:2759-2763`.
...
- [x] **G2038-019 — Do Nothing rewards Customers gained before the deepfake vote.**
  - Evidence: `components/headlines.json` limits the bonus to Customers gained after the vote. `lab/environment/selected-rules-match.js:3892-3899` compares final Customers with the Era-start snapshot.
...
- [x] **G2038-020 — The Court Adopts Supported Meaning hard-codes two rivals instead of half rounded up.**
  - Evidence: `components/headlines.json` requires at least half of the controller's rivals, rounded up. `lab/environment/selected-rules-match.js:4713-4721` checks `rivals.length >= 2` for every player count.
...
- [x] **G2038-021 — Emergency Power is committed before the actions that determine Production demand.**
  - Evidence: `components/headlines.json` assigns emergency Power during Production. `lab/environment/selected-rules-match.js:1501-1512` asks for 0–2 immediately after the Headline resolves, before action selection, then consumes the stored number at `lab/environment/selected-rules-match.js:3577-3583`.
...
- [x] **G2038-022 — The Social Graph disappears when the controlled remote tile is reachable normally.**
  - Evidence: `components/factions.json` permits any controlled Consumer or Media destination without moving there. `lab/environment/selected-rules-match.js:1999-2031` excludes every tile already present among normal movement resolutions.
...
- [x] **G2038-023 — Allocation Window does not create usable unsold temporary Compute.**
  - Evidence: `components/factions.json` creates temporary Compute before offers and says all remaining tokens disappear at cycle end. `lab/environment/selected-rules-match.js:1023-1049` permits `allocation_hold`, while `lab/environment/selected-rules-match.js:1076-1099` creates value only when a sale is accepted.
...
- [x] **G2038-024 — Export Controls does not block Allocation Window Compute sales.**
  - Evidence: `components/headlines.json` prohibits Compute trades for the cycle. `lab/environment/selected-rules-match.js:1313-1318` sets `computeTradeBlocked`, but Allocation Window executes through `lab/environment/selected-rules-match.js:996-1103` without checking it.
...
- [x] **G2038-025 — An isolated Generator becomes an Advanced Network anchor.**
  - Evidence: `advanced.md:19-34` starts the graph at the first Facility or a Link and propagates through adjacent owned sites. `web/src/engine.js:707-740` first adds every Facility adjacent to any Generator, then propagates from all of those Facilities.
...
- [x] **G2038-026 — Mega-Cluster Power is assigned after, and outside, the player's allocation choice.**
  - Evidence: `rules.md:600-606` tells players to allocate Power to Facilities and Mega-Clusters. `lab/environment/selected-rules-match.js:3721-3769` offers only Facility subsets; `lab/environment/selected-rules-match.js:3796-3831` then powers projects automatically in creation order from leftovers.
...
- [x] **G2038-027 — A joint Mega-Cluster can be initiated only from the lead host.**
  - Evidence: `rules.md:358,365-372` allows the acting piece to end on either host. Solo choices enumerate both hosts in `lab/environment/selected-rules-match.js:2164-2183`; joint choices require and record only the lead's left host in `lab/environment/selected-rules-match.js:2192-2215`.
...
- [x] **G2038-028 — Joint Mega-Cluster Scrutiny ownership is unspecified, but the executable assigns it to the lead.**
  - Evidence: construction adds two Scrutiny in `rules.md:350-354`; joint costs and outputs are split in `rules.md:365-372` without assigning that Scrutiny. `lab/environment/selected-rules-match.js:3244-3257` gives both cubes to the lead.
...
- [x] **G2038-029 — A rejected Advanced Power request may target the same rival again.**
  - Evidence: `advanced.md:62-67` requires the second request to a different rival Network even after rejection. `lab/environment/selected-rules-match.js:3587-3612,3669-3687` excludes a supplier only after an accepted sale.
...
- [x] **G2038-030 — Executable Production resolves its resource producers in the wrong order.**
  - Evidence: `rules.md:608-611` orders powered Facilities, Customer income, Mega-Clusters, then numbered Joint Ventures. `lab/environment/selected-rules-match.js:3796-3899` resolves Mega-Clusters, Facilities, Joint Ventures, and Customers.
...
- [x] **G2038-031 — Scrutiny overflow penalties are chosen automatically.**
  - Evidence: `rules.md:633-637` lets the player pay Runway or lose Trust when both are available. `lab/environment/core-economy-match.js:183-195` always spends Runway first.
...
- [x] **G2038-032 — Audit penalties are chosen automatically.**
  - Evidence: `rules.md:655-662` gives an available payment choice in Eras I–III and the defined Era IV fallback. `lab/environment/selected-rules-match.js:3967-4012` always spends Runway whenever the configured amount is available.
...
- [x] **G2038-033 — Agent Swarm offers known-unresolvable Core Actions and converts them into forced no-ops.**
  - Evidence: `rules.md:423-427` says to resolve two different unused Core Actions and pay all costs. `lab/environment/selected-rules-match.js:3303-3318` includes unused actions with zero current resolutions; `lab/environment/selected-rules-match.js:3321-3337` then manufactures a forced no-op.
...
- [x] **G2038-034 — The Shovels does not define whether Agent Swarm's two Core Actions aggregate into one spend.**
  - Evidence: `components/factions.json` triggers on at least two Compute spent “in one action.” Agent Swarm resolves two Core Actions in `rules.md:423-427`, but `lab/environment/selected-rules-match.js:4600-4605` measures Compute before and after the entire selected Escalation.
...
- [x] **G2038-035 — “Adjacent rival Power connection” has no complete Default Game endpoint rule.**
  - Evidence: Default Game explicitly has local Power rather than a Network in `rules.md:257-265`, while the trade rule uses “adjacent rival Power connection” in `rules.md:593-599`. The executable defines adjacency as any adjacent pair of currently eligible buyer and supplier Facilities in `lab/environment/selected-rules-match.js:3596-3611`.
...
- [x] **G2038-036 — Escalation card faces omit their unlock Era and timing metadata.**
  - Evidence: `components/programs.json` records `unlockedRound` and `timing` for every card. `content/templates/card-reference.md:206-244` projects only name, rules text, and flavor while telling players to select an unlocked card.
...
- [x] **G2038-037 — The Era III reference card omits immediate Power purchases.**
  - Evidence: the authoritative unlock rule introduces Joint Ventures and immediate Power purchases in `rules.md:391-395`. The Era III card in `components/reference-cards.json` says only “agreements” plus the two named Escalations.
...
- [x] **G2038-038 — Training card faces have no projected gameplay text.**
  - Evidence: `content/templates/component-reference.md:3-5` says printed Training cards own their exact text, and `content/templates/card-reference.md:3-6` claims to collect every card effect. Its Training section at `content/templates/card-reference.md:608-623` includes only names and flavor, while all special behavior exists only in `rules.md:728-743`.
...
- [x] **G2038-039 — Power Source card faces omit their mechanical contracts.**
  - Evidence: `content/templates/component-reference.md:3-5` gives printed Power cards exact-text authority. `content/templates/card-reference.md:625-645` shows only tagline and public claim, omitting construction site, cost, capacity, Trust, recurring Scrutiny, uniqueness, and Fusion output defined in `rules.md:817-844`.
...
- [x] **G2038-040 — The Dossier orientation instruction names the wrong card.**
  - Evidence: `content/templates/component-reference.md:84-86` says to place the current Era card face down, while setup gives each player Era-labelled Dossier cards in `rules.md:44-48` and filing uses the matching Dossier in `rules.md:625-629`.
...
- [x] **G2038-041 — Starting-grid identity conflicts between “first constructed Facility” and “Facility 1.”**
  - Evidence: `rules.md:49-53` and `content/templates/component-reference.md:34-41` assign the grid to the first Facility constructed. `physical/component-spec.md:136-143` permanently assigns it to Facility 1, but no rule requires Facility 1 to be constructed first.
...
- [x] **G2038-042 — Prediction Bag token source is contradictory.**
  - Evidence: `rules.md:463-467,493-498` and `physical/component-spec.md:38-46` use faction-coloured Scrutiny cubes as prediction tokens. `physical/component-inventory.md:54-58` says generic track cubes become Prediction Bag tokens.
...
- [x] **G2038-043 — The physical kit has no defined place to retain World Ending inputs.**
  - Evidence: setup requires recording Setup Collective Trust in `rules.md:57-59`; final resolution requires final Collective Trust and unresolved Systemic Risk in `rules.md:487-492,900-920`. `physical/score-sheet.md:1-27` has neither field, and the Governance Board zones at `physical/component-spec.md:64-75` provide no writable ending record.
...
- [x] **G2038-044 — The final sentence contradicts the Prediction Bag winner replacement.**
  - Evidence: `rules.md:487-507,885-894` makes Mandate standings provisional and lets a matching eligible claimant become the sole final winner. `rules.md:941-948` then states without qualification that highest Mandate wins.
...
- [x] **G2038-045 — “Emergency Power” is confusable with Emergency Infrastructure.**
  - Evidence: `rules.md:593-599` says installed Generator capacity may be sold but “emergency Power” may not. Emergency Infrastructure is itself an installed Generator in `rules.md:763-767,832-840`; Headline-granted supplemental Power is defined separately in `components/headlines.json`.
...
- [x] **G2038-046 — Canonical data contains stale Generator costs that conflict with the rules.**
  - Evidence: location-owned construction costs are one and two Runway in `components/game.json` and `rules.md:763-767`. The same power sources carry `runwayCost` values of two and three in `components/game.json`.
...
- [x] **G2038-050 — Cloud Partnership chooses its beneficiary by seat order.**
  - Evidence: `experimental/components/tactics.json` says another player gains one Runway without naming a forced recipient. `lab/environment/selected-rules-match.js:1454-1462` always pays the next seat modulo player count.
...
- [x] **G2038-051 — Talent Raid recruits at the CEO's current tile instead of the acting piece's destination.**
  - Evidence: `experimental/components/tactics.json` places the Team at the acting piece's destination. `lab/environment/selected-rules-match.js:1463-1477` has no acting resolution context and places it at the CEO's current tile.
...
- [x] **G2038-052 — Board Reshuffle chooses which Action to ready automatically.**
  - Evidence: `experimental/components/tactics.json` lets the player ready Organize or Influence. `lab/environment/selected-rules-match.js:1478-1480` readies the first matching used Action in array order.
...
- [x] **G2038-053 — Weights Leak chooses the rival Facility automatically.**
  - Evidence: `experimental/components/tactics.json` says to resolve one powered rival Facility. `lab/environment/selected-rules-match.js:1488-1494` selects the first rival with a powered Facility and that rival's first powered Facility.
...
- [x] **G2038-054 — Custom Silicon chooses the first Facility automatically.**
  - Evidence: `experimental/components/tactics.json` applies permanently to one Facility. `lab/environment/selected-rules-match.js:1481-1482` marks `player.facilities[0]` without a target decision.
...
- [x] **G2038-055 — Open Letter is not public before votes are committed.**
  - Evidence: `experimental/components/tactics.json` and `experimental/tactics-rules.md:25-29` require the option and added vote to be public before anyone commits a vote. `lab/environment/selected-rules-match.js:1167-1195` bundles Open Letter use into each player's simultaneous vote decision and reveals it only while totals are calculated.
