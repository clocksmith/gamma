...
**Rules under review:** `0.8.0-rc.4-test`
...
`dist/docs/core-rules.md` is a review draft at `0.8.0-rc.4-test`. Executable
game `0.14.3` implements its Default Game profile under
...
Candidate `0.8.0-rc.4-test` and executable `0.14.3` retain the synchronized
...
The rc.4 candidate changes no playable rule from rc.3. Executable `0.14.3`
adds profile-scoped Action and Mandate-source telemetry so strategy validity
can be checked directly instead of inferred from faction aggregates.
