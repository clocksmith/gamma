...
The lean physical rulebook is under controlled review at `0.5.0-rc.5-test`.
Executable game `0.8.4` implements that candidate under
...
- `http://localhost:8038/` — play the synchronized `0.8.4` game and export its
  replay.
- `http://localhost:8038/lab` — run `0.8.4` tournaments, strategy evolution,
...
`npm run check` is the release gate. It verifies the executable `0.8.4`
bundle, its synchronized `0.5.0-rc.5-test` physical-rules candidate, both
