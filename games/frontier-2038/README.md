...
The lean physical rulebook is under controlled review at `0.5.0-rc.12-test`.
Executable game `0.8.11` implements that candidate under
...
- `http://localhost:8038/` — play the synchronized `0.8.11` game and export its
  replay.
- `http://localhost:8038/lab` — run `0.8.11` tournaments, strategy evolution,
...
`npm run check` is the release gate. It verifies the executable `0.8.11`
bundle, its synchronized `0.5.0-rc.12-test` physical-rules candidate, both
