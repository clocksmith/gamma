...
const distRoot = resolve(projectRoot, "dist", "physical-kit");
...
const outputRoot = resolve(distRoot, kitId);
if (!outputRoot.startsWith(`${distRoot}/`)) {
...
const gallery = labeledGallery(
  await readFile(resolve(projectRoot, "dist/gallery-baseline.html"), "utf8"),
  identity
);
...
for (const path of sourceDataFiles) {
  await copyFile(
    ...
    resolve(outputRoot, "source-data", path.slice("data/".length))
  );
}
await copyFile(
...
await copyFile(
  resolve(projectRoot, "simulation/contracts/playtest-receipt.schema.json"),
...
const relativeFiles = [
  ...
  ...sourceDataFiles.map((path) => `source-data/${path.slice("data/".length)}`)
].sort();
const files = {};
