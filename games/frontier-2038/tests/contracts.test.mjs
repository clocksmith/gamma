...
test("release index separates executable game from physical rules candidate", async () => {
  ...
  assert.equal(current.gameVersion, "0.8.6");
  assert.equal(executable.gameVersion, "0.8.6");
  assert.equal(current.rulesCandidate.version, "0.5.0-rc.7-test");
  assert.equal(candidate.artifactKind, "physical-rules-candidate");
  assert.equal(candidate.implementation.status, "synchronized");
  assert.equal(candidate.implementation.executableGameVersion, "0.8.6");
  assert.equal(candidate.implementation.implementedByGameVersion, "0.8.6");
...
test("complexity-reduction review rules preserve precision and remove table accounting", async () => {
  ...
  for (const clause of [
    "**Rules version:** 0.5.0-rc.7-test",
    "synchronized with executable game 0.8.6",
