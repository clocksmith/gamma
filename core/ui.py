...
def color_text(text: str, color_code: str) -> str:
    return (
        f"{color_code}{text}{cfg.COLOR_RESET}"
...
def wrap_print(
    text: str, indent: str = "", width: int = 70, initial_indent_add: str = ""
):
    ...
    while True:
        ...
        if valid_choices:
            ...
            if user_input_lower in valid_choices_lower:
                original_index = valid_choices_lower.index(user_input_lower)
                return valid_choices[original_index]  # Return original case
            else:
                print(
                    color_text(
...
def display_intro():
    ...
    wrap_print(
        "You'll see a sentence and guess the sequence of text 'tokens' the LLM prefers to generate next."
    )
    wrap_print(
        "Explore how context (via attention) and sampling parameters (Temperature, Top-K, Top-P) influence the LLM's choices."
    )
...
def _get_engine_specific_display_config(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if engine_name == "pytorch":
        engine_cfg_display["PyTorch Attn Impl"] = getattr(
            args, "pytorch_attn", cfg.PYTORCH_ATTN_IMPLEMENTATION
        )
        if getattr(args, "load_in_4bit", False):
            engine_cfg_display["Quantization"] = (
                f"4-bit ({getattr(args, 'bnb_4bit_compute_dtype', 'bfloat16')})"
            )
        elif getattr(args, "load_in_8bit", False):
            engine_cfg_display["Quantization"] = "8-bit"
        engine_cfg_display["KV Cache Used"] = getattr(
            args, "use_kv_cache", cfg.PYTORCH_USE_KV_CACHE
        )
    elif engine_name == "llamacpp":
        engine_cfg_display["GPU Layers"] = getattr(
            args, "llama_cpp_n_gpu_layers", cfg.LLAMA_CPP_N_GPU_LAYERS
        )
        engine_cfg_display["Context Size"] = getattr(
            args, "llama_cpp_n_ctx", cfg.LLAMA_CPP_N_CTX
        )
    elif engine_name == "onnx":
        engine_cfg_display["ONNX Providers"] = getattr(
            args, "onnx_providers", cfg.ONNX_PROVIDERS
        )
        engine_cfg_display["ONNX Tokenizer"] = getattr(
            args, "onnx_tokenizer", "Not Set (CRITICAL!)"
        )
    elif engine_name == "jax":
        engine_cfg_display["JAX DType"] = getattr(args, "jax_dtype", cfg.JAX_DTYPE)
    elif engine_name == "mlx":
        if getattr(args, "mlx_adapter_path", None):
            engine_cfg_display["MLX Adapter"] = args.mlx_adapter_path
    return engine_cfg_display


def display_current_config(
    args: argparse.Namespace, title: str = "Current Game Configuration"
):
    print_header(title)
    ...
    if engine_specifics:
        ...
        for key, val in engine_specifics.items():
            print(f"  {key:<18}: {val}")
    ...
    print(f"  Max Rounds:       {args.steps}")
    print(f"  Choices/Round:    {args.num_choices}")
    print(f"  Tokens/Choice:    {args.permutation_length}")
    print(f"  Focus Words Mode: {'Yes' if args.focus_words else 'No'}")
    print(f"  Player Choice Mode: {'Yes' if args.player_choice_mode else 'No'}")
    print("Display Settings:")
    print(f"  Show Attention:   {'Yes' if args.show_attention else 'No'}")
    print(f"  Verbose Mode:     {'Yes' if args.verbose else 'No'}")
    print(f"  Colors Enabled:   {'Yes' if cfg.USE_COLORS else 'No'}")
    print_separator("-")


def _get_engine_cli_params(
    engine_name: str, current_args: argparse.Namespace
) -> List[Tuple[str, str, str, Any, Optional[str]]]:
    """Returns list of (key_char, attr_name, description, converter_func, type_hint) for engine-specific CLI params."""
    params = []
    if engine_name == "pytorch":
        params.extend(
            [
                (
                    ...
                    "load_in_4bit",
                    "Load in 4-bit (PT)",
                    lambda v: v.lower() in ["true", "yes", "y", "1"],
                    "yes/no",
                ),
                (
                    "q8",
        ...
        params.append(
            (
                "otok",
                "onnx_tokenizer",
                "ONNX Tokenizer Path/Name",
                lambda v: str(v),
                "string",
            )
        )
    # Add more engine-specific CLI modifiable params here
    return params
...
def confirm_or_modify_config(args: argparse.Namespace) -> bool:
    param_details_core = {
        "e": (
            "engine",
            "Engine",
            lambda current_val: select_engine_interactively(current_val),
            f"from {SUPPORTED_ENGINES_UI_LIST}",
        ),
        "m": (
            "model",
            "Model Identifier",
            lambda current_val, eng=args.engine: select_model_interactively(
                eng, current_val
            ),
            "name/path",
        ),
    ...
    while True:
        ...
        choice = get_user_input(
            ...
            [
                cfg.SHORTCUT_CONFIRM_CONFIG_ACCEPT.lower(),
                cfg.SHORTCUT_CONFIRM_CONFIG_MODIFY.lower(),
            ],
            allow_quit=True,
        ).lower()
        ...
        if choice == cfg.SHORTCUT_CONFIRM_CONFIG_MODIFY:
            ...
            print(
                "Enter key of parameter to change (e.g., 't' for Temperature), then new value. Press Enter to skip."
            )

            # Combine core and engine-specific params for modification UI
            all_modifiable_params = dict(param_details_core)  # Start with core
            if args.engine:  # If engine is set, add its specific params
                ...
                for (
                    ...
                    converter,
                    type_h_eng,
                ) in engine_cli_params_list:
                    # Ensure engine-specific keys don't clash with core keys
                    # This simple UI uses single char keys; more robust UI might need full names
                    if key_char not in all_modifiable_params:
                        all_modifiable_params[key_char] = (
                            attr_name,
                            desc,
                            converter,
                            type_h_eng,
                        ...
                        print(
                            f"Warning: Key '{key_char}' for engine param '{desc}' clashes with a core param. Skipping interactive modification for it."
                        )

            for key, (attr, desc, _, type_h) in all_modifiable_params.items():
                current_val = getattr(args, attr, "Not Set")
                if isinstance(current_val, bool):
                    current_val_disp = "Yes" if current_val else "No"
                elif isinstance(current_val, float):
                    current_val_disp = f"{current_val:.2f}"
                else:
                    current_val_disp = str(current_val)
                ...
                new_val_str = get_user_input(
                    prompt_text,
                    allow_quit=False,
                    allow_empty=True,
                    default_val_on_empty=cfg.SHORTCUT_MODIFY_PARAM_SKIP,
                )
                ...
                try:
                    ...
                    if attr == "engine":
                        new_val = convert_func(getattr(args, attr, cfg.DEFAULT_ENGINE))
                    elif attr == "model":
                        new_val = convert_func(
                            getattr(args, attr, cfg.DEFAULT_MODEL_NAME), args.engine
                        )
                    else:
                    ...
                    if new_val is not None:
                        ...
                        if attr == "engine" and new_val != old_engine:
                            print(
                                color_text(
                                    f"Engine changed to '{new_val}'. Model selection may need an update.",
                                    cfg.COLOR_YELLOW,
                                )
                            )
                            if getattr(args, "model", None):
                                model_confirm = get_user_input(
                                    f"Keep model '{args.model}' for new engine '{new_val}'? (y/n)",
                                    ["y", "n"],
                                )
                                if model_confirm == "n":
                                    new_model = select_model_interactively(
                                        new_val, getattr(args, "model", None)
                                    )
                                    if new_model == cfg.SHORTCUT_QUIT:
                                        return False
                                    setattr(args, "model", new_model)
                except ValueError:
                    print(
                        color_text(
                            f"Invalid value type for '{desc}'. Please try again.",
                            cfg.COLOR_RED,
                        )
                    )
                    ...
                    print(
                        color_text(
                            f"Error setting parameter '{desc}': {e}", cfg.COLOR_RED
                        )
                    )
    return False
...
def select_engine_interactively(current_default_engine: str) -> Optional[str]:
    ...
    for i, name in enumerate(SUPPORTED_ENGINES_UI_LIST):
        print(
            f"  {i+1}) {name.capitalize()}{'*' if name == current_default_engine else ''}"
        )
    prompt = f"Select engine number (1-{len(SUPPORTED_ENGINES_UI_LIST)})"
    default_idx_str = str(
        SUPPORTED_ENGINES_UI_LIST.index(current_default_engine) + 1
        if current_default_engine in SUPPORTED_ENGINES_UI_LIST
        else 1
    )
    choice = get_user_input(
        prompt,
        [str(i + 1) for i in range(len(SUPPORTED_ENGINES_UI_LIST))],
        allow_empty=True,
        default_val_on_empty=default_idx_str,
    )
    if choice == cfg.SHORTCUT_QUIT:
        return None
...
def select_model_interactively(
    selected_engine: str, current_default_model: Optional[str] = None
) -> Optional[str]:
    ...
    placeholders = {
        "pytorch": cfg.DEFAULT_MODEL_NAME,
        "tensorflow": cfg.DEFAULT_MODEL_NAME,
        "jax": cfg.DEFAULT_MODEL_NAME,
        "llamacpp": cfg.DEFAULT_GGUF_MODEL_PLACEHOLDER,
        "onnx": cfg.DEFAULT_ONNX_MODEL_PLACEHOLDER,
        "mlx": cfg.DEFAULT_MLX_MODEL_PLACEHOLDER,
    }
    effective_default = current_default_model or placeholders.get(
        selected_engine, "e.g., suitable_model_name_or_path"
    )

    if selected_engine in ["pytorch", "tensorflow", "jax", "mlx"]:
        model_prompt += (
            f" (HuggingFace name or local path, e.g., '{effective_default}')"
        )
    elif selected_engine == "llamacpp":
        model_prompt += f" (Path to GGUF file, e.g., '{effective_default}')"
    elif selected_engine == "onnx":
        model_prompt += (
            f" (Path to ONNX model directory/file, e.g., '{effective_default}')"
        )

    model_identifier = get_user_input(
        model_prompt,
        valid_choices=None,
        allow_quit=True,
        allow_empty=True,
        default_val_on_empty=effective_default,
    )
    if model_identifier == cfg.SHORTCUT_QUIT:
        return None
    return model_identifier


def display_round_header(round_num: int, max_rounds: int):
    print_header(f"Round {round_num} of {max_rounds}")


def display_current_sentence(sentence: str):
    print(f"\n📜 {color_text('Current Context:', cfg.COLOR_CYAN)}")
    wrap_print(f'"{sentence}..."', indent="   ")


def display_attention_heatmap(
    token_texts: List[str], normalized_scores: List[float], verbose: bool
):
    if (
        not token_texts
        or not normalized_scores
        or len(token_texts) != len(normalized_scores)
    ):
        return
    print(
    ...
    if verbose:
        wrap_print(
            "Color intensity indicates model focus on previous tokens to predict the next one. Higher score = more focus.",
            indent="   ",
        )
    ...
    color_map = [
        (0.2, cfg.COLOR_MAGENTA_DIM),
        (0.4, cfg.COLOR_MAGENTA_LIGHT),
        (0.6, cfg.COLOR_MAGENTA_MEDIUM),
        (0.8, cfg.COLOR_MAGENTA_BRIGHT),
        (1.01, cfg.COLOR_MAGENTA_INTENSE),
    ]
    ...
    for token, score in zip(token_texts, normalized_scores):
        ...
        if cfg.USE_COLORS:
            colored_part = color_text(f"{formatted_token} [{score:.2f}]", chosen_color)
        else:  # No color fallback
            stars = "*" * int(score * 5)  # Simple 0-5 star rating
            colored_part = f"{formatted_token} [{score:.2f}] {stars}"
        colored_tokens.append(colored_part)

    # Smart wrapping for attention display
    line_limit = 78
    current_line_len = 0
    line_buffer = []
    for item in colored_tokens:
        import re  # For stripping ANSI codes to get visible length

        visible_len = (
            len(re.sub(r"\x1b\[[0-9;]*m", "", item)) + 2
        )  # +2 for space and potential bracket
...
def display_probability_stage(
    ...
    if verbose:
        stage_map = {
            "Raw": "Initial guess.",
            "Temperature": "Adjusted by Temp.",
            "Top-K": "Top-K likely.",
            "Top-P": "Smallest set summing to Top-P.",
        }
        for k_map, desc in stage_map.items():
            if k_map in stage_name:
                wrap_print(desc, indent="   ")
                break
        if "[Final" in stage_name:
            wrap_print("Final distribution for selection.", indent="   ")

    if not tokens or not probs:
        print(
            color_text(
                "   (No valid tokens at this stage or all filtered out)",
                cfg.COLOR_YELLOW,
            )
        )
        return
    ...
    for i in range(num_to_show):
        print(f"   {'*' if i == 0 else ' '} {tokens[i]:<{max_tl}} : {probs[i]:.4f}")
    if len(tokens) > num_to_show:
        print(f"   ... ({len(tokens) - num_to_show} more not shown)")
    if verbose:
        print(
            f"   (Cumulative probability of top {num_to_show}: {total_prob_shown:.3f})"
        )
...
def display_player_choices(
    ...
    if focus_words:
        print(
            color_text(
                "   (Focus Words Mode: Choices favor common word tokens)",
                cfg.COLOR_CYAN,
            )
        )
    ...
    for i, choice_token_texts_list in enumerate(choices_texts):
        ...
        for idx, token_str in enumerate(choice_token_texts_list):
            # Smart spacing based on previous token and current token characteristics
            if (
                idx > 0
                and not token_str.startswith(" ")
                and formatted_sequence
                and not formatted_sequence.endswith(" ")
                and (
                    formatted_sequence[-1].isalnum() or token_str[0].isalnum()
                    if token_str
                    else False
                )
            ):
                formatted_sequence += " "  # Add space if needed for readability
...
def display_guess_result(
    chosen_tokens_texts: List[str],
    correct_tokens_texts: List[str],
    score: int,
    max_score: int,
    is_perfect: bool,
):
    ...
    # Join tokens for display, attempting smart spacing
    def smart_join(tokens: List[str]) -> str:
        ...
        for i, token in enumerate(tokens):
            if (
                ...
                and (res[-1].isalnum() or (token and token[0].isalnum()))
            ):
                res += " "
            res += token
        return res.strip()

    chosen_str = smart_join(chosen_tokens_texts)
    correct_str = smart_join(correct_tokens_texts)
    ...
    if is_perfect:
        ...
        print(
            color_text(
                f"🎯 Partial Match! Score: {score}/{max_score}", cfg.COLOR_YELLOW
            )
        )
    else:
        print(color_text(f"❌ No Match. Score: {score}/{max_score}", cfg.COLOR_RED))


def display_final_score(
    total_score: int, total_max_score: int, final_text: str, game_duration: float
):
        ...
        print(
            color_text(
                ...
                (
                    cfg.COLOR_GREEN
                    if perc >= 75
                    else cfg.COLOR_YELLOW if perc >= 50 else cfg.COLOR_BLUE
                ),
            )
        )
    else:
...
def display_loading_error(model_identifier: str, error: Exception):
    ...
    wrap_print(color_text(str(error), cfg.COLOR_RED), indent="   ")
    wrap_print(
        color_text(
            "Check: model ID, internet, disk/memory, libraries, API keys, and engine-specific requirements.",
            cfg.COLOR_RED,
        ),
        indent="   ",
    )
...
def display_engine_error(engine_name: str, error: Exception):
    ...
    wrap_print(
        color_text(
            f"Ensure libraries for '{engine_name}' are installed correctly and compatible (see requirements-*.txt).",
            cfg.COLOR_RED,
        ),
        indent="   ",
    )
...
def display_token_explanation_if_needed(
    engine: LLMEngine,
    token_id: Any,
    token_text: str,
    previously_explained_tokens: set,
    is_part_of_player_choice: bool = False,
):
    """Explains a token if it's special/punctuation and new to the user, especially in focus_words mode."""
    is_special_or_punct = not engine.is_word_like_token(token_id, token_text)
    try:
        hashable_token_id = int(
    ...
    if is_special_or_punct and hashable_token_id not in previously_explained_tokens:
        prefix = (
            "  Player Choice Note: " if is_part_of_player_choice else "  Model Note: "
        )
        ...
        print(color_text(explanation, cfg.COLOR_YELLOW))
